	...
	for roomIndex, room := range hs.Rooms {
		...
		for eventIndex, event := range room.Events {
			...
			if event.StateKey != nil {
				path = "/chat/client/r0/rooms/$roomId/state/$eventType/$stateKey"
				subs["$stateKey"] = *event.StateKey
			} else {
				path = "/chat/client/r0/rooms/$roomId/send/$eventType/$txnId"
			...
			if event.Type == "m.room.member" && event.StateKey != nil &&
				...
				if ok {
					switch membership {
					case "join":
						path = "/chat/client/r0/join/$roomId"
						method = "POST"
					...
					case "leave":
						path = "/chat/client/r0/rooms/$roomId/leave"
						method = "POST"
						if *event.StateKey != event.Sender {
							// it's a kick
							path = "/chat/client/r0/rooms/$roomId/kick"
							method = "POST"
							event.Content["user_id"] = *event.StateKey
						}
					case "invite":
						path = "/chat/client/r0/rooms/$roomId/invite"
						method = "POST"
						event.Content["user_id"] = *event.StateKey
					}
				}
				...
				*event.StateKey == "" {
				...
				if ok {
					...
					instrs = append(instrs, instruction{
						method:        "PUT",
						path:          "/chat/client/r0/directory/room/" + url.PathEscape(alias),
...
func instructionOneTimeKeyUpload(hs b.Homeserver, user b.User) instruction {
	...
	return instruction{
		method:      "POST",
		path:        "/chat/client/r0/keys/upload",
