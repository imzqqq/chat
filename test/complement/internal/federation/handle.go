...
func HandleMakeSendJoinRequests() func(*Server) {
	return func(s *Server) {
		s.mux.Handle("/chat/federation/v1/make_join/{roomID}/{userID}", http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
			MakeJoinRequestsHandler(s, w, req)
		})).Methods("GET")

		s.mux.Handle("/chat/federation/v2/send_join/{roomID}/{eventID}", http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
...
func HandleInviteRequests(inviteCallback func(*gomatrixserverlib.Event)) func(*Server) {
	return func(s *Server) {
		...
		s.mux.Handle("/chat/federation/v2/invite/{roomID}/{eventID}", http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
			fedReq, errResp := gomatrixserverlib.VerifyHTTPRequest(
				req, time.Now(), gomatrixserverlib.ServerName(s.ServerName), s.keyRing,
			)
			if fedReq == nil {
...
func HandleDirectoryLookups() func(*Server) {
	return func(s *Server) {
		...
		s.mux.Handle("/chat/federation/v1/query/directory", http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
...
// HandleEventRequests is an option which will process GET /chat/federation/v1/event/{eventId} requests universally when requested.
func HandleEventRequests() func(*Server) {
	return func(srv *Server) {
		srv.mux.Handle("/chat/federation/v1/event/{eventID}", http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
			vars := mux.Vars(req)
...
// HandleKeyRequests is an option which will process GET /chat/key/v2/server requests universally when requested.
func HandleKeyRequests() func(*Server) {
	return func(srv *Server) {
		keymux := srv.mux.PathPrefix("/chat/key/v2").Subrouter()
...
// HandleMediaRequests is an option which will process /chat/media/v1/download/* using the provided map
...
func HandleMediaRequests(mediaIds map[string]func(w http.ResponseWriter)) func(*Server) {
	return func(srv *Server) {
		mediamux := srv.mux.PathPrefix("/chat/media").Subrouter()
...
// HandleTransactionRequests is an option which will process GET /chat/federation/v1/send/{transactionID} requests universally when requested.
...
func HandleTransactionRequests(pduCallback func(*gomatrixserverlib.Event), eduCallback func(gomatrixserverlib.EDU)) func(*Server) {
	return func(srv *Server) {
		srv.mux.Handle("/chat/federation/v1/send/{transactionID}", http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
