...
func (c *CSAPI) UploadContent(t *testing.T, fileBody []byte, fileName string, contentType string) string {
	...
	res := c.MustDoFunc(
		t, "POST", []string{"chat", "media", "r0", "upload"},
...
func (c *CSAPI) DownloadContent(t *testing.T, mxcUri string) ([]byte, string) {
	...
	res := c.MustDo(t, "GET", []string{"chat", "media", "r0", "download", origin, mediaId}, struct{}{})
...
func (c *CSAPI) CreateRoom(t *testing.T, creationContent interface{}) string {
	t.Helper()
	res := c.MustDo(t, "POST", []string{"chat", "client", "r0", "createRoom"}, creationContent)
	body := ParseJSON(t, res)
...
func (c *CSAPI) JoinRoom(t *testing.T, roomIDOrAlias string, serverNames []string) string {
	...
	// join the room
	res := c.MustDoFunc(t, "POST", []string{"chat", "client", "r0", "join", roomIDOrAlias}, WithQueries(query))
...
func (c *CSAPI) LeaveRoom(t *testing.T, roomID string) {
	t.Helper()
	// leave the room
	c.MustDoFunc(t, "POST", []string{"chat", "client", "r0", "rooms", roomID, "leave"})
}
...
func (c *CSAPI) InviteRoom(t *testing.T, roomID string, userID string) {
	...
	c.MustDo(t, "POST", []string{"chat", "client", "r0", "rooms", roomID, "invite"}, body)
}
...
func (c *CSAPI) SendEventSynced(t *testing.T, roomID string, e b.Event) string {
	t.Helper()
	c.txnID++
	paths := []string{"chat", "client", "r0", "rooms", roomID, "send", e.Type, strconv.Itoa(c.txnID)}
	if e.StateKey != nil {
		paths = []string{"chat", "client", "r0", "rooms", roomID, "state", e.Type, *e.StateKey}
	}
...
func (c *CSAPI) SyncUntil(t *testing.T, since, filter, key string, check func(gjson.Result) bool) {
	...
	for {
		...
		res := c.MustDoFunc(t, "GET", []string{"chat", "client", "r0", "sync"}, WithQueries(query))
		body := ParseJSON(t, res)
...
func (c *CSAPI) RegisterUser(t *testing.T, localpart, password string) (userID, accessToken string) {
	...
	res := c.MustDo(t, "POST", []string{"chat", "client", "r0", "register"}, reqBody)
