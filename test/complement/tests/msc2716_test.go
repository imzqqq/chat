...
func TestBackfillingHistory(t *testing.T) {
	...
	t.Run("parallel", func(t *testing.T) {
		...
		t.Run("Backfilled historical events resolve with proper state in correct order", func(t *testing.T) {
			...
			messagesRes := alice.MustDoFunc(t, "GET", []string{"chat", "client", "r0", "rooms", roomID, "messages"}, client.WithContentType("application/json"), client.WithQueries(url.Values{
				"dir":   []string{"b"},
				"limit": []string{"100"},
			}))
		...
		t.Run("Historical messages are visible when joining on federated server - auto-generated base insertion event", func(t *testing.T) {
			...
			messagesRes := remoteCharlie.MustDoFunc(t, "GET", []string{"chat", "client", "r0", "rooms", roomID, "messages"}, client.WithContentType("application/json"), client.WithQueries(url.Values{
				"dir":   []string{"b"},
				"limit": []string{"100"},
			}))
		...
		t.Run("Historical messages are visible when joining on federated server - pre-made insertion event", func(t *testing.T) {
			...
			insertionSendRes := as.MustDoFunc(t, "PUT", []string{"chat", "client", "r0", "rooms", roomID, "send", insertionEvent.Type, "txn-m123"}, client.WithJSONBody(t, insertionEvent.Content))
		...
		t.Run("Historical messages are visible when already joined on federated server", func(t *testing.T) {
			...
			remoteCharlie.MustDoFunc(t, "GET", []string{"chat", "client", "r0", "rooms", roomID, "messages"}, client.WithContentType("application/json"), client.WithQueries(url.Values{
				"dir": []string{"b"},
			...
			beforeMarkerMessagesRes := remoteCharlie.MustDoFunc(t, "GET", []string{"chat", "client", "r0", "rooms", roomID, "messages"}, client.WithContentType("application/json"), client.WithQueries(url.Values{
				"dir":   []string{"b"},
				"limit": []string{"100"},
			}))
			...
			// We can't use as.SendEventSynced(...) because application services can't use the /sync API
			markerSendRes := as.MustDoFunc(t, "PUT", []string{"chat", "client", "r0", "rooms", roomID, "send", markerEvent.Type, "txn-m123"}, client.WithJSONBody(t, markerEvent.Content))
...
func fetchUntilMessagesResponseHas(t *testing.T, c *client.CSAPI, roomID string, check func(gjson.Result) bool) {
	...
	for {
		...
		messagesRes := c.MustDoFunc(t, "GET", []string{"chat", "client", "r0", "rooms", roomID, "messages"}, client.WithContentType("application/json"), client.WithQueries(url.Values{
			"dir":   []string{"b"},
			"limit": []string{"100"},
		}))
		messsageResBody := client.ParseJSON(t, messagesRes)
...
func ensureVirtualUserRegistered(t *testing.T, c *client.CSAPI, virtualUserLocalpart string) {
	res := c.DoFunc(
		t,
		"POST",
		[]string{"chat", "client", "r0", "register"},
...
func batchSendHistoricalMessages(
	...
	res = c.DoFunc(
		t,
		"POST",
		[]string{"chat", "client", "unstable", "org.matrix.msc2716", "rooms", roomID, "batch_send"},
