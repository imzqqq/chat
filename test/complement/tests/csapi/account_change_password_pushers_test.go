...
func TestChangePasswordPushers(t *testing.T) {
	...
	t.Run("Pushers created with a different access token are deleted on password change", func(t *testing.T) {
		...
		reqBody := client.WithJSONBody(t, map[string]interface{}{
			"data": map[string]interface{}{
				"url": "https://dummy.url/chat/push/v1/notify",
			},
			"profile_tag":         "tag",
			"kind":                "http",
			"app_id":              "complement",
		...
		_ = sessionOptional.MustDoFunc(t, "POST", []string{"chat", "client", "r0", "pushers", "set"}, reqBody)

		changePassword(t, passwordClient, password1, password2)

		pushersSize := 0

		res := passwordClient.DoFunc(t, "GET", []string{"chat", "client", "r0", "pushers"})
	...
	t.Run("Pushers created with a the same access token are not deleted on password change", func(t *testing.T) {
		...
		_ = passwordClient.MustDoFunc(t, "POST", []string{"chat", "client", "r0", "pushers", "set"}, reqBody)
