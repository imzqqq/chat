...
func TestE2EKeyBackupReplaceRoomKeyRules(t *testing.T) {
	...
	res := alice.MustDoFunc(t, "POST", []string{"chat", "client", "r0", "room_keys", "version"}, client.WithJSONBody(t, map[string]interface{}{
	...
	t.Run("parallel", func(t *testing.T) {
		for i := range testCases {
			...
			t.Run(fmt.Sprintf("%+v", tc.input), func(t *testing.T) {
				...
				alice.MustDoFunc(t, "PUT", []string{"chat", "client", "r0", "room_keys", "keys", roomID, tc.sessionID},
				...
				for _, testKey := range tc.keysThatDontReplace {
					...
					checkResp := alice.MustDoFunc(t, "GET", []string{"chat", "client", "r0", "room_keys", "keys", roomID, tc.sessionID},
