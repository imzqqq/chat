...
func TestProfileAvatarURL(t *testing.T) {
	...
	t.Run("PUT /profile/:user_id/avatar_url sets my avatar", func(t *testing.T) {
		...
		res := authedClient.MustDoFunc(t, "PUT", []string{"chat", "client", "r0", "profile", authedClient.UserID, "avatar_url"}, reqBody)
	...
	t.Run("GET /profile/:user_id/avatar_url publicly accessible", func(t *testing.T) {
		res := unauthedClient.DoFunc(t, "GET", []string{"chat", "client", "r0", "profile", authedClient.UserID, "avatar_url"})
