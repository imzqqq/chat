...
func TestSyncFilter(t *testing.T) {
	...
	t.Run("Can download filter", func(t *testing.T) {
		...
		res := authedClient.MustDo(t, "GET", []string{"chat", "client", "r0", "user", "@alice:hs1", "filter", filterID}, nil)
...
func createFilter(t *testing.T, authedClient *client.CSAPI, reqBody []byte, userID string) string {
	t.Helper()
	res := authedClient.MustDo(t, "POST", []string{"chat", "client", "r0", "user", userID, "filter"}, json.RawMessage(reqBody))
