...
func TestDeviceManagement(t *testing.T) {
	...
	t.Run("GET /device/{deviceId}", func(t *testing.T) {
		...
		_ = unauthedClient.MustDoFunc(t, "POST", []string{"chat", "client", "r0", "login"}, reqBody)

		res := authedClient.MustDoFunc(t, "GET", []string{"chat", "client", "r0", "devices", deviceID})
	...
	t.Run("GET /device/{deviceId} gives a 404 for unknown devices", func(t *testing.T) {
		...
		res := authedClient.DoFunc(t, "GET", []string{"chat", "client", "r0", "devices", "unknown_device"})
	...
	t.Run("GET /devices", func(t *testing.T) {
		...
		res := authedClient.MustDoFunc(t, "GET", []string{"chat", "client", "r0", "devices"})
		must.MatchResponse(t, res, match.HTTPResponse{
			StatusCode: 200,
			JSON: []match.JSON{
	...
	t.Run("PUT /device/{deviceId} updates device fields", func(t *testing.T) {
		...
		_ = authedClient.MustDoFunc(t, "PUT", []string{"chat", "client", "r0", "devices", deviceID}, reqBody)
	...
	t.Run("PUT /device/{deviceId} gives a 404 for unknown devices", func(t *testing.T) {
		...
		res := authedClient.DoFunc(t, "PUT", []string{"chat", "client", "r0", "devices", "unknown_device"}, reqBody)
