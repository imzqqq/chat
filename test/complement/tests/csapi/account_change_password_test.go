...
func TestChangePassword(t *testing.T) {
	...
	t.Run("After changing password, can't log in with old password", func(t *testing.T) {
		...
		res := unauthedClient.DoFunc(t, "POST", []string{"chat", "client", "r0", "login"}, reqBody)
	...
	t.Run("After changing password, existing session still works", func(t *testing.T) {
		res := passwordClient.DoFunc(t, "GET", []string{"chat", "client", "r0", "account", "whoami"})
		must.MatchResponse(t, res, match.HTTPResponse{
			StatusCode: 200,
		})
	})
	...
	t.Run("After changing password, a different session no longer works by default", func(t *testing.T) {
		res := sessionTest.DoFunc(t, "GET", []string{"chat", "client", "r0", "account", "whoami"})
		must.MatchResponse(t, res, match.HTTPResponse{
			StatusCode: 401,
		})
	})
	...
	t.Run("After changing password, different sessions can optionally be kept", func(t *testing.T) {
		...
		res := passwordClient.DoFunc(t, "POST", []string{"chat", "client", "r0", "account", "password"}, reqBody)

		must.MatchResponse(t, res, match.HTTPResponse{
			StatusCode: 200,
		})
		res = sessionOptional.DoFunc(t, "GET", []string{"chat", "client", "r0", "account", "whoami"})
...
func createSession(t *testing.T, deployment *docker.Deployment, userID, password string) *client.CSAPI {
	...
	res := authedClient.DoFunc(t, "POST", []string{"chat", "client", "r0", "login"}, reqBody)
