...
func TestPresence(t *testing.T) {
	...
	t.Run("GET /presence/:user_id/status fetches initial status", func(t *testing.T) {
		res := authedClient.DoFunc(t, "GET", []string{"chat", "client", "r0", "presence", "@alice:hs1", "status"})
	...
	t.Run("PUT /presence/:user_id/status updates my presence", func(t *testing.T) {
		...
		res := authedClient.DoFunc(t, "PUT", []string{"chat", "client", "r0", "presence", "@alice:hs1", "status"}, reqBody)
		must.MatchResponse(t, res, match.HTTPResponse{
			StatusCode: 200,
		})
		res = authedClient.DoFunc(t, "GET", []string{"chat", "client", "r0", "presence", "@alice:hs1", "status"})
