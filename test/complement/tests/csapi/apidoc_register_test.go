...
func TestRegistration(t *testing.T) {
	...
	t.Run("parallel", func(t *testing.T) {
		...
		t.Run("POST {} returns a set of flows", func(t *testing.T) {
			t.Parallel()
			res := unauthedClient.DoFunc(t, "POST", []string{"chat", "client", "r0", "register"}, client.WithRawBody(json.RawMessage(`{}`)))
		...
		t.Run("POST /register rejects usernames with special characters", func(t *testing.T) {
			...
			for _, ch := range specialChars {
				res := unauthedClient.DoFunc(t, "POST", []string{"chat", "client", "r0", "register"},
