...
func TestLogin(t *testing.T) {
	...
	t.Run("parallel", func(t *testing.T) {
		...
		t.Run("GET /login yields a set of flows", func(t *testing.T) {
			t.Parallel()
			res := unauthedClient.DoFunc(t, "GET", []string{"chat", "client", "r0", "login"}, client.WithRawBody(json.RawMessage(`{}`)))
		...
		t.Run("POST /login can login as user", func(t *testing.T) {
			t.Parallel()
			res := unauthedClient.MustDo(t, "POST", []string{"chat", "client", "r0", "login"}, json.RawMessage(`{
		...
		t.Run("POST /login as non-existing user is rejected", func(t *testing.T) {
			t.Parallel()
			res := unauthedClient.DoFunc(t, "POST", []string{"chat", "client", "r0", "login"}, client.WithRawBody(json.RawMessage(`{
				"type": "m.login.password",
				"identifier": {
					"type": "m.id.user",
					"user": "i-dont-exist"
