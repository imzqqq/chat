...
func TestProfileDisplayName(t *testing.T) {
	...
	t.Run("PUT /profile/:user_id/displayname sets my name", func(t *testing.T) {
		...
		_ = authedClient.MustDoFunc(t, "PUT", []string{"chat", "client", "r0", "profile", authedClient.UserID, "displayname"}, reqBody)
	})
	...
	t.Run("GET /profile/:user_id/displayname publicly accessible", func(t *testing.T) {
		res := unauthedClient.DoFunc(t, "GET", []string{"chat", "client", "r0", "profile", authedClient.UserID, "displayname"})
