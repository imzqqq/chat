...
func TestEventRelationships(t *testing.T) {
	...
	res := bob.MustDo(t, "POST", []string{"chat", "client", "unstable", "event_relationships"}, map[string]interface{}{
		"event_id":       eventD,
...
func TestFederatedEventRelationships(t *testing.T) {
	...
	waiter := NewWaiter()
	srv.Mux().HandleFunc("/chat/federation/unstable/event_relationships", func(w http.ResponseWriter, req *http.Request) {
		defer waiter.Finish()
	...
	res := alice.MustDo(t, "POST", []string{"chat", "client", "unstable", "event_relationships"}, map[string]interface{}{
