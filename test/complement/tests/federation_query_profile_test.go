...
func TestOutboundFederationProfile(t *testing.T) {
	...
	t.Run("Outbound federation can query profile data", func(t *testing.T) {
		...
		srv.Mux().Handle("/chat/federation/v1/query/profile", http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
			userID := req.URL.Query().Get("user_id")
			if userID != remoteUserID {
				w.WriteHeader(500)
				t.Fatalf("GET /chat/federation/v1/query/profile with wrong user ID, got '%s' want '%s'", userID, remoteUserID)
				return
			}
			...
			if err != nil {
				w.WriteHeader(500)
				t.Fatalf("GET /chat/federation/v1/query/profile failed to marshal response: %s", err)
				return
			}
			w.WriteHeader(200)
			w.Write(resBody)
		...
		res := unauthedClient.MustDo(t, "GET", []string{"chat", "client", "r0", "profile", remoteUserID, "displayname"}, nil)
