...
sub on_request
{
   ...
   if(
      $path eq "/chat/identity/api/v1/pubkey/isvalid" or
      $path eq "/chat/identity/v2/pubkey/isvalid"
   ) {
      $self->on_is_valid( $req );
   }
   elsif( ( $key_name ) = $path =~ m#^/chat/identity/api/v1/pubkey/([^/]*)$# ) {
      $self->on_pubkey( $req, $key_name );
   }
   elsif( ( $key_name ) = $path =~ m#^/chat/identity/v2/pubkey/([^/]*)$# ) {
      $self->check_v2( $req ) and $self->on_pubkey( $req, $key_name );
   }
   elsif( $path eq "/chat/identity/api/v1/lookup" ) {
      $self->on_v1_lookup( $req );
   }
   elsif( $path eq "/chat/identity/v2/lookup" ) {
      $self->check_v2( $req ) and $self->on_v2_lookup( $req );
   }
   elsif( $path eq "/chat/identity/v2/hash_details" ) {
      $self->check_v2( $req ) and $self->on_hash_details( $req );
   }
   elsif( $path eq "/chat/identity/api/v1/store-invite" ) {
      $self->on_store_invite( $req );
   }
   elsif( $path eq "/chat/identity/v2/store-invite" ) {
      $self->check_v2( $req ) and $self->on_store_invite( $req );
   }
   elsif( $path eq "/chat/identity/api/v1/3pid/getValidated3pid" ) {
      $self->on_get_validated_3pid( $req );
   }
   elsif( $path eq "/chat/identity/v2/3pid/getValidated3pid" ) {
      $self->check_v2( $req ) and $self->on_get_validated_3pid( $req );
   }
   elsif ( $path eq "/chat/identity/api/v1/3pid/bind" ) {
      $self->on_bind( $req );
   }
   elsif ( $path eq "/chat/identity/v2/3pid/bind" ) {
      $self->check_v2( $req ) and $self->on_bind( $req );
   }
   elsif (  # v2 /unbind does not require an access_token param
      $path eq "/chat/identity/v2/3pid/unbind" or
      $path eq "/chat/identity/api/v1/3pid/unbind"
   ) {
      $self->on_unbind( $req );
   }
   else {
...
sub on_store_invite
{
   ...
   my $key_validity_url = "https://" . $self->name . "/chat/identity/v2/pubkey/isvalid";
...
call the C</chat/federation/v1/3pid/onbind> endpoint of the HS specified by C<hs_uribase>
(if defined).
...
sub bind_identity
{
   ...
   $self->{http_client}->do_request_json(
      uri     => URI->new( "$hs_uribase/chat/federation/v1/3pid/onbind" ),
      method  => "POST",
      content => \%resp,
   );
}
