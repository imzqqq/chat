...
sub matrix_login_with_cas
{
   ...
   my $HS_URI = $homeserver_info->client_location . "/chat/client/r0/login/cas/ticket?redirectUrl=" . uri_escape($REDIRECT_URL);
...
sub make_ticket_request
{
   ...
   my $login_uri = $homeserver_info->client_location . "/chat/client/r0/login/cas/ticket?session=$session&ticket=$CAS_TICKET";
