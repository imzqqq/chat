   ...
   return repeat( sub {
      ...
      (
         ...
         do_request_json_for( $admin_user,
            method   => "GET",
            full_uri => "/_chat/admin/v1/purge_history_status/$purge_id",
         )
      })->then( sub {
         my ($body) = @_;
...
test "/purge_history",
   ...
   do => sub {
      ...
      matrix_put_room_state( $user, $room_id,
         ...
         do_request_json_for( $user,
            method   => "POST",
            full_uri => "/_chat/admin/v1/purge_history/$room_id/${ \uri_escape( $last_event_id ) }",
            content  => {}
...
test "/purge_history by ts",
   ...
   do => sub {
      ...
      matrix_put_room_state( $user, $room_id,
         ...
         do_request_json_for( $admin,
            method   => "POST",
            full_uri => "/_chat/admin/v1/purge_history/$room_id",
            content  => {
...
multi_test "Shutdown room",
   ...
   do => sub {
      ...
      matrix_create_room( $user,
         ...
         do_request_json_for( $admin,
            method   => "POST",
            full_uri => "/_chat/admin/v1/rooms/$room_id/delete",
            content  => {
