   ...
   do => sub {
      ...
      $user->http->do_request(
         ...
         do_request_json_for( $admin,
            method   => "POST",
            full_uri => "/_chat/admin/v1/quarantine_media/$room_id",
            content  => {},
         );
      ...
      ->then( sub {
         $user->http->do_request(
            method   => "GET",
            full_uri => "/chat/media/r0/download/$content_id",
         )->main::expect_http_404;
      })->SyTest::pass_on_done( "404 on getting quarantined local media" )
      ->then( sub {
         $user->http->do_request(
            method => "GET",
            full_uri => "/chat/media/r0/thumbnail/$content_id",
            params => {
               width  => 32,
               height => 32,
               method => "scale",
