...
def get_client_id_data(url):
    # FIXME(imzqqq): ensure not localhost via `little_boxes.urlutils.is_url_valid`
...
def indieauth_flow():
    ...
    # XXX(imzqqq): a whitelist for me values?

    # TODO(imzqqq): redirect_uri checks
    if not auth["redirect_uri"]:
        abort(400)

    DB.indieauth.insert_one(auth)

    # FIXME(imzqqq): fetch client ID and validate redirect_uri
