...
def admin_update_actor() -> _Response:
    # FIXME(imzqqq): make this a task, and keep track of our own actor_hash at startup
    update = ap.Update(
        actor=MY_PERSON.id,
...
def admin_new() -> _Response:
    ...
    if request.args.get("reply"):
        ...
        # FIXME(imzqqq): if reply of reply, fetch all participants
...
def admin_notifications() -> _Response:
    ...
    # FIXME(imzqqq): show unfollow (performed by the current actor) and liked???
    mentions_query = {
...
@login_required
def u2f_register():
    # TODO(imzqqq): ensure no duplicates
