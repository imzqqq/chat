...
def _delete_process_inbox(delete: ap.Delete, new_meta: _NewMeta) -> None:
    ...
    try:
        # FIXME(imzqqq): call the DB here instead? like for the oubox
...
def _announce_process_inbox(announce: ap.Announce, new_meta: _NewMeta) -> None:
    ...
    # TODO(imzqqq): actually drop it without storing it and better logging, also move the check somewhere else
    # or remove it?
    try:
