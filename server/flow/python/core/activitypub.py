...
def _meta(activity: ap.BaseActivity) -> _NewMeta:
    ...
    except Exception:  # TODO(imzqqq): should be ValueError, but replies trigger a KeyError on object
        pass
...
def post_to_inbox(activity: ap.BaseActivity) -> None:
    ...
    # TODO(imzqqq): support ignore from Honk
...
class MicroblogPubBackend(Backend):
    ...
    def _fetch_iri(self, iri: str) -> ap.ObjectType:  # noqa: C901
        ...
        # TODO(imzqqq): handle the liked collection too
        ...
        if iri.startswith(BASE_URL):
            ...
            # TODO(imzqqq): also check the REPLIES box
...
def build_ordered_collection(
    ...
    # XXX(imzqqq): implements prev with prev=<first item cursor>?
...
def activity_from_doc(raw_doc: Dict[str, Any], embed: bool = False) -> Dict[str, Any]:
    ...
    # Handle Questions
    # TODO(imzqqq): what about object embedded by ID/URL?
...
def update_cached_actor(actor: ap.BaseActivity) -> None:
    ...
    # TODO(imzqqq): Also update following (it's in the object)
...
def handle_replies(create: ap.Create) -> None:
    ...
    # FIXME(imzqqq): can be a 403 too, in this case what to do? not error at least
