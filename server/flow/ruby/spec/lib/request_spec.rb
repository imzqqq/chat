...
describe Request do
  ...
  describe '#perform' do
    ...
    context 'with private host' do
      ...
      it 'raises Flow::ValidationError' do
        resolver = double
        ...
        expect { subject.perform }.to raise_error Flow::ValidationError
      end
    end
  end
  ...
  describe "response's body_with_limit method" do
    it 'rejects body more than 1 megabyte by default' do
      ...
      expect { subject.perform { |response| response.body_with_limit } }.to raise_error Flow::LengthValidationError
    end
    ...
    it 'rejects body by given size' do
      stub_request(:any, 'http://example.com').to_return(body: SecureRandom.random_bytes(2.kilobytes))
      expect { subject.perform { |response| response.body_with_limit(1.kilobyte) } }.to raise_error Flow::LengthValidationError
    end
