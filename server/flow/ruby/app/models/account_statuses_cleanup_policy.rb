...
class AccountStatusesCleanupPolicy < ApplicationRecord
  ...
  def compute_cutoff_id
    ...
    max_id = Flow::Snowflake.id_at(min_status_age.seconds.ago, with_random: false)
  ...
  def old_enough_scope(max_id = nil)
    ...
    # non-snowflake statuses as older than they really are, but Flow
    # has switched to snowflake IDs significantly over 2 years ago anyway.
    max_id = [max_id, Flow::Snowflake.id_at(min_status_age.seconds.ago, with_random: false)].compact.min
