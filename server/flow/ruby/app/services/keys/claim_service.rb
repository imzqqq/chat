...
class Keys::ClaimService < BaseService
  ...
  rescue HTTP::Error, OpenSSL::SSL::SSLError, Flow::Error => e
  ...
  def fetch_resource_with_post(uri)
    build_post_request(uri).perform do |response|
      raise Flow::UnexpectedResponseError, response unless response_successful?(response) || response_error_unsalvageable?(response)
