...
编辑 `/etc/nginx/sites-available/flow`，替换 `example.com` 为你自己的域名，你可以根据自己的需求做出其它的一些调整。
...
这个命令将获取一个证书，自动更新 `/etc/nginx/sites-available/flow` 以使用新证书并重载nginx以使变更生效。
...
* `/etc/systemd/system/flow-web.service`
* `/etc/systemd/system/flow-sidekiq.service`
* `/etc/systemd/system/flow-streaming.service`

最后，启动新systemd服务并将该服务设为开机自动激活：

```bash
systemctl daemon-reload
systemctl start flow-web flow-sidekiq flow-streaming
