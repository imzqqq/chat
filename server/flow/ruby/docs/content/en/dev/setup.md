...
description: Instructions on how to start developing for Flow.
menu:
  docs:
    weight: 20
    parent: dev
...
There are multiple processes that need to be run for the full set of Flow’s functionality, although they can be selectively omitted. To run all of them with just one command, you can install Foreman with `gem install foreman --no-document` and then use:

```text
foreman start
```

In the Flow directory. This will start processes defined in `Procfile.dev`, which will give you: A Rails server, a Webpack server, a streaming API server, and Sidekiq. Of course, you can run any of those things stand-alone depending on your needs.
