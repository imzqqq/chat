...
All microformats classes use a prefix. The prefix indicates the type of the element, independent of hierarchy. These are the microformats classes as used in Flow's codebase.
...
Within `h-entry` or `h-cite`, represents the title of the entry. Unused in Flow.
...
## Additional classes {#flow}

These elements are attached by Flow for parsing metadata, but are not technically part of the Microformats vocabulary.
