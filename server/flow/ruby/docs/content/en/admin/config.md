...
description: Setting environment variables for your Flow installation.
menu:
  docs:
    weight: 30
    parent: admin
...
Flow uses environment variables as its configuration.

For convenience, it can read them from a flat file called `.env.production` in the Flow directory (called a "dotenv" file), but they can always be overridden by a specific process. For example, systemd service files can read environment variables from an `EnvironmentFile` or from inline definitions with `Environment`, so you can have different configuration parameters for specific services. They can also be specified when calling Flow from the command line.
...
`WEB_DOMAIN` is an optional environment variable allowing to install Flow on one domain, while having the users' handles on a different domain, e.g. addressing users as `@alice@example.com` but accessing Flow on `flow.example.com`. This may be useful if your domain name is already used for a different website but you still want to use it as a Flow identifier because it looks better or shorter.

As with `LOCAL_DOMAIN`, `WEB_DOMAIN` cannot be safely changed once set, as this will confuse remote servers that knew of your previous settings and may break communication with them or make it unreliable. As the issues lie with remote servers' understanding of your accounts, re-installing Flow from scratch will not fix the issue. Therefore, please be extremelly cautious when setting up `LOCAL_DOMAIN` and `WEB_DOMAIN`.

To install Flow on `flow.example.com` in such a way it can serve `@alice@example.com`, set `LOCAL_DOMAIN` to `example.com` and `WEB_DOMAIN` to `flow.example.com`. This also requires additional configuration on the server hosting `example.com` to redirect or proxy requests to `https://example.com/.well-known/webfinger` to `https://flow.example.com/.well-known/webfinger`. For instance, with nginx, the configuration could look like the following:

```
location /.well-known/webfinger {
  return 301 https://flow.example.com$request_uri;
}
```

#### `ALTERNATE_DOMAINS`

If you have multiple domains pointed at your Flow server, this setting will allow Flow to recognize itself when users are addressed using those other domains. Separate the domains by commas, e.g. `foo.com,bar.com`

#### `AUTHORIZED_FETCH` {#authorized_fetch}

Also called "secure mode". When set to `true`, the following changes occur:

- Flow will stop generating linked-data signatures for public posts, which prevents them from being re-distributed efficiently but without precise control. Since a linked-data object with a signature is entirely self-contained, it can be passed around without making extra requests to the server where it originates.
- Flow will require HTTP signature authentication on ActivityPub representations of public posts and profiles, which are normally available without any authentication. Profiles will only return barebones technical information when no authentication is supplied.
- Flow will require any REST/streaming API access to have a user context (i.e. having gone through an OAuth authorization screen with an active user), when normally some API endpoints are available without any authentication.
...
When set to `true`, Flow will restrict federation to servers you have manually approved only, as well as disable all public pages and some REST APIs. Limited federation mode is based on secure mode (`AUTHORIZED_FETCH_MODE`).
...
{{< hint style="warning" >}}
This mode is intended for private use only, such as in academic instituations or internal company networks, as it effectively creates a data silo, which is contrary to Flow's mission of decentralization.
{{</ hint >}}
...
Generate with `rake flow:webpush:generate_vapid_key`. Changing it will break push notifications.
...
Environment. Can be `production`, `development`, or `test`. If you are running Flow on your personal computer for development purposes, use `development`. That is also the default. If you are running Flow online, use `production`. Flow will load different configuration defaults based on the environment.
...
If set to true, Flow will answer requests for files in its `public` directory. This may be necessary if the reverse proxy (e.g. nginx) has no file system access to the `public` directory itself, such as in a containerized environment. It is a suboptimal setting because serving static files directly from the file system will always be much faster than serving them through the Ruby on Rails process.

#### `RAILS_LOG_LEVEL`

Determines the amount of logs generated by Flow. Defaults to `info`, which generates a log entry about every request served by Flow and every background job processed by Flow. This can be useful but can get quite noisy and strain the I/O of your machine if there is a lot of traffic/activity. In that case, `warning` is recommended, which will only output information about things that are going wrong, and otherwise stay quiet. Possible values are `debug`, `info`, `warning`, `error`, `fatal` and `unknown`.

#### `TRUSTED_PROXY_IP`

If your Flow web process is on the same machine as your reverse proxy (e.g. nginx), then you don't need this setting. Otherwise, you need to set it to the IP from which your reverse proxy sends requests to Flow's web process, otherwise Flow will record the reverse proxy's own IP as the IP of all requests, which would be bad because IP addresses are used for important rate limits and security functions.
...
If provided, namespaces all Redis keys. This allows sharing the same Redis database between different projects or Flow servers.
...
If set to `true`, Flow will use ElasticSearch for its search functions.
...
Useful if the ElasticSearch server is shared between multiple projects or different Flow servers. Defaults to value of `REDIS_NAMESPACE`.

### StatsD {#statsd}

#### `STATSD_ADDR`

If set, Flow will log some events and metrics into a StatsD instance identified by its hostname and port.

Example value: `localhost:8125`

#### `STATSD_NAMESPACE`

If set, all StatsD keys will be prefixed with this. Defaults to `Flow.production` when `RAILS_ENV` is `production`, `Flow.development` when it's `development`, etc.

## Limits {#limits}

#### `SINGLE_USER_MODE`

If set to `true`, the frontpage of your Flow server will always redirect to the first profile in the database and registrations will be disabled.
...
By default, Flow will automatically detect the visitor's language from browser headers and display the Flow interface in that language (if it's supported). If you are running a language-specific or regional server, that behaviour may mislead visitors who do not speak your language into signing up on your server. For this reason, you may want to set this variable to a specific language.
...
Flow stores home feeds in RAM (specifically, in the Redis database). This makes them very fast to access and update, but it also means that you don't want to keep them there if they're not used, and you don't want to spend resources on inserting new items into home feeds that will not be accessed. For this reason, Flow periodically clears out home feeds of users who haven't been online in a while, and if they re-appear, it regenerates those home feeds from database data. By default, users are considered active if they have been online in the past `7` days.
...
{{< hint style="info" >}}
You must serve the files with CORS headers, otherwise some functions of Flow's web UI will not work. For example, `Access-Control-Allow-Origin: *`
{{</ hint >}}
...
This variable only has any effect when running `rake db:migrate` and it is extremely specific to the Flow upgrade process. There are two types of database migrations, those that run before new code is deployed and running, and those that run after. By default, both types of migrations are executed. If you shut down all Flow processes before running migrations, then there is no difference. The variable makes sense for zero-downtime upgrades. You will see in the upgrade instructions of a specific Flow version if you need to use it or not.
