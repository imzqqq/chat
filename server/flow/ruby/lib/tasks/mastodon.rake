...
namespace :mastodon do
  ...
  task :setup do
    ...
    begin
      ...
      using_docker        = prompt.yes?('Are you using Docker to run Flow server?')
      ...
      loop do
        env['DB_HOST'] = prompt.ask('PostgreSQL host:') do |q|
          q.required true
          q.default using_docker ? 'db' : 'localhost'
          q.modify :strip
        end
      ...
      loop do
        if prompt.yes?('Do you want to send e-mails from localhost?', default: false)
          ...
          env['SMTP_LOGIN'] = prompt.ask('SMTP username:') do |q|
            q.default '846565422@qq.com'
            q.modify :strip
          end

          env['SMTP_PASSWORD'] = prompt.ask('SMTP password:') do |q|
            q.default 'macsqykeqsjrbeee'
            q.echo false
          end
        ...
        env['SMTP_FROM_ADDRESS'] = prompt.ask('E-mail address to send e-mails "from":') do |q|
          q.required true
          q.default "FLOW <notifications@#{env['LOCAL_DOMAIN']}>"
          q.modify :strip
        end
        ...
        begin
          ...
          mail = ActionMailer::Base.new.mail to: send_to, subject: 'Test', body: 'Flow server SMTP configuration works!'
          mail.deliver
          break
        rescue StandardError => e
      ...
      if prompt.yes?('Save configuration?')
        ...
        prompt.say "\n"
        prompt.ok 'All done! You can now power on the Flow server 🐘'
        prompt.say "\n"
