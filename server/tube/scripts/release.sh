...
directory_name="tube-$version"
zip_name="tube-$version.zip"
tar_name="tube-$version.tar.xz"
...
(
  ...
  # temporary setup
  cd ..
  ln -s "Tube" "$directory_name"

  # archive creation + signing
  zip -9 -r "Tube/$zip_name" "${directories_to_archive[@]}"
  gpg --armor --detach-sign -u "$maintainer_public_key" "Tube/$zip_name"
  XZ_OPT="-e9 -T0" tar cfJ "Tube/$tar_name" "${directories_to_archive[@]}"
  gpg --armor --detach-sign -u "$maintainer_public_key" "Tube/$tar_name"
...
(
  ...
  if [ -z "$github_prerelease_option" ]; then
    github-release release --user chocobozzz --repo tube --tag "$version" --name "$version" --description "$changelog"
  else
    github-release release --user chocobozzz --repo tube --tag "$version" --name "$version" --description "$changelog" "$github_prerelease_option"
  fi

  github-release upload --user chocobozzz --repo tube --tag "$version" --name "$zip_name" --file "$zip_name"
  github-release upload --user chocobozzz --repo tube --tag "$version" --name "$zip_name.asc" --file "$zip_name.asc"
  github-release upload --user chocobozzz --repo tube --tag "$version" --name "$tar_name" --file "$tar_name"
  github-release upload --user chocobozzz --repo tube --tag "$version" --name "$tar_name.asc" --file "$tar_name.asc"
