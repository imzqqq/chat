...
Tube lets you embed videos and programmatically control their playback. This documentation covers how to interact with the Tube Embed API.

## Playground

Any Tube embed URL (ie `https://my-instance.example.com/videos/embed/52a10666-3a18-4e73-93da-e8d3c12c305a`) can be viewed as an embedding playground which
allows you to test various aspects of Tube embeds. Simply replace `/embed` with `/test-embed` and visit the URL in a browser.
...
Given an existing Tube embed `<iframe>` **with API enabled** (`https://my-instance.example.com/videos/embed/52a10666-3a18-4e73-93da-e8d3c12c305a?api=1`),
one can use the Tube Embed API to control it by first including the library. You can include it via Yarn with:

```
yarn add @tube/embed-api
```

Now just use the `PeerTubePlayer` class exported by the module:

```typescript
import { PeerTubePlayer } from '@tube/embed-api'
...
```
<script src="https://unpkg.com/@tube/embed-api/build/player.min.js"></script>
...
`duration` field and `ended` `playbackState` are available in Tube >= 2.2.
