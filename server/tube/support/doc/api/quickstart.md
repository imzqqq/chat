...
```bash
$ curl https://tube.example.com/api/v1/oauth-clients/local
```
...
$ curl -X POST \
 -d "client_id=v1ikx5hnfop4mdpnci8nsqh93c45rldf&client_secret=AjWiOapPltI6EnsWQwlFarRtLh4u8tDt&grant_type=password&response_type=code&username=your_user&password=your_password" \
  https://tube.example.com/api/v1/users/token
```
...
```bash
$ curl -H 'Authorization: Bearer 90286a0bdf0f7315d9d3fe8dabf9e1d2be9c97d0' https://tube.example.com/api/v1/jobs/completed
```
...
```bash
$ curl https://tube.example.com/api/v1/videos
```

## Libraries

[Convenience libraries](https://framagit.org/framasoft/tube/clients) are generated automatically from the [OpenAPI specification](https://github.com/Chocobozzz/Tube/blob/develop/support/doc/api/openapi.yaml) for the following languages:

- [python](https://framagit.org/framasoft/tube/clients/python)
- [go](https://framagit.org/framasoft/tube/clients/go)
- [kotlin](https://framagit.org/framasoft/tube/clients/kotlin)
