...
- [Tips](#tips)
  - [Compatibility with Tube](#compatibility-with-tube)
 ...
 * Their name starts with `tube-theme-` instead of `tube-plugin-`
...
A plugin registers functions in JavaScript to execute when Tube (server and client) fires events. There are 3 types of hooks:
 ...
 * `static`: same than `action` but Tube waits their execution
...
All client scripts have scopes so Tube client only loads scripts it needs:
...
Plugins can declare static directories that Tube will serve (images for example)
from `/plugins/{plugin-name}/{plugin-version}/static/`
or `/themes/{theme-name}/{theme-version}/static/` routes.

### CSS

Plugins can declare CSS files that Tube will automatically inject in the client.
...
Plugins can register settings, that Tube will inject in the administration interface.
...
Plugins can store/load JSON data, that Tube will store in its database (so don't put files in there).
...
You can also store files in the plugin data directory (`/{plugins-directory}/data/{npm-plugin-name}`) **in Tube >= 3.2**.
...
If you want to add a classic username/email and password auth method (like [LDAP](https://framagit.org/framasoft/tube/official-plugins/-/tree/master/tube-plugin-auth-ldap) for example):
...
function register (...) {
  ...
  registerIdAndPassAuth({
    ...
    // Tube will try all id and pass plugins in the weight DESC order
    // Exposing this value in the plugin settings could be interesting
    getWeight: () => 60,

    // Optional function called by Tube when the user clicked on the logout button
    onLogout: user => {
      console.log('User %s logged out.', user.username')
    },

    // Optional function called by Tube when the access token or refresh token are generated/refreshed
    hookTokenValidity: ({ token, type }) => {
      if (type === 'access') return { valid: true }
      if (type === 'refresh') return { valid: false }
    },

    // Used by Tube when the user tries to authenticate
    login: ({ id, password }) => {
...
You can also add an external auth method (like [OpenID](https://framagit.org/framasoft/tube/official-plugins/-/tree/master/tube-plugin-auth-openid-connect), [SAML2](https://framagit.org/framasoft/tube/official-plugins/-/tree/master/tube-plugin-auth-saml2) etc):
...
function register (...) {
  ...
  const result = registerExternalAuth({
    ...
    // If the user click on the auth button, Tube will forward the request in this function
...
async function register ({
...

  {
    const builder = (options) => {
      ...
      // Always copy stream when Tube use libfdk_aac or aac encoders
      return {
        copy: true
      }
    }
...
Tube will try different encoders depending on their priority.
...
Tube provides your plugin some helpers. For example:
...
To notify the user with the Tube ToastModule:
...
You can translate some strings of your plugin (Tube will use your `translations` object of your `package.json` file):
...
Tube will send this field value in `body.pluginData['my-field-name']` and fetch it from `video.pluginData['my-field-name']`.
...
Tube provides some selectors (using `id` HTML attribute) on important blocks so plugins can easily change their style.
...
Tube provides some HTML id so plugins can easily insert their own element:
...
Tube plugins and themes should be published on [NPM](https://www.npmjs.com/) so that Tube indexes take into account your plugin (after ~ 1 day). An official plugin index is available on [packages.joinpeertube.org](https://packages.joinpeertube.org/api/v1/plugins), with no interface to present packages.

> The official plugin index source code is available at https://framagit.org/framasoft/tube/plugin-index
 ...
 * Add the appropriate prefix:
   * If you develop a plugin, add `tube-plugin-` prefix to your plugin name (for example: `tube-plugin-mysupername`)
   * If you develop a theme, add `tube-theme-` prefix to your theme name (for example: `tube-theme-mysupertheme`)
 * Clone the quickstart repository
 * Configure your repository
 * Update `README.md`
 * Update `package.json`
 * Register hooks, add CSS and static files
 * Test your plugin/theme with a local Tube installation
 * Publish your plugin/theme on NPM

### Clone the quickstart repository

If you develop a plugin, clone the `tube-plugin-quickstart` repository:

```
$ git clone https://framagit.org/framasoft/tube/tube-plugin-quickstart.git tube-plugin-mysupername
```

If you develop a theme, clone the `tube-theme-quickstart` repository:

```
$ git clone https://framagit.org/framasoft/tube/tube-theme-quickstart.git tube-theme-mysupername
```
...
```
$ cd tube-plugin-mysupername # or cd tube-theme-mysupername
...
Update the `package.json` fields:
   * `name` (should start with `tube-plugin-` or `tube-theme-`)
   * `description`
   * `homepage`
   * `author`
   * `bugs`
   * `engine.tube` (the Tube version compatibility, must be `>=x.y.z` and nothing else)

**Caution:** Don't update or remove other keys, or Tube will not be able to index/install your plugin.
...
It's up to you to check the code you write will be compatible with the Tube NodeJS version, and will be supported by web browsers.
...
```ts
import { RegisterServerOptions } from '@tube/tube-types'
...
```ts
import { Video } from '@tube/tube-types';
import { RegisterClientOptions } from '@tube/tube-types/client';

function register({ registerHook, peertubeHelpers }: RegisterClientOptions) {
  registerHook({
    target: 'action:admin-plugin-settings.init',
    handler: ({ npmName }: { npmName: string }) => {
      if ('tube-plugin-transcription' !== npmName) {
        return;
      }
    },
  });
...
The key should be one of the locales defined in [i18n.ts](https://github.com/Chocobozzz/Tube/blob/develop/shared/models/i18n/i18n.ts).
...
You'll need to have a local Tube instance:
 * Follow the [dev prerequisites](https://github.com/Chocobozzz/Tube/blob/develop/.github/CONTRIBUTING.md#prerequisites)
 (to clone the repository, install dependencies and prepare the database)
 * Build Tube (`--light` to only build the english language):
 ...
 * Run Tube (you can access to your instance on http://localhost:9000):
...
```
$ node ./dist/server/tools/tube.js auth add -u 'http://localhost:9000' -U 'root' --password 'test'
```

Then, you can install or reinstall your local plugin/theme by running:

```
$ node ./dist/server/tools/tube.js plugins install --path /your/absolute/plugin-or-theme/path
```
...
and republish it on NPM. Remember that the Tube index will take into account your new plugin/theme version after ~24 hours.
...
you can deprecate it. The plugin index will automatically remove it preventing users to find/install it from the Tube admin interface:

```bash
$ npm deprecate tube-plugin-xxx@"> 0.0.0" "explain here why you deprecate your plugin/theme"
```
...
### Compatibility with Tube

Unfortunately, we don't have enough resources to provide hook compatibility between minor releases of Tube (for example between `1.2.x` and `1.3.x`).
So please:
...
```
  * Don't try to require parent Tube modules, only use `peertubeHelpers`. If you need another helper or a specific hook, please [create an issue](https://github.com/Chocobozzz/Tube/issues/new/choose)
  * Don't use Tube dependencies. Use your own :)

If your plugin is broken with a new Tube release, update your code and the `peertubeEngine` field of your `package.json` field.
This way, older Tube versions will still use your old plugin, and new Tube versions will use your updated plugin.
...
You can take a look to "official" Tube plugins if you want to take inspiration from them: https://framagit.org/framasoft/tube/official-plugins
