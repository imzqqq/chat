...
Follow the below guides, and check their versions match [required external dependencies versions](https://github.com/Chocobozzz/Tube/blob/master/engines.yaml). You can check them automatically via `sudo npx engineslist`.
...
Now that dependencies are installed, before running Tube you should start PostgreSQL and Redis:
...
```
sudo -H -u tube CC=/opt/rh/devtoolset-7/root/usr/bin/gcc CXX=/opt/rh/devtoolset-7/root/usr/bin/g++ yarn install --production --pure-lockfile
```
...
Now that dependencies are installed, before running Tube you should enable and start PostgreSQL and Redis:
...
- /var/www/tube does not exist yet (expected? done in future steps? documentation?).
...
```
useradd my-tube-user
passwd my-tube-user
usermod my-tube-user -a -G wheel	# Add my-tube-user to sudoers
su my-tube-user
```
...
sudo ln -s /etc/nginx/sites-enabled/tube /etc/nginx/conf.d/tube.conf
```
...
To add the 'tube' user, you first have to create the 'www' folder and once the 'tube' user is added, you have to set the access permissions.
...
1. bootstrap pkg, initialize db and install tube's dependencies, always as root (sudo not yet installed):
...
If `sudo -u postgres createuser -P tube` gives you an `unknown user: postgres` error, you can try `sudo -u _postgres createuser -U tube`.

## Gentoo

1. Add this to ``/etc/portage/sets/tube``:
...
cat << EOF >> /etc/portage/package.keywords/tube
# required by yarn (argument) for Tube
sys-apps/yarn ~amd64
EOF
```

3. Compile the tube set:
