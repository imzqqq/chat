...
export class ServersCommand extends AbstractCommand {
  ...
  async cleanupTests () {
    ...
    if (isGithubCI()) {
      ...
      const origin = this.buildDirectory('logs/tube.log')
      const destname = `tube-${this.server.internalServerNumber}.log`
  ...
  async waitUntilLog (str: string, count = 1, strictCount = true) {
    const logfile = this.buildDirectory('logs/tube.log')
  ...
  getLogContent () {
    return readFile(this.buildDirectory('logs/tube.log'))
  }
