...
import { PeerTubeP2PInfoButtonOptions, PlayerNetworkInfo } from '../tube-videojs-typings'
...
class P2pInfoButton extends Button {
  ...
  createEl () {
    ...
    const subDivWebtorrent = videojs.dom.createEl('div', {
      className: 'vjs-tube-hidden' // Hide the stats before we get the info
    }) as HTMLDivElement
    ...
    this.player_.on('p2pInfo', (event: any, data: PlayerNetworkInfo) => {
      ...
      if (!data) {
        subDivHttp.className = 'vjs-tube-displayed'
        subDivWebtorrent.className = 'vjs-tube-hidden'
      ...
      subDivHttp.className = 'vjs-tube-hidden'
      subDivWebtorrent.className = 'vjs-tube-displayed'
    })
