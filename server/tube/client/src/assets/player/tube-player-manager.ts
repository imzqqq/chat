...
import 'videojs-dock'
import '@tube/videojs-contextmenu'
...
import './tube-plugin'
import './tube-resolutions-plugin'
import './videojs-components/next-previous-video-button'
import './videojs-components/p2p-info-button'
import './videojs-components/tube-link-button'
import './videojs-components/tube-load-progress-bar'
...
import { HlsJsEngineSettings } from '@tube/p2p-media-loader-hlsjs'
...
import { getAverageBandwidthInStore, saveAverageBandwidth } from './tube-player-local-storage'
import {
...
export class PeertubePlayerManager {
  ...
  static async initialize (mode: PlayerMode, options: PeertubePlayerManagerOptions, onPlayerChange: (player: videojs.Player) => void) {
    ...
    if (mode === 'p2p-media-loader') {
      [ p2pMediaLoader ] = await Promise.all([
        import('@tube/p2p-media-loader-hlsjs'),
  ...
  private static async getVideojsOptions (
    ...
    const plugins: VideoJSPluginOptions = {
      tube: {
        mode,
        autoplay, // Use tube plugin autoplay because we could get the file by webtorrent
