...
} from './tube-custom-tags'
import { CustomMarkupComponent } from './tube-custom-tags/shared'
...
export class CustomMarkupService {
  private angularBuilders: { [ selector: string ]: AngularBuilderFunction } = {
    'tube-button': el => this.buttonBuilder(el),
    'tube-video-embed': el => this.embedBuilder(el, 'video'),
    'tube-playlist-embed': el => this.embedBuilder(el, 'playlist'),
    'tube-video-miniature': el => this.videoMiniatureBuilder(el),
    'tube-playlist-miniature': el => this.playlistMiniatureBuilder(el),
    'tube-channel-miniature': el => this.channelMiniatureBuilder(el),
    'tube-videos-list': el => this.videosListBuilder(el)
  }

  private htmlBuilders: { [ selector: string ]: HTMLBuilderFunction } = {
    'tube-container': el => this.containerBuilder(el)
  }
  ...
  private containerBuilder (el: HTMLElement) {
    ...
    root.classList.add('tube-container', layoutClass)
