...
const CONFIG = {
  ...
  DATABASE: {
    DBNAME: config.has('database.name') ? config.get<string>('database.name') : 'tube' + config.get<string>('database.suffix'),
  ...
  PEERTUBE: {
    CHECK_LATEST_VERSION: {
      ENABLED: config.get<boolean>('tube.check_latest_version.enabled'),
      URL: config.get<string>('tube.check_latest_version.url')
    }
  },
  WEBADMIN: {
    CONFIGURATION: {
