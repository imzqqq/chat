...
async function checkActivityPubUrls () {
  ...
  if (WEBSERVER.HOST !== parsed.host) {
    ...
    logger.warn(
      'It seems Tube was started (and created some data) with another domain name. ' +
  ...
  if (isProdInstance()) {
    ...
    for (const key of Object.keys(configStorage)) {
      if (configStorage[key].startsWith('storage/')) {
        logger.warn(
          'Directory of %s should not be in the production directory of Tube. Please check your production configuration file.',
          key
        )
      }
    }
...
async function checkFFmpegVersion () {
  ...
  if (major < 4 || (major === 4 && minor < 1)) {
    logger.warn('Your ffmpeg version (%s) is outdated. Tube supports ffmpeg >= 4.1. Please upgrade.', version)
  }
}
