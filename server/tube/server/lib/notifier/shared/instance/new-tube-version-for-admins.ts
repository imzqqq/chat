...
export class NewPeerTubeVersionForAdmins extends AbstractNotification <NewPeerTubeVersionForAdminsPayload> {
  ...
  log () {
    logger.info('Notifying %s admins of new Tube version %s.', this.admins.length, this.payload.latestVersion)
  }
  ...
  createEmail (to: string) {
    return {
      to,
      template: 'tube-version-new',
      subject: `A new Tube version is available: ${this.payload.latestVersion}`,
      locals: {
