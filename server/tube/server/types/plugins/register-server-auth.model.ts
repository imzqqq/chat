...
interface RegisterServerAuthBase {
  ...
  // Called by Tube when a user from your plugin logged out
  // Returns a redirectUrl sent to the client or nothing
  onLogout?(user: MUser, req: express.Request): Promise<string>

  // Your plugin can hook Tube access/refresh token validity
...
export interface RegisterServerAuthPassOptions extends RegisterServerAuthBase {
  // Weight of this authentication so Tube tries the auth methods in DESC weight order
  getWeight(): number

  // Used by Tube to login a user
