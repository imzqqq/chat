...
describe('Test plugin scripts', function () {
  ...
  it('Should install a theme from stateless CLI', async function () {
    ...
    await server.cli.execWithEnv(`npm run plugin:install -- --npm-name tube-theme-background-red`)
  })

  it('Should have the theme and the plugin registered when we restart tube', async function () {
    this.timeout(30000)
  ...
  it('Should uninstall a plugin from stateless CLI', async function () {
    ...
    await server.cli.execWithEnv(`npm run plugin:uninstall -- --npm-name tube-plugin-test`)
  })

  it('Should have removed the plugin on another tube restart', async function () {
    this.timeout(30000)
