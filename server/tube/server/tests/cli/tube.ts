...
describe('Test CLI wrapper', function () {
  ...
  const cmd = 'node ./dist/server/tools/tube.js'
  ...
  describe('Manage plugins', function () {
    ...
    it('Should install a plugin', async function () {
      ...
      await cliCommand.execWithEnv(`${cmd} plugins install --npm-name tube-plugin-hello-world`)
    })
    ...
    it('Should list installed plugins', async function () {
      ...
      expect(res).to.contain('tube-plugin-hello-world')
    })

    it('Should uninstall the plugin', async function () {
      const res = await cliCommand.execWithEnv(`${cmd} plugins uninstall --npm-name tube-plugin-hello-world`)

      expect(res).to.not.contain('tube-plugin-hello-world')
    })

    it('Should install a plugin in requested version', async function () {
      this.timeout(60000)

      await cliCommand.execWithEnv(`${cmd} plugins install --npm-name tube-plugin-hello-world --plugin-version 0.0.17`)
    })
