...
import { isHTTPSignatureVerified, isJsonLDSignatureVerified, parseHTTPSignature } from '../../../helpers/tube-crypto'
...
describe('Test activity pub helpers', function () {
  describe('When checking the Linked Signature', function () {
    ...
    it('Should fail with an invalid Mastodon signature', async function () {
      ...
      const fromActor = { publicKey, url: 'http://localhost:9002/accounts/tube' }
    ...
    it('Should fail with an invalid Tube signature', async function () {
      const keys = require(buildAbsoluteFixturePath('./ap-json/tube/invalid-keys.json'))
      const body = require(buildAbsoluteFixturePath('./ap-json/tube/announce-without-context.json'))

      const actorSignature = { url: 'http://localhost:9002/accounts/tube', privateKey: keys.privateKey }
      const signedBody = await buildSignedActivity(actorSignature as any, body)

      const fromActor = { publicKey: keys.publicKey, url: 'http://localhost:9002/accounts/tube' }
      const result = await isJsonLDSignatureVerified(fromActor as any, signedBody)

      expect(result).to.be.false
    })

    it('Should succeed with a valid Tube signature', async function () {
