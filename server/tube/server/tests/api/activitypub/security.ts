...
import { buildDigest } from '@server/helpers/tube-crypto'
...
function setKeysOfServer (onServer: PeerTubeServer, ofServer: PeerTubeServer, publicKey: string, privateKey: string) {
  const url = 'http://localhost:' + ofServer.port + '/accounts/tube'
...
function getAnnounceWithoutContext (server: PeerTubeServer) {
  const json = require(buildAbsoluteFixturePath('./ap-json/tube/announce-without-context.json'))
...
describe('Test ActivityPub security', function () {
  ...
  const keys = require(buildAbsoluteFixturePath('./ap-json/tube/keys.json'))
  const invalidKeys = require(buildAbsoluteFixturePath('./ap-json/tube/invalid-keys.json'))
  const baseHttpSignature = () => ({
    algorithm: HTTP_SIGNATURE.ALGORITHM,
    authorizationHeaderName: HTTP_SIGNATURE.HEADER_NAME,
    keyId: 'acct:tube@localhost:' + servers[1].port,
    key: keys.privateKey,
  ...
  before(async function () {
    ...
    const to = { url: 'http://localhost:' + servers[0].port + '/accounts/tube' }
    const by = { url: 'http://localhost:' + servers[1].port + '/accounts/tube', privateKey: keys.privateKey }
  ...
  describe('When checking Linked Data Signature', function () {
    before(async function () {
      ...
      const to = { url: 'http://localhost:' + servers[0].port + '/accounts/tube' }
      const by = { url: 'http://localhost:' + servers[2].port + '/accounts/tube', privateKey: keys.privateKey }
      await makeFollowRequest(to, by)
    })
    ...
    it('Should fail with bad keys', async function () {
      ...
      body.actor = 'http://localhost:' + servers[2].port + '/accounts/tube'

      const signer: any = { privateKey: invalidKeys.privateKey, url: 'http://localhost:' + servers[2].port + '/accounts/tube' }
    ...
    it('Should fail with an altered body', async function () {
      ...
      const signer: any = { privateKey: keys.privateKey, url: 'http://localhost:' + servers[2].port + '/accounts/tube' }
      const signedBody = await buildSignedActivity(signer, body)

      signedBody.actor = 'http://localhost:' + servers[2].port + '/account/tube'
