...
describe('Test server plugins API validators', function () {
  ...
  const npmPlugin = 'tube-plugin-hello-world'
  const pluginName = 'hello-world'
  let npmVersion: string

  const themePlugin = 'tube-theme-background-red'
  ...
  describe('When listing available plugins/themes', function () {
    ...
    it('Should fail with an invalid current tube engine', async function () {
  ...
  describe('When getting a plugin or the registered settings or public settings', function () {
    ...
    it('Should fail with an invalid npm name', async function () {
      ...
      for (const suffix of [ 'tube-plugin-TOTO', 'tube-plugin-TOTO/registered-settings', 'tube-plugin-TOTO/public-settings' ]) {
        await makeGetRequest({
          url: server.url,
          path: path + suffix,
          token: server.accessToken,
  ...
  describe('When updating plugin settings', function () {
    ...
    it('Should fail with an invalid npm name', async function () {
      ...
      await makePutBodyRequest({
        url: server.url,
        path: path + 'tube-plugin-TOTO/settings',
        fields: { settings },
        token: server.accessToken,
        expectedStatus: HttpStatusCode.BAD_REQUEST_400
      })
    ...
    it('Should fail with an invalid npm name', async function () {
      ...
      for (const suffix of [ 'install', 'update', 'uninstall' ]) {
        await makePostBodyRequest({
          url: server.url,
          path: path + suffix,
          fields: { npmName: 'tube-plugin-TOTO' },
          token: server.accessToken,
          expectedStatus: HttpStatusCode.BAD_REQUEST_400
        })
      }
