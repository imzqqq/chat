...
describe('Test follows', function () {
  ...
  describe('Data propagation after follow', function () {
    ...
    it('Should have 2 followings on server 1', async function () {
      ...
      expect(server3Follow.following.name).to.equal('tube')
    ...
    it('Should have the correct follows counts', async function () {
      await expectAccountFollows({ server: servers[0], handle: 'tube@' + servers[0].host, followers: 0, following: 2 })
      await expectAccountFollows({ server: servers[0], handle: 'root@' + servers[1].host, followers: 1, following: 0 })
      await expectAccountFollows({ server: servers[0], handle: 'tube@' + servers[2].host, followers: 1, following: 0 })

      // Server 2 and 3 does not know server 1 follow another server (there was not a refresh)
      await expectAccountFollows({ server: servers[1], handle: 'tube@' + servers[0].host, followers: 0, following: 1 })
      await expectAccountFollows({ server: servers[1], handle: 'root@' + servers[1].host, followers: 1, following: 0 })
      await expectAccountFollows({ server: servers[1], handle: 'tube@' + servers[1].host, followers: 0, following: 0 })

      await expectAccountFollows({ server: servers[2], handle: 'tube@' + servers[0].host, followers: 0, following: 1 })
      await expectAccountFollows({ server: servers[2], handle: 'tube@' + servers[2].host, followers: 1, following: 0 })
    })
    ...
    it('Should have the correct follows counts after the unfollow', async function () {
      await expectAccountFollows({ server: servers[0], handle: 'tube@' + servers[0].host, followers: 0, following: 1 })
      await expectAccountFollows({ server: servers[0], handle: 'root@' + servers[1].host, followers: 1, following: 0 })
      await expectAccountFollows({ server: servers[0], handle: 'tube@' + servers[2].host, followers: 0, following: 0 })

      await expectAccountFollows({ server: servers[1], handle: 'tube@' + servers[0].host, followers: 0, following: 1 })
      await expectAccountFollows({ server: servers[1], handle: 'root@' + servers[1].host, followers: 1, following: 0 })
      await expectAccountFollows({ server: servers[1], handle: 'tube@' + servers[1].host, followers: 0, following: 0 })

      await expectAccountFollows({ server: servers[2], handle: 'tube@' + servers[0].host, followers: 0, following: 0 })
      await expectAccountFollows({ server: servers[2], handle: 'tube@' + servers[2].host, followers: 0, following: 0 })
    })
