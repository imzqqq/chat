...
describe('Test plugins', function () {
  ...
  it('Should list and search available plugins and themes', async function () {
...

    {
      ...
      const p = body.data.find(p => p.npmName === 'tube-theme-background-red')
  ...
  it('Should install a plugin and a theme', async function () {
    ...
    await command.install({ npmName: 'tube-plugin-hello-world' })
    await command.install({ npmName: 'tube-theme-background-red' })
  })
  ...
  it('Should get public settings', async function () {
    const body = await command.getPublicSettings({ npmName: 'tube-plugin-hello-world' })
  ...
  it('Should update the settings', async function () {
    ...
    await command.updateSettings({
      npmName: 'tube-plugin-hello-world',
      settings
    })
  })
  ...
  it('Should get a plugin and a theme', async function () {
    {
      const plugin = await command.get({ npmName: 'tube-plugin-hello-world' })
    ...
    {
      const plugin = await command.get({ npmName: 'tube-theme-background-red' })
  ...
  it('Should update the plugin and the theme', async function () {
    ...
    const packageJSON = await command.getPackageJSON('tube-plugin-hello-world')
    const oldVersion = packageJSON.version

    packageJSON.version = '0.0.1'
    await command.updatePackageJSON('tube-plugin-hello-world', packageJSON)
    ...
    {
      await command.update({ npmName: 'tube-plugin-hello-world' })
      ...
      const updatedPackageJSON = await command.getPackageJSON('tube-plugin-hello-world')
      expect(updatedPackageJSON.version).to.equal(oldVersion)
    }
  })

  it('Should uninstall the plugin', async function () {
    await command.uninstall({ npmName: 'tube-plugin-hello-world' })
  ...
  it('Should uninstall the theme', async function () {
    await command.uninstall({ npmName: 'tube-theme-background-red' })
  })
