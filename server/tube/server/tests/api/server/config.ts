...
function checkInitialConfig (server: PeerTubeServer, data: CustomConfig) {
  expect(data.instance.name).to.equal('Tube')
  expect(data.instance.shortDescription).to.equal(
    'Tube, an ActivityPub-federated video streaming platform using P2P directly in your web browser.'
  )
  expect(data.instance.description).to.equal('Welcome to this Tube instance!')
...
function checkUpdatedConfig (data: CustomConfig) {
  expect(data.instance.name).to.equal('Tube updated')
