...
async function checkServer1And2HasFollowers (servers: PeerTubeServer[], state = 'accepted') {
  ...
  for (const fn of fns) {
    ...
    expect(follow.follower.url).to.equal('http://localhost:' + servers[0].port + '/accounts/tube')
    expect(follow.following.url).to.equal('http://localhost:' + servers[1].port + '/accounts/tube')
  }
}
...
describe('Test follows moderation', function () {
  ...
  it('Should accept a follower', async function () {
    ...
    await commands[1].acceptFollower({ follower: 'tube@localhost:' + servers[0].port })
    await waitJobs(servers)
  ...
  it('Should reject another follower', async function () {
    ...
    await commands[2].rejectFollower({ follower: 'tube@localhost:' + servers[0].port })
    await waitJobs(servers)
