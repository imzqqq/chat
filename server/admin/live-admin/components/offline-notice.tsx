...
export default function Offline({ logs = [], config }: OfflineProps) {
      ...
      content: (
        <div>
          <a
            ...
            rel="noopener noreferrer"
          >
            Learn how you can add your Live stream to other sites you control.
          </a>
        </div>
      ),
    },
  ...
  if (!config?.yp?.enabled) {
    data.push({
      ...
      title: 'Find an audience on the Live Directory',
      content: (
        <div>
          List yourself in the Live Directory and show off your stream. Enable it in{' '}
  ...
  data.push({
    ...
    content: (
      <div>
        If you&apos;re having issues or would like to know how to customize and configure your Live
        server visit <Link href="/help">the help page.</Link>
      </div>
    ),
  });
