...
export class IdentityClient {
    ...
    public getAccount(): Promise<IdentityServerAccount> {
        return this.doRequest("GET", "/chat/identity/v2/account");
    }
    ...
    public getTermsOfService(): Promise<Policies> {
        return this.doRequest("GET", "/chat/identity/v2/terms");
    }
    ...
    public acceptTerms(termsUrls: string[]): Promise<void> {
        return this.doRequest("POST", "/chat/identity/v2/terms", null, {
    ...
    public async lookup(identifiers: Threepid[], allowPlaintext = false): Promise<string[]> {
        const hashInfo = await this.doRequest("GET", "/chat/identity/v2/hash_details");
        ...
        const resp = await this.doRequest("POST", "/chat/identity/v2/lookup", null, body);
    ...
    public async makeEmailInvite(emailAddress: string, roomId: string): Promise<IdentityServerInvite> {
        ...
        return await this.doRequest("POST", "/chat/identity/v2/store-invite", qs, inviteReq);
    }
     ...
     * @param {string} endpoint The endpoint to call. For example: "/chat/identity/v2/account"
    ...
    public static async acquire(oidc: OpenIDConnectToken, serverUrl: string, mxClient: MatrixClient): Promise<IdentityClient> {
        const account = await doHttpRequest(serverUrl, "POST", "/chat/identity/v2/account/register", null, oidc);
