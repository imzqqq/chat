...
class BaseClientAPI:
    ...
    async def versions(self) -> VersionsResponse:
        ...
        resp = await self.api.request(Method.GET, "chat/client/versions")
    ...
    async def _discover(cls, domain: str, session: ClientSession) -> Optional[URL]:
        ...
        try:
            async with session.get(parsed_url / "chat/client/versions") as resp:
                data = VersionsResponse.deserialize(await resp.json())
                if len(data.versions) == 0:
                    raise ValueError("no versions defined in /chat/client/versions response")
