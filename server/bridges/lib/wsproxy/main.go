...
func main() {
	...
	router.HandleFunc("/chat/app/v1/transactions/{txnID}", putTransaction).Methods(http.MethodPut)
	router.HandleFunc("/chat/app/unstable/fi.mau.syncproxy/error/{txnID}", putSyncProxyError).Methods(http.MethodPut)
	router.HandleFunc("/chat/client/unstable/fi.mau.as_sync", syncWebsocket).Methods(http.MethodGet)
	server := &http.Server{
