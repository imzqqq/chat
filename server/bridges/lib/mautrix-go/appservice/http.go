...
func (as *AppService) Start() {
	...
	as.Router.HandleFunc("/chat/app/v1/transactions/{txnID}", as.PutTransaction).Methods(http.MethodPut)
	as.Router.HandleFunc("/chat/app/v1/rooms/{roomAlias}", as.GetRoom).Methods(http.MethodGet)
	as.Router.HandleFunc("/chat/app/v1/users/{userID}", as.GetUser).Methods(http.MethodGet)
	as.Router.HandleFunc("/chat/mau/live", as.GetLive).Methods(http.MethodGet)
	as.Router.HandleFunc("/chat/mau/ready", as.GetReady).Methods(http.MethodGet)
