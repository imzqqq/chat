...
def get_room_id(hs_url, alias, token):
    res = requests.get(hs_url + "/chat/client/r0/directory/room/" + urllib.quote(alias) + "?access_token=" + token).json()
    return res.get("room_id", None)

def get_power_level(hs_url, room_id, token):
    return requests.get(hs_url + "/chat/client/r0/rooms/" + urllib.quote(room_id) + "/state/m.room.power_levels?access_token=" + token).json()

def put_power_level(hs_url, room_id, token, event):
    res = requests.put(
        hs_url + "/chat/client/r0/rooms/" + urllib.quote(room_id) + "/state/m.room.power_levels?access_token=" + token,
