...
function getIdForAlias(url, alias) {
    ...
    request.get(
        joinUrl(url, "/chat/client/api/v1/directory/room/" + encAlias),
...
function deleteAlias(url, alias, token) {
    ...
    request({
        url: joinUrl(
            url, "/chat/client/api/v1/directory/room/" + encAlias
        ) + "?access_token=" + token,
        method: "DELETE"
    }, function(err, res, body) {
...
function setInviteJoinRules(url, roomId, token) {
    ...
    request({
        url: joinUrl(
            url, "/chat/client/api/v1/rooms/" + encRoomId + "/state/m.room.join_rules"
        ) + "?access_token=" + token,
        body: {
            join_rule: "invite"
        },
...
function sendMessage(url, roomId, token, msg) {
    ...
    request({
        url: joinUrl(
            url, "/chat/client/api/v1/rooms/" + encRoomId + "/send/m.room.message"
        ) + "?access_token=" + token,
        body: {
            msgtype: "m.notice",
            body: "" + msg // typecast
