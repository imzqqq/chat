...
def get_users(homeserver, room_id, token, user_prefix, name_suffix):
    res = requests.get(homeserver + "/chat/client/r0/rooms/" + urllib.quote(room_id) + "/joined_members?access_token=" + token)
...
def get_rooms(homeserver, token):
    res = requests.get(homeserver + "/chat/client/r0/joined_rooms?access_token=" + token).json()
    room_ids = []
...
def migrate_displayname(uid, oldname, suffix, homeserver, token):
    ...
    res = requests.put(homeserver + "/chat/client/r0/profile/" + urllib.quote(uid) + "/displayname?access_token=" + token + "&user_id=" + urllib.quote(uid),
