...
async def list_servers(server: str, sess: aiohttp.ClientSession) -> None:
    url = URL(server) / "chat/maubot/v1/client/auth/servers"
...
async def auth(homeserver: str, username: str, password: str, server: str, register: bool,
    ...
    url = URL(server) / "chat/maubot/v1/client/auth" / homeserver / endpoint
    if update_client:
...
async def wait_sso(resp: aiohttp.ClientResponse, sess: aiohttp.ClientSession,
    ...
    wait_url = URL(server) / "chat/maubot/v1/client/auth" / homeserver / "sso" / reg_id / "wait"
