...
export async function resolveServer(request, baseURL) {
    ...
    {
        const {status} = await request(`${baseURL}/chat/client/versions`, {method: "GET"}).response();
        if (status !== 200) {
...
export class HomeServer {
    ...
    async getUserProfile(userId) {
        const {body} = await this._request(`${this.baseURL}/chat/client/r0/profile/${encodeURIComponent(userId)}`).response();
        return body;
    }
    ...
    async getRoomSummary(roomIdOrAlias, viaServers) {
        ...
        const {body, status} = await this._request(`${this.baseURL}/chat/client/unstable/im.nheko.summary/rooms/${encodeURIComponent(roomIdOrAlias)}/summary${query}`).response();
        if (status !== 200) return;
        return body;
    }

    async findPublicRoomById(roomId) {
        const {body, status} = await this._request(`${this.baseURL}/chat/client/r0/directory/list/room/${encodeURIComponent(roomId)}`).response();
        ...
        do {
            ...
            const {body, status} = await this._request(`${this.baseURL}/chat/client/r0/publicRooms?${queryParams}`).response();
    ...
    async getRoomIdFromAlias(alias) {
        const {status, body}  = await this._request(`${this.baseURL}/chat/client/r0/directory/room/${encodeURIComponent(alias)}`).response();
        if (status === 200) {
            return body.room_id;
        }
    }
    ...
    async getPrivacyPolicyUrl(lang = "en") {
        ...
        const {status, body}  = await this._request(`${this.baseURL}/chat/client/r0/register`, options).response();
    ...
    mxcUrlThumbnail(url, width, height, method) {
        ...
        if (parts) {
            ...
            const httpUrl = `${this.baseURL}/chat/media/r0/thumbnail/${encodeURIComponent(serverName)}/${encodeURIComponent(mediaId)}`;
