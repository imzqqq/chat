...
func ExampleClient_BuildURLWithQuery() {
	...
	fmt.Println(out)
	// Output: https://matrix.org/chat/client/r0/sync?filter_id=5
}
...
func ExampleClient_BuildURL() {
	...
	fmt.Println(out)
	// Output: https://matrix.org/chat/client/r0/user/@example:matrix.org/filter
}

func ExampleClient_BuildBaseURL() {
	userID := "@example:matrix.org"
	cli, _ := NewClient("https://matrix.org", userID, "abcdef123456")
	out := cli.BuildBaseURL("chat", "client", "r0", "directory", "room", "#matrix:matrix.org")
	fmt.Println(out)
	// Output: https://matrix.org/chat/client/r0/directory/room/%23matrix:matrix.org
}
