...
var federationPathPrefixV1 = "/chat/federation/v1"
var federationPathPrefixV2 = "/chat/federation/v2"
...
func (ac *FederationClient) Backfill(
	...
	u := url.URL{
		Path:     "/chat/federation/v1/backfill/" + roomID,
		RawQuery: query.Encode(),
	}
	path := u.RequestURI()
...
func (ac *FederationClient) MSC2836EventRelationships(
	...
	res.roomVersion = roomVersion
	path := "/chat/federation/unstable/event_relationships"
...
func (ac *FederationClient) MSC2946Spaces(
	...
	path := "/chat/federation/unstable/org.matrix.msc2946/spaces/" + url.PathEscape(roomID)
	req := NewFederationRequest("POST", dst, path)
	if err = req.SetContent(r); err != nil {
		return
	}
