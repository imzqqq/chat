...
func (fc *Client) LookupUserInfo(
	...
	url := url.URL{
		...
		Path:     "/chat/federation/v1/openid/userinfo",
...
func (fc *Client) GetVersion(
	...
	url := url.URL{
		Scheme: "matrix",
		Host:   string(s),
		Path:   "/chat/federation/v1/version",
	}
	req, err := http.NewRequest("GET", url.String(), nil)
	if err != nil {
		return
...
func (fc *Client) CreateMediaDownloadRequest(
	...
	requestURL := "matrix://" + string(matrixServer) + "/chat/media/v1/download/" + string(matrixServer) + "/" + mediaID + "?allow_remote=false"
