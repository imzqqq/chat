...
const exampleGetRequest = "GET /chat/federation/v1/query/directory?room_alias=%23test%3Alocalhost%3A44033 HTTP/1.1\r\n" +
...
// The headers have been reordered to match the order net/http writes them in.
const examplePutRequest = "PUT /chat/federation/v1/send/1493385816575/ HTTP/1.1\r\n" +
	"Host: localhost:44033\r\n" +
	"Content-Length: 321\r\n" +
	"Authorization: X-Matrix" +
	" origin=\"localhost:8800\"" +
...
func TestSignGetRequest(t *testing.T) {
	request := NewFederationRequest(
		"GET", "localhost:44033",
		"/chat/federation/v1/query/directory?room_alias=%23test%3Alocalhost%3A44033",
	)
...
func TestVerifyGetRequest(t *testing.T) {
	...
	wantPath := "/chat/federation/v1/query/directory?room_alias=%23test%3Alocalhost%3A44033"
...
func TestSignPutRequest(t *testing.T) {
	request := NewFederationRequest(
		"PUT", "localhost:44033", "/chat/federation/v1/send/1493385816575/",
	)
...
func TestVerifyPutRequest(t *testing.T) {
	...
	wantPath := "/chat/federation/v1/send/1493385816575/"
	if request.RequestURI() != wantPath {
		t.Errorf("Wanted request.RequestURI() to be %q got %q", wantPath, request.RequestURI())
	}
}
