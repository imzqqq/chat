...
### Local client sends an event in an existing room.
  ...
  0) The client sends a PUT `/chat/client/r0/rooms/{roomId}/send` request
  ...
  3a) The ClientSync reads the event from the "OutputRoomEvent" kafka topic:
    ...
    * Wakes up any pending `/chat/client/r0/sync` requests for those users.
  ...
  3b) The FederationSender reads the event from the "OutputRoomEvent" kafka topic:
    ...
    * Sends a `/chat/federation/v1/send` request to those servers.
...
### Remote server sends an event in an existing room.
  ...
  0) The remote server sends a `PUT /chat/federation/v1/send` request and an
  ...
  1) The FederationReceiver:
    ...
    * If the RoomServer doesn't know the state of the room at an event then
      ...
      `GET /chat/federation/v1/state_ids` falling back to
      `GET /chat/federation/v1/state` if necessary.
