...
Chat server uses parametrised templates to generate the content of emails it sends and
webpages it shows to users.

By default, Chat server will use the templates listed [here](https://github.com/matrix-org/chat/tree/master/chat/res/templates).
Server admins can configure an additional directory for Chat server to look for templates
...
default templates from within the Chat server package will be used.
...
Below are the templates Chat server will look for when generating the content of an email:
  ...
  When rendering, this template is given the following variables:
    ...
    * `reason`: information on the event that triggered the email to be sent. It's an
      object with the following attributes:
        ...
        * `delay_before_mail_ms`: the amount of time in milliseconds Chat server always waits
...
* `add_threepid.html` and `add_threepid.txt`: The contents of address verification emails
  sent when an address is added to a Chat account.
  When rendering, these templates are given a `link` variable which contains the link the
  user must click in order to validate their email address.
...
Below are the templates Chat server will look for when generating pages related to
...
Below are the templates Chat server will look for when generating pages related to SSO:
...
* `sso_account_deactivated.html`: HTML page shown during single sign-on if a deactivated
  user (according to Chat server's database) attempts to login.
  This template has no additional variables.
