...
Chat server comes with a non-standard login type to support
...
will be removed in a future version of Chat server.
...
The JSON Web Token integration in Chat server uses the
 ...
 * If you installed Chat server into a virtualenv, run `/path/to/env/bin/pip
   install chat[pyjwt]` to install the necessary dependencies.
...
1.  Configure Chat server with JWT logins, note that this example uses a pre-shared
...
3.  Query for the login types and ensure `org.matrix.login.jwt` is there:
    ...
    ```bash
    curl http://localhost:8080/chat/client/r0/login
    ```
4.  Login used the generated JSON web token from above:

    ```bash
    $ curl http://localhost:8082/chat/client/r0/login -X POST \
