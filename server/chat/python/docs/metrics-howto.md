# How to monitor Chat server metrics using Prometheus
...
1.  Enable Chat server metrics:

    There are two methods of enabling metrics in Chat server.
    ...
    This provides a simple way of adding metrics to your Chat server
    installation, and serves under `/_chat/metrics`. If you do not
    ...
    different thread to Chat server. This can make it more resilient to
    heavy load meaning metrics cannot be retrieved, and can be exposed
    to just internal networks easier. The served metrics are available
    over HTTP only, and will be available at `/_chat/metrics`.
...
1.  Add a Prometheus target for Chat server.
    ...
    where `my.server.here` is the IP address of Chat server, and `port` is
...
1.  Consider using the [grafana dashboard](https://github.com/matrix-org/chat/tree/master/contrib/grafana/)
    and required [recording rules](https://github.com/matrix-org/chat/tree/master/contrib/prometheus/) 

## Monitoring workers

To monitor a Chat server installation using [workers](workers.md),
...
Example Prometheus target for Chat server with workers:
...
Chat server 1.2 updates the Prometheus metrics to match the naming
...
The duplicated metrics deprecated in Chat server 0.27.0 have been removed.
...
Chat server 0.27.0 begins the process of rationalising the duplicate
...
As of chat version 0.18.2, the format of the process-wide metrics has
