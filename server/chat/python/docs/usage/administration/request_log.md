...
HTTP request logs are written by chat (see [`site.py`](../chat/http/site.py) for details).

See the following for how to decode the dense data available from the default logging configuration.

```
2020-10-01 12:00:00,000 - chat.access.http.8080 - 311 - INFO - PUT-1000- 192.168.0.1 - 8080 - {another-matrix-server.com} Processed request: 0.100sec/-0.000sec (0.000sec, 0.000sec) (0.001sec/0.090sec/3) 11B !200 "PUT /chat/federation/v1/send/1600000000000 HTTP/1.1" "Chat/1.20.1" [0 dbevts]
...
| BBBB  | Logger name (`chat.access.(http\|https).<tag>`, where 'tag' is defined in the `listeners` config section, normally the port) |
