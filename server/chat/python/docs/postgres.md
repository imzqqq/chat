...
Chat server supports PostgreSQL versions 9.6 or later.

## Install postgres client libraries

Chat server will require the python postgres client library in order to
connect to a postgres database.

-   If you are using the [debian/ubuntu
...
-   If you installed chat [in a
    virtualenv](setup/installation.md#installing-from-source), you can install
    the library with:

        ~/chat/env/bin/pip install "chat-server[postgres]"

    (substituting the path to your virtualenv for `~/chat/env`, if
...
Then, create a postgres user and a database with:
    ...
    createdb --encoding=UTF8 --locale=C --template=template0 --owner=synapse_user chat
...
the connection between your homeserver and database dropping, otherwise Chat server
...
The script `synapse_port_db` allows porting an existing chat server
...
Firstly, shut down the currently running chat server and copy its
database file (typically `homeserver.db`) to another location. Once the
copy is complete, restart chat. For instance:
...
Edit the database section as described in the section *Chat server config*
...
To complete the conversion shut down the chat server and run the port
...
Once that has completed, change the chat config to point at the
...
Chat server should now be running against PostgreSQL.
...
* If the `synapse_user` user has a password, add the password to the `database:`
  ...
  ```
  host    chat     synapse_user    ::1/128     md5  # or `scram-sha-256` instead of `md5` if you use that
  ```
...
* If the `synapse_user` user does not have a password, then a password doesn't
  ...
  ```
  host    chat     synapse_user    ::1/128     trust
  ```
...
Chat server will refuse to set up a new database if it has the wrong values of
...
Chat server uses Postgres sequences to generate IDs for various tables. A sequence
and associated table can get out of sync if, for example, Chat server has been
downgraded and then upgraded again.

To fix the issue shut down Chat server (including any and all workers) and run the
SQL command included in the error message. Once done Chat server should start
successfully.
