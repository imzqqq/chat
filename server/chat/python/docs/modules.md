...
Chat server supports extending its functionality by configuring external modules.

## Using modules

To use a module on Chat server, add it to the `modules` section of the configuration file:
...
custom code on your Chat server homeserver. Server admins are encouraged to verify the
...
configuring modules in another part of Chat server's configuration file. Currently, only the
spam checker interface is compatible with this new system.

## Writing a module

A module is a Python class that uses Chat server's module API to interact with the
homeserver. It can register callbacks that Chat server will call on specific operations, as
well as web resources to attach to Chat server's web server.

When instantiated, a module is given its parsed configuration as well as an instance of
the `chat.module_api.ModuleApi` class. The configuration is a dictionary, and is
either the output of the module's `parse_config` static method (see below), or the
configuration associated with the module in Chat server's configuration file.

See the documentation for the `ModuleApi` class
[here](https://github.com/matrix-org/chat/blob/master/chat/module_api/__init__.py).
...
`chat.module_api.errors.ConfigError` if not.

### Registering a web resource

Modules can register web resources onto Chat server's web server using the following module
API method:
...
register a resource for the path `/_chat/client/my_super_module/say_hello`, Chat server
will serve it at `http(s)://[HS_URL]/_chat/client/my_super_module/say_hello`. Note
that Chat server does not allow registering resources for several sub-paths in the `/chat`
namespace (such as anything under `/chat/client` for example). It is strongly
recommended that modules register their web resources under the `/_chat/client`
namespace.
...
register a resource for the same path, the module that appears first in Chat server's
...
Modules can use Chat server's module API to register callbacks. Callbacks are functions that
Chat server will call when performing specific actions. Callbacks must be asynchronous, and
...
Chat server instances. Spam checker callbacks can be registered using the module API's
`register_spam_checker_callbacks` method.

The available spam checker callbacks are:

```python
async def check_event_for_spam(event: "chat.events.EventBase") -> Union[bool, str]
```
...
represented by their Chat user ID (e.g. `@alice:example.com`).
...
whether the given user (represented by their Chat user ID) is allowed to create a room.

```python
async def user_may_create_room_alias(user: str, room_alias: "chat.types.RoomAlias") -> bool
```

Called when trying to associate an alias with an existing room. The module must return a
`bool` indicating whether the given user (represented by their Chat user ID) is allowed
...
Chat user ID) is allowed to publish the given room.
...
* `user_id`: The Chat ID for this user.
...
) -> "chat.spam_checker_api.RegistrationBehaviour"
```
...
* `username`: The username the user would like to register. Can be `None`, meaning that
  Chat server will generate one later.
...
async def check_media_file_for_spam(
    file_wrapper: "chat.rest.media.v1.media_storage.ReadableFileWrapper",
    file_info: "chat.rest.media.v1._base.FileInfo",
) -> bool
```
...
Chat server instance. Account validity callbacks can be registered using the module API's
...
represented by their Chat user ID.
...
async def check_event_allowed(
    event: "chat.events.EventBase",
    state_events: "chat.types.StateMap",
...
If the boolean returned by the module is `True`, it may also tell Chat server to replace the
...
async def on_create_room(
    requester: "chat.types.Requester",
...
async def get_users_for_states(
    self,
    state_updates: Iterable["chat.api.UserPresenceState"],
) -> Dict[str, Set["chat.api.UserPresenceState"]]:
```
...
must return a dictionary that maps from Chat user IDs (which can be local or remote) to the
`UserPresenceState` changes that they should be forwarded.

Chat server will then attempt to send the specified presence updates to each user when possible.

```python
async def get_interested_users(
        self, 
        user_id: str
) -> Union[Set[str], "chat.module_api.PRESENCE_ALL_USERS"]
```
...
The callback is given the Chat user ID for a local user that is requesting presence data and
should return the Chat user IDs of the users whose presence state they are allowed to
query. The returned users can be local or remote. 

Alternatively the callback can return `chat.module_api.PRESENCE_ALL_USERS`
...
In order to port a module that uses Chat server's old module interface, its author needs to:
...
use the new `modules` section in Chat server's configuration file (see [this section](#using-modules)
for more info).
...
a web resource to the path `/_chat/client/demo/hello` that returns a JSON object.
