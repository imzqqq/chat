...
The Chat Server supports integration with TURN server via the
...
The following sections describe how to install [coturn](<https://github.com/coturn/coturn>) (which implements the TURN REST API) and integrate it with chat.
...
After updating the homeserver configuration, you must restart chat:

  * If you use synctl:
    ```sh
    cd /where/you/run/chat
    ./synctl restart
    ```
  * If you use systemd:
    ```
 ...
 * You can test your Chat homeserver TURN setup with https://test.voip.librepush.net/.
 ...
 * There is a WebRTC test tool at
    ...
    * look for the `GET /chat/client/r0/voip/turnServer` request made by a
