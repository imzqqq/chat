<h2 style="color:red">
This page of the Chat server documentation is now deprecated. For up to date
...
Chat server supports configuring a module that can specify additional users
...
be imported by the running Chat server.
...
* An instance of `chat.module_api.ModuleApi`.
...
Chat User IDs, and values being a python set
of `chat.handlers.presence.UserPresenceState` instances.

Chat server will then attempt to send the specified presence updates to each user when
possible.
...
**Required.** An asynchronous method that is passed a single Chat User ID. This
...
This method should return a python set of Chat User IDs, or the object
`chat.events.presence_router.PresenceRouter.ALL_USERS` to indicate that the passed
...
from chat.handlers.presence import UserPresenceState
...
class ExamplePresenceRouter:
    """An example implementation of chat.presence_router.PresenceRouter.
    ...
    Args:
        ...
        module_api: An instance of Chat server's ModuleApi.
    """
...
Chat server, amend your homeserver config file with the following.
