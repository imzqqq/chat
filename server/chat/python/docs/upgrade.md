...
Chat server. The extra instructions that may be required are listed later in
this document.

-   Check that your versions of Python and PostgreSQL are still
    supported.

    Chat server follows upstream lifecycles for [Python](https://endoflife.date/python) and
...
-   If Chat server was installed using [prebuilt
    packages](setup/installation.md#prebuilt-packages), you will need to follow the
    normal process for upgrading those packages.

-   If Chat server was installed from source, then:

    1.  Activate the virtualenv before upgrading. For example, if
        Chat server is installed in a virtualenv in `~/chat/env` then
        run:
    ...
    2.  If Chat server was installed using pip then upgrade to the latest
        version by running:

        ```bash
        pip install --upgrade chat-server
        ```

        If Chat server was installed using git then upgrade to the latest
        version by running:
...
```bash
# you may need to replace 'localhost:8080' if chat is not configured
# to listen on port 8080.

curl http://localhost:8080/_chat/admin/v1/server_version
```
...
-   Debian:
    ...
    wget https://packages.matrix.org/debian/pool/main/m/chat-server-py3/chat-server-py3_1.3.0+stretch1_amd64.deb
    dpkg -i chat-server-py3_1.3.0+stretch1_amd64.deb
    ```
...
The following admin APIs were deprecated in [Chat server 1.25](https://github.com/matrix-org/chat/blob/v1.25.0/CHANGES.md#removal-warning)
(released on 2021-01-13) and have now been removed:

-   `POST /_chat/admin/v1/purge_room`
...
[reCAPTCHA](../chat/res/templates/recaptcha.html) or
[terms](../chat/res/templates/terms.html) fallback pages.
...
Since Chat server 1.6.0 (2019-11-26) you can set a proxy for outbound HTTP requests via
...
See [using a forward proxy with Chat server documentation](setup/forward_proxy.md) for
details.
...
modules system introduced in Chat server v1.37.0. Authors of third-party rules modules can refer
to [this documentation](modules.md#porting-an-existing-module-that-uses-the-old-interface)
to update their modules. Chat server administrators can refer to [this documentation](modules.md#using-modules)
...
```
chat=# select pg_size_pretty(pg_relation_size('events_order_room'));
 pg_size_pretty
----------------
 288 MB
(1 row)
...
to update their modules. Chat server administrators can refer to [this
...
file](https://github.com/matrix-org/chat/blob/v1.34.0/docs/sample_config.yaml#L1515).
...
`account_validity.account_renewed_html_path` Chat server config options.
...
template](https://github.com/matrix-org/chat/blob/release-v1.33.0/chat/res/templates/account_renewed.html)
...
[here](https://github.com/matrix-org/chat/blob/release-v1.33.0/chat/res/templates/account_previously_renewed.html),
...
regression](https://github.com/matrix-org/chat/issues/9853) that can
overwhelm connected Prometheus instances. This issue is not present in
Chat server v1.32.0rc1.
...
[here](https://github.com/matrix-org/chat/pull/9854#issuecomment-823472183).
...
This release of Chat server requires Python 3.6+ and PostgresSQL 9.6+ or
SQLite 3.22+.

## Removal of old List Accounts Admin API

The deprecated v1 "list accounts" admin API
(`GET /_chat/admin/v1/users/<user_id>`) has been removed in this
version.

The [v2 list accounts API](admin_api/user_admin_api.md#list-accounts)
has been available since Chat server 1.7.0 (2019-12-13), and is accessible
under `GET /_chat/admin/v2/users`.

The deprecation of the old endpoint was announced with Chat server 1.28.0
...
`/chat/client/r0/register` endpoint. This behaviour was deprecated in
Chat server v1.30.0.
...
When using Chat server with a reverse proxy (in particular, when using the
[x_forwarded]{.title-ref} option on an HTTP listener), Chat server now
expects to receive an [X-Forwarded-Proto]{.title-ref} header on incoming
HTTP requests. If it is not set, Chat server will log a warning on each
received request.
...
Chat server also requires the [Host]{.title-ref} header to be preserved.
...
-   If your server is configured for single sign-on via an OpenID
    ...
    `[chat public baseurl]/_chat/client/oidc/callback` to the list
...
-   If your server is configured for single sign-on via a SAML2 identity
    ...
    `[chat public baseurl]/_chat/client/saml2/authn_response` as a
    ...
    `[chat public baseurl]/_chat/client/saml2/metadata.xml`. If
    your SAML2 identity provider uses this property to validate or
    otherwise identify Chat server, its configuration will need to be
...
occurs, you may want to roll-back to a previous version of Chat server.
...
5.  Downgrade Chat server by following the instructions for your
...
This is the last release of Chat server which guarantees support with Python
...
In the future, Chat server will follow upstream schedules for ending support
...
Chat server v1.25.0 includes new settings, `ip_range_blacklist` and
`ip_range_whitelist`, for controlling outgoing requests from Chat server for
...
normalisation of the localpart of the Chat ID. This allows for the
...
If your Chat server configuration uses a custom mapping provider
([oidc_config.user_mapping_provider.module]{.title-ref} is specified and
not equal to
[chat.handlers.oidc_handler.JinjaOidcMappingProvider]{.title-ref})
...
by Chat server. An example is shown below:

```python
from chat.types import map_username_to_mxid_localpart
...
## Removal historical Chat server Admin API

Historically, the Chat server Admin API has been accessible under:

-   `/chat/client/api/v1/admin`
-   `/chat/client/unstable/admin`
-   `/chat/client/r0/admin`
-   `/_chat/admin/v1`

The endpoints with `/chat/client/*` prefixes have been removed as of
...
The deprecation of the old endpoints was announced with Chat server 1.20.0
...
A future will release of Chat server will make using `structured: true` an
error.
...
making use of `ThirdPartyEventRules` in Chat server. If you make use of a
...
## Forwarding `/_chat/client` through your reverse proxy

The [reverse proxy documentation](reverse_proxy.md)
has been updated to include reverse proxy directives for
`/_chat/client/*` endpoints. As the user password reset flow now uses
endpoints under this prefix, **you must update your reverse proxy
configurations for user password reset to work**.

Additionally, note that the [Chat server worker documentation](workers.md) has been updated to

:   state that the `/_chat/client/password_reset/email/submit_token`
    endpoint can be handled

by all workers. If you make use of Chat server's worker feature, please
update your reverse proxy configuration to reflect this change.

## New HTML templates

A new HTML template,
[password_reset_confirmation.html](https://github.com/matrix-org/chat/blob/develop/chat/res/templates/password_reset_confirmation.html),
has been added to the `chat/res/templates` directory. If you are
...
template directories for Chat server to start. The default templates will be
...
`/_chat/client/password_reset/{medium}/submit_token` with the query
parameters from the original link, presented as a URL-encoded form. See
the file itself for more details.

## Updated Single Sign-on HTML Templates

The `saml_error.html` template was removed from Chat server and replaced
with the `sso_error.html` template. If your Chat server is configured to use
...
server. Chat server will not respond to HTTP requests while this update is
taking place.

# Upgrading to v1.13.0

## Incorrect database migration in old chat versions

A bug was introduced in Chat server 1.4.0 which could cause the room
directory to be incomplete or empty if Chat server was upgraded directly
from v1.2.1 or earlier, to versions between v1.4.0 and v1.12.x.

This will *not* be a problem for Chat server installations which were:
...
`sso_account_deactivated.html`) were added to Chat server. If your Chat server
is configured to use SSO and a custom
`sso_redirect_confirm_template_dir` configuration then these templates
will need to be copied from
[chat/res/templates](chat/res/templates) into that directory.

## Chat server SSO Plugins Method Deprecation

Plugins using the `complete_sso_login` method of
`chat.module_api.ModuleApi` should update to using the async/await
...
3.  Downgrade Chat server by following the instructions for your
    installation method in the "Rolling back to older versions"
    section above.
...
large server). Chat server will not respond to HTTP requests while this
...
1.  As an initial check to see if you will be affected, you can try
    ...
    [sqlite3]{.title-ref} console. It is safe to run it while Chat server is
    still running.
    ...
    process: ie, if it takes 5 minutes, then it is likely that Chat server
    will be unresponsive for 5 minutes during the upgrade.

    If you consider an outage of this duration to be acceptable, no
    further action is necessary and you can simply start Chat server 1.12.0.
...
2.  The easiest workaround for this issue is to manually create a new
    ...
    operation may in itself cause Chat server to stop running for some time.
    Chat server admins are reminded that [SQLite is not recommended for use
    outside a test environment](postgres.md).

3.  Once the index has been created, the `SELECT` query in step 1 above
    should complete quickly. It is therefore safe to upgrade to Chat server
    1.12.0.

4.  Once Chat server 1.12.0 has successfully started and is responding to
...
Chat server will now log a warning on start up if used with a PostgreSQL
...
Specifying a `log_file` config option will now cause Chat server to refuse
...
In an attempt to configure Chat server in a privacy preserving way, the
...
Chat server will expect these files to exist inside the configured template
directory, and **will fail to start** if they are absent. To view the
default templates, see
[chat/res/templates](https://github.com/matrix-org/chat/tree/master/chat/res/templates).

## 3pid verification changes

**Note: As of this release, users will be unable to add phone numbers or
email addresses to their accounts, without changes to the Chat server
...
password-reset, Chat server must confirm the operation with the owner of the
...
Previous versions of Chat server delegated the task of 3pid verification to
an identity server by default. In most cases this server is `vector.im`
or `matrix.org`.

In Chat server 1.4.0, for security and privacy reasons, the homeserver will
...
support lands in Chat server.

Once delegated 3pid verification support has been disabled in the
`vector.im` and `matrix.org` identity servers, all Chat server versions that
...
-   Configure Chat server to use an email server.
-   Run or choose an identity server which allows delegated email
    verification and delegate to it.

#### Configure SMTP in Chat server

To configure an SMTP server for Chat server, modify the configuration
...
with existing Chat server installs that set up external server handling for
these tasks before v1.4.0. If
`email.trust_identity_server_for_password_resets` is `true` and no
trusted identity server domains are configured, Chat server will report an
...
`account_threepid_delegates`, then Chat server will send email verification
messages itself, using the configured SMTP server (see above). that
type.

### Phone numbers

Chat server does not support phone-number verification itself, so the only
...
-   The 'room statistics' engine was heavily reworked in this release
    (see [#5971](https://github.com/matrix-org/chat/pull/5971)),
    ...
    versions of Chat server, they will be used to populate the room
...
Chat server v1.1.0 removes support for older Python and PostgreSQL versions,
as outlined in [our deprecation
notice](https://matrix.org/blog/2019/04/08/chat-deprecating-postgres-9-4-and-python-2-x).

## Minimum Python Version

Chat server v1.1.0 has a minimum Python requirement of Python 3.5. Python
3.6 or Python 3.7 are recommended as they have improved internal string
handling, significantly reducing memory usage.

If you use current versions of the Chat.org-distributed Debian
packages or Docker images, action is not required.

If you install Chat server in a Python virtual environment, please see
"Upgrading to v0.34.0" for notes on setting up a new virtualenv under
Python 3.

## Minimum PostgreSQL Version

If using PostgreSQL under Chat server, you will need to use PostgreSQL 9.5
...
Chat server v1.0 is the first release to enforce validation of TLS
...
When a user requests a password reset, Chat server will send an email to the
user to confirm the request.

Previous versions of Chat server delegated the job of sending this email to
...
Therefore, by default, Chat server v1.0 will send the confirmation email
itself. If Chat server is not configured with an SMTP server, password reset
...
custom `template_dir` in your Chat server config, these files will need to be added.
...
Please be aware that, before Chat server v1.0 is released around March 2019,
...
1.  This release is the first to fully support Python 3. Chat server will
    now run on Python versions 3.5, or 3.6 (as well as 2.7). We
    recommend switching to Python 3, as it has been shown to give
    performance improvements.

    For users who have installed Chat server into a virtualenv, we recommend
    doing this by creating a new virtualenv. For example:

        virtualenv -p python3 ~/chat/env3
        source ~/chat/env3/bin/activate
        pip install chat-server

    You can then start chat as normal, having activated the new
    virtualenv:
    ...
    We are also making available Debian packages which will run Chat server
    on Python 3. You can switch to these packages with
    `apt-get install chat-server-py3`, however, please read
    [debian/NEWS](https://github.com/matrix-org/chat/blob/release-v0.34.0/debian/NEWS)
    before doing so. The existing `chat-server` packages will
...
3.  This release no longer installs the (unmaintained) Chat Console
...
should only affect you if you (a) deploy your Chat server instance from a
...
performance changes to chat are having an impact to the general
community.

# Upgrading to v0.15.0

If you want to use the new URL previewing API
(`/chat/media/r0/preview_url`) then you have to explicitly enable it
...
helps us to track the growth of the Chat community, and helps us to
make Chat a success, as well as to convince other networks that they
should peer with us.
...
To pull in these dependencies in a virtual env, run:
    ...
    python chat/python_dependencies.py | xargs -n 1 pip install
...
chat configuration, and [<signing_key>]{.title-ref} is the location
of the signing key as specified in the chat configuration.
...
format has changed. Run:
    ...
    python -m chat.app.homeserver --config-path <CONFIG> --generate-config
...
SSL certificate to use. Once done you can run the home server using:
    ...
    $ python chat/app/homeserver.py --config-path homeserver.config
