...
works out how to map attributes of a SSO response to Chat-specific
user attributes. Details such as user ID localpart, displayname, and even avatar
URLs are all things that can be mapped from talking to a SSO service.

As an example, a SSO service may return the email address
"john.smith@example.com" for a user, whereas Chat server will need to figure out how
to turn that into a displayname when creating a Chat user for this individual.
It may choose `John Smith`, or `Smith, John [Example.com]` or any number of
variations. As each Chat server configuration may want something different, this is
...
Chat ID based on the SSO attributes, or if the user should be allowed to
choose their own username.

In the first case - where users are automatically allocated a Chat ID - it is
the responsibility of the mapping provider to normalise the SSO attributes and
map them to a valid Chat ID. The [specification for Chat
IDs](https://matrix.org/docs/spec/appendices#user-identifiers) has some
information about what is considered valid.

If the mapping provider does not assign a Chat ID, then Chat server will
automatically serve an HTML page allowing the user to pick their own username.

External mapping providers are provided to Chat server in the form of an external
Python module. You can retrieve this module from [PyPI](https://pypi.org) or elsewhere,
but it must be importable via Chat server (e.g. it must be in the same virtualenv
as Chat server). The Chat server config is then modified to point to the mapping provider
...
user mapping provider built in to Chat server. If using a custom module, you should
...
* `map_user_attributes(self, userinfo, token, failures)`
    ...
    - Returns a dictionary with two keys:
      - `localpart`: A string, used to generate the Chat ID. If this is
...
Chat server has a built-in OpenID mapping provider if a custom provider isn't
specified in the config. It is located at
[`chat.handlers.oidc.JinjaOidcMappingProvider`](https://github.com/matrix-org/chat/blob/develop/chat/handlers/oidc.py).
...
* `__init__(self, parsed_config, module_api)`
   - Arguments:
     - `parsed_config` - A configuration object that is the return value of the
       `parse_config` method. You should set any configuration options needed by
       the module here.
     - `module_api` - a `chat.module_api.ModuleApi` object which provides the
...
* `saml_response_to_user_attributes(self, saml_response, failures, client_redirect_url)`
    - Arguments:
      ...
      - `client_redirect_url` - A string, the URL that the client will be
                                redirected to.
    - This method must return a dictionary, which will then be used by Chat server
       ...
       * `emails` - A list of emails for the new user. If not provided, will
       ...
       Alternatively it can raise a `chat.api.errors.RedirectException` to
...
Chat server has a built-in SAML mapping provider if a custom provider isn't
specified in the config. It is located at
[`chat.handlers.saml.DefaultSamlMappingProvider`](https://github.com/matrix-org/chat/blob/develop/chat/handlers/saml.py).
