...
# Handling spam in Chat server

Chat server has support to customize spam checking behavior. It can plug into a
...
able to be imported by the running Chat server.
...
It then implements methods which return a boolean to alter behavior in Chat server.
...
are documented in the `chat.events.spamcheck.SpamChecker` class.
...
```python
from chat.spam_checker_api import RegistrationBehaviour
...
example using the Chat server spam checking API, including a bot for dynamic
configuration.
