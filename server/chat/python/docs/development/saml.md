...
resource for being able to tinker with the SAML options within Chat server without needing to
deploy and configure a complicated software stack.

To make Chat server (and therefore Chat client) use it:

1. Use the samling.html URL above or deploy your own and visit the IdP Metadata tab.
2. Copy the XML to your clipboard.
3. On your Chat server server, create a new file `samling.xml` next to your `homeserver.yaml` with
...
7. Restart Chat server.

Then in Chat client:

1. Visit the login page with a Chat client pointing at your homeserver.
