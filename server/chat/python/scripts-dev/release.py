...
def prepare():
    ...
    try:
        ...
        raise click.ClickException("Not in Chat repo.")
    ...
    # Get the current version and AST from root Chat module.
    ...
    with open("chat/__init__.py", "w") as f:
    ...
    subprocess.run(
        f'dch -M -v {debian_version} "New chat release {debian_version}."',
        shell=True,
    )
    ...
    click.launch(
        f"https://github.com/matrix-org/chat/blob/{repo.active_branch.name}/CHANGES.md"
    )
...
def tag(gh_token: Optional[str]):
    ...
    if not gh_token:
        click.launch(f"https://github.com/matrix-org/chat/releases/edit/{tag_name}")
        return

    # Create a new draft release
    gh = Github(gh_token)
    gh_repo = gh.get_repo("matrix-org/chat")
    ...
    click.launch(
        f"https://github.com/matrix-org/chat/actions?query=branch%3A{tag_name}"
    )
...
def upload():
    ...
    pypi_asset_names = [
        ...
        f"chat-server-{current_version}.tar.gz",
    ]
    ...
    with TemporaryDirectory(prefix=f"synapse_upload_{tag_name}_") as tmpdir:
        for name in pypi_asset_names:
            ...
            url = f"https://github.com/matrix-org/chat/releases/download/{tag_name}/{name}"
...
def parse_version_from_module() -> Tuple[
    ...
    # later.
    with open("chat/__init__.py") as f:
    ...
    if not version_node:
        print("Failed to find '__version__' definition in chat/__init__.py")
        sys.exit(1)
