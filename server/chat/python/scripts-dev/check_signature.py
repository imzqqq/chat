...
def get_targets(server_name):
    ...
    try:
        ...
        # answers = dns.resolver.query("chat._tcp." + server_name, "SRV")
        answers = dns.resolver.query("chat._tcp." + server_name, "SRV")
        ###
        for srv in answers:
            yield (srv.target, srv.port)
    except dns.resolver.NXDOMAIN:
        yield (server_name, 8880)


def get_server_keys(server_name, target, port):
    ### MARK
    url = "https://%s:%i/chat/key/v1" % (target, port)
    ###
...
def main():
    ...
    for target, port in get_targets(server_name):
        try:
            ...
            ### MARK
            print("Using keys from https://%s:%s/chat/key/v1" % (target, port))
            ###
