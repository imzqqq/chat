#!/usr/bin/env bash
#
# This script generates SQL files for creating a brand new Chat server DB with the latest
# schema, on both SQLite3 and Postgres.
#
# It does so by having Chat server generate an up-to-date SQLite DB, then running
...
### MARK
REQUIRED_DEPS=("chat-server" "psycopg2")
###
...
# cd to root of the chat directory
cd "$(dirname "$0")/.."

# Create temporary SQLite and Postgres homeserver db configs and key file
TMPDIR=$(mktemp -d)
KEY_FILE=$TMPDIR/test.signing.key # default Chat signing key path
...
python -m chat.app.homeserver --generate-keys -c "$SQLITE_CONFIG"
