...
# against Complement.
#
# It makes a Chat server image which represents the current checkout,
# builds a chat-complement image on top, then runs tests with it.
#
...
#
# By default Chat server is run in monolith mode. This can be overridden by
...
# Build the base Chat image from the local checkout
docker build -t matrixdotorg/chat -f "docker/Dockerfile" .

# If we're using workers, modify the docker files slightly.
if [[ -n "$WORKERS" ]]; then
  # Build the workers docker image (from the base Chat image).
  docker build -t matrixdotorg/chat-workers -f "docker/Dockerfile-workers" .

  export COMPLEMENT_BASE_IMAGE=complement-chat-workers
  COMPLEMENT_DOCKERFILE=SynapseWorkers.Dockerfile
  # And provide some more configuration to complement.
  export COMPLEMENT_CA=true
  export COMPLEMENT_VERSION_CHECK_ITERATIONS=500
else
  export COMPLEMENT_BASE_IMAGE=complement-chat
  COMPLEMENT_DOCKERFILE=Chat.Dockerfile
fi

# Build the Complement image from the Chat image we just built.
