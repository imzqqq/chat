...
# for port in 16680 16681 16682; do
    echo "Starting server on port $port... "

    https_port=$((port + 800))
    mkdir -p ./cl/demo/$port
    pushd ./cl/demo/$port
    ...
    if ! grep -F "Customisation made by cl/start.sh" -q  $DIR/etc/$port.config; then
        printf '\n\n# Customisation made by cl/start.sh\n' >> $DIR/etc/$port.config
