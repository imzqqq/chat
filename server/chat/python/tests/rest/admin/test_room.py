...
from chat.api.constants import EventTypes, Membership
from chat.api.errors import Codes
from chat.rest.client import directory, events, login, room
...
@parameterized_class(
    ("method", "url_template"),
    [
        ("POST", "/_chat/admin/v1/rooms/%s/delete"),
        ("DELETE", "/_chat/admin/v1/rooms/%s"),
    ],
)
...
class RoomTestCase(unittest.HomeserverTestCase):
    ...
    def test_list_rooms_pagination(self):
        ...
        while should_repeat:
            ...
            url = "/_chat/admin/v1/rooms?from=%d&limit=%d&order_by=%s" % (
                start,
                limit,
                "name",
            )
        ...
        url = "/_chat/admin/v1/rooms?from=%d&limit=%d" % (start, limit)
        channel = self.make_request(
            "GET",
            url.encode("ascii"),
            access_token=self.admin_user_tok,
    ...
    def test_correct_room_attributes(self):
        ...
        url = "/chat/client/r0/directory/room/%s" % (urllib.parse.quote(test_alias),)
        channel = self.make_request(
            "PUT",
            url.encode("ascii"),
            {"room_id": room_id},
    ...
    def test_room_list_sort_order(self):
        ...
        def _set_canonical_alias(room_id: str, test_alias: str, admin_user_tok: str):
            # Create a new alias to this room
            url = "/chat/client/r0/directory/room/%s" % (
        ...
        def _order_test(
            ...
            url = "/_chat/admin/v1/rooms?order_by=%s" % (order_type,)
            if reverse:
                url += "&dir=b"
            channel = self.make_request(
                "GET",
    ...
    def test_search_term(self):
        ...
        def _search_test(
            ...
            """
            url = "/_chat/admin/v1/rooms?search_term=%s" % (search_term,)
            channel = self.make_request(
                "GET",
                url.encode("ascii"),
                access_token=self.admin_user_tok,
    ...
    def test_single_room(self):
        ...
        url = "/_chat/admin/v1/rooms/%s" % (room_id_1,)
        channel = self.make_request(
            "GET",
            url.encode("ascii"),
            access_token=self.admin_user_tok,
    ...
    def test_room_members(self):
        ...
        url = "/_chat/admin/v1/rooms/%s/members" % (room_id_1,)
        channel = self.make_request(
            "GET",
            url.encode("ascii"),
            access_token=self.admin_user_tok,
        ...
        url = "/_chat/admin/v1/rooms/%s/members" % (room_id_2,)
        channel = self.make_request(
            "GET",
            url.encode("ascii"),
            access_token=self.admin_user_tok,
    ...
    def test_room_state(self):
        ...
        url = "/_chat/admin/v1/rooms/%s/state" % (room_id,)
        channel = self.make_request(
            "GET",
            url.encode("ascii"),
            access_token=self.admin_user_tok,
...
class JoinAliasRoomTestCase(unittest.HomeserverTestCase):
    ...
    def prepare(self, reactor, clock, homeserver):
        ...
        self.url = f"/_chat/admin/v1/join/{self.public_room_id}"
    ...
    def test_room_does_not_exist(self):
        ...
        """
        body = json.dumps({"user_id": self.second_user_id})
        url = "/_chat/admin/v1/join/!unknown:test"
    ...
    def test_room_is_not_valid(self):
        ...
        """
        body = json.dumps({"user_id": self.second_user_id})
        url = "/_chat/admin/v1/join/invalidroom"
    ...
    def test_join_private_room_if_not_member(self):
        ...
        url = f"/_chat/admin/v1/join/{private_room_id}"
        body = json.dumps({"user_id": self.second_user_id})
    ...
    def test_context_as_non_admin(self):
        ...
        for tok in [user_tok, user_tok_2]:
            channel = self.make_request(
                "GET",
                "/_chat/admin/v1/rooms/%s/context/%s"
...
class MakeRoomAdminTestCase(unittest.HomeserverTestCase):
    ...
    def prepare(self, reactor, clock, homeserver):
        ...
        self.url = "/_chat/admin/v1/rooms/{}/make_room_admin".format(
            self.public_room_id
        )
    ...
    def test_public_room(self):
        ...
        channel = self.make_request(
            "POST",
            f"/_chat/admin/v1/rooms/{room_id}/make_room_admin",
            content={},
            access_token=self.admin_user_tok,
        )
