...
from chat.crypto.keyring import PerspectivesKeyFetcher
...
class BaseRemoteKeyResourceTestCase(unittest.HomeserverTestCase):
    ...
    def create_test_resource(self):
        return create_resource_tree(
            {"/chat/key/v2": KeyApiV2Resource(self.hs)}, root_resource=NoResource()
        )
    ...
    def expect_outgoing_key_request(
        ...
        async def get_json(destination, path, ignore_backoff=False, **kwargs):
            ...
            self.assertEqual(
                path, "/chat/key/v2/server/%s" % (urllib.parse.quote(key_id),)
            )
