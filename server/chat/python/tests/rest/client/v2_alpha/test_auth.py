...
from chat.rest.client import account, auth, devices, login, register
...
class UIAuthTests(unittest.HomeserverTestCase):
    ...
    def default_config(self):
        ...
        # False, so chat will see the requested uri as http://..., so using http in
        # the public_baseurl stops Chat trying to redirect to https.
        config["public_baseurl"] = "http://chat.test"
    ...
    def test_grandfathered_identifier(self):
        ...
        Chat used to require clients to submit a "user" field for m.login.password
    ...
    def test_can_change_body(self):
        ...
        When Chat is updated to be spec compliant, the call to re-use the
    ...
    def test_ui_auth_via_sso(self):
        ...
        This includes:
          ...
          * calling back to the chat oidc callback
...
class RefreshAuthTests(unittest.HomeserverTestCase):
    ...
    def test_login_issue_refresh_token(self):
        ...
        login_without_refresh = self.make_request(
            "POST", "/chat/client/r0/login", body
        )
        ...
        login_with_refresh = self.make_request(
            "POST",
            "/chat/client/r0/login?org.matrix.msc2918.refresh_token=true",
            body,
        )
    ...
    def test_register_issue_refresh_token(self):
        ...
        register_with_refresh = self.make_request(
            "POST",
            "/chat/client/r0/register?org.matrix.msc2918.refresh_token=true",
            {
                "username": "test3",
                "password": self.user_pass,
                "auth": {"type": LoginType.DUMMY},
    ...
    def test_token_refresh(self):
        ...
        refresh_response = self.make_request(
            "POST",
            "/chat/client/unstable/org.matrix.msc2918.refresh_token/refresh",
    ...
    def test_refresh_token_invalidation(self):
        ...
        for access_token in [
            ...
            whoami_response = self.make_request(
                "GET", "/chat/client/r0/account/whoami", access_token=access_token
            )
