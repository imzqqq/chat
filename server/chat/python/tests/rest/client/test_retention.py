...
class RetentionTestCase(unittest.HomeserverTestCase):
    ...
    def test_visibility(self):
        """Tests that chat.visibility.filter_events_for_client correctly filters out
        outdated events
        """
    ...
    def _test_retention_event_purged(self, room_id: str, increment: float):
        ...
        The main reason for sending a second event is because currently Chat won't
...
class RetentionNoDefaultPolicyTestCase(unittest.HomeserverTestCase):
    ...
    def get_event(self, room_id, event_id, expected_code=200):
        url = "/chat/client/r0/rooms/%s/event/%s" % (room_id, event_id)
