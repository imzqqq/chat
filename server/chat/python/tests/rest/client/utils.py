...
class RestHelper:
    ...
    def create_room_as(
        ...
        Args:
            ...
            is_public: If True, the `visibility` parameter will be set to the
                ...
                to "private".
            room_version: The room version to create the room as. Defaults to Chat's
        ...
        path = "/chat/client/r0/createRoom"
    ...
    def change_membership(
        ...
        path = "/chat/client/r0/rooms/%s/state/m.room.member/%s" % (room, targ)
        if tok:
            path = path + "?access_token=%s" % tok
    ...
    def send_event(
        ...
        path = "/chat/client/r0/rooms/%s/send/%s/%s" % (room_id, type, txn_id)
        if tok:
            path = path + "?access_token=%s" % tok
    ...
    def _read_write_state(
        ...
        """
        path = "/chat/client/r0/rooms/%s/state/%s/%s" % (
            room_id,
            event_type,
            state_key,
        )
    ...
    def upload_media(
        ...
        path = "/chat/media/r0/upload?filename=%s" % (filename,)
        channel = make_request(
            self.hs.get_reactor(),
            FakeSite(resource),
            "POST",
    ...
    def auth_via_oidc(
        ...
        Starts by making a request to the relevant chat redirect endpoint, which is
        ...
        # back to chat's OIDC callback resource. However, we do need the "state"
        # param that chat passes to the IdP via query params, as well as the cookie
        # that chat passes to the client.
    ...
    def complete_oidc_auth(
        ...
        Chat's OIDC callback endpoint, intercepting the HTTP requests that will get
        sent back to the OIDC provider.

        Requires the OIDC callback resource to be mounted at the normal place.

        Args:
            oauth_uri: the OIDC URI returned by chat's redirect endpoint (ie,
               from initiate_sso_login or initiate_sso_ui_auth).
            cookies: the cookies set by chat's redirect endpoint, which will be
    ...
    def initiate_sso_login(
        ...
        channel = make_request(
            ...
            self.site,
            "GET",
            "/chat/client/r0/login/sso/redirect?" + urllib.parse.urlencode(params),
        )
    ...
    def initiate_sso_ui_auth(
        ...
        sso_redirect_endpoint = (
            "/chat/client/r0/auth/m.login.sso/fallback/web?"
