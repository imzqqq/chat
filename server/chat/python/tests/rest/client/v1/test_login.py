...
from chat.rest.client import devices, login, logout, register
from chat.rest.client.account import WhoamiRestServlet
from chat.rest.chat.client import build_synapse_client_resource_tree
from chat.types import create_requester
...
# chat server name: used to populate public_baseurl in some tests
SYNAPSE_SERVER_PUBLIC_HOSTNAME = "chat"

# public_baseurl for some tests. It uses an http:// scheme because
# FakeChannel.isSecure() returns False, so chat will see the requested uri as
# http://..., so using http in the public_baseurl stops Chat trying to redirect to
# https://....
...
LOGIN_URL = b"/chat/client/r0/login"
TEST_URL = b"/chat/client/r0/account/whoami"
...
class LoginRestServletTestCase(unittest.HomeserverTestCase):
    ...
    def _delete_device(self, access_token, user_id, password, device_id):
        ...
        auth = {
            "type": "m.login.password",
            # https://github.com/matrix-org/chat/issues/5665
...
class MultiSSOTestCase(unittest.HomeserverTestCase):
    ...
    def default_config(self) -> Dict[str, Any]:
        ...
        config["cas_config"] = {
            ...
            "service_url": "https://matrix.goodserver.com:8880",
        }
    ...
    def test_get_login_flows(self):
        ...
        channel = self.make_request("GET", "/chat/client/r0/login")
    ...
    def test_multi_sso_redirect_to_cas(self):
        ...
        channel = self.make_request(
            "GET",
            "/_chat/client/pick_idp?redirectUrl="
    ...
    def test_multi_sso_redirect_to_unknown(self):
        ...
        channel = self.make_request(
            "GET",
            "/_chat/client/pick_idp?redirectUrl=http://x&idp=xyz",
        )
    ...
    def _make_sso_redirect_request(
        ...
        """Send a request to /chat/client/r0/login/sso/redirect
        ...
        """
        endpoint = (
            "/chat/client/unstable/org.matrix.msc2858/login/sso/redirect"
            if unstable_endpoint
            else "/chat/client/r0/login/sso/redirect"
        )
...
class CASTestCase(unittest.HomeserverTestCase):
    ...
    def make_homeserver(self, reactor, clock):
        ...
        self.redirect_path = "_chat/client/login/sso/redirect/confirm"

        config = self.default_config()
        config["public_baseurl"] = (
            config.get("public_baseurl") or "https://matrix.goodserver.com:8880"
        )
        config["cas_config"] = {
            "enabled": True,
            "server_url": CAS_SERVER,
        ...
        async def get_raw(uri, args):
            ...
            mock's return value) because the corresponding Chat code awaits on it.
            """
            return (
                """
    ...
    def test_cas_redirect_confirm(self):
        ...
        """
        base_url = "/chat/client/r0/login/cas/ticket?redirectUrl"
        ...
        # Get Chat to call the fake CAS and serve the template.
    ...
    def test_cas_redirect_login_fallback(self):
        self._test_redirect("https://example.com/chat/static/client/login")

    def _test_redirect(self, redirect_url):
        """Tests that the SSO login flow serves a redirect for the given redirect URL."""
        cas_ticket_url = (
            "/chat/client/r0/login/cas/ticket?redirectUrl=%s&ticket=ticket"
...
# RSS256, with a public key configured in chat as "jwt_secret", and tokens
...
class JWTPubKeyTestCase(unittest.HomeserverTestCase):
    ...
    # This key's pubkey is used as the jwt_secret setting of chat. Valid
    ...
    # This key is used to sign tokens that shouldn't be accepted by chat.
...
class AppserviceLoginRestServletTestCase(unittest.HomeserverTestCase):
    ...
    def register_as_user(self, username):
        self.make_request(
            b"POST",
            "/chat/client/r0/register?access_token=%s" % (self.service.token,),
            {"username": username},
        )
...
class UsernamePickerTestCase(HomeserverTestCase):
    ...
    def test_username_picker(self):
        ...
        self.assertEqual(picker_url, "/_chat/client/pick_username/account_details")
