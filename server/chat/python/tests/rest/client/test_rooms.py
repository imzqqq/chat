...
from chat.api.constants import EventContentFields, EventTypes, Membership
from chat.api.errors import HttpResponseException
from chat.handlers.pagination import PurgeStatus
from chat.rest import admin
from chat.rest.client import account, directory, login, profile, room, sync
from chat.types import JsonDict, RoomAlias, UserID, create_requester
from chat.util.stringutils import random_string

from tests import unittest
from tests.test_utils import make_awaitable

PATH_PREFIX = b"/chat/client/api/v1"
...
class RoomsCreateTestCase(RoomBase):
    ...
    def test_post_room_invitees_invalid_mxid(self):
        # POST with invalid invitee, see https://github.com/matrix-org/chat/issues/4088
...
class RoomJoinRatelimitTestCase(RoomBase):
    ...
    def test_join_local_ratelimit_profile_change(self):
        ...
        path = "/chat/client/r0/profile/%s/displayname" % self.user_id
        ...
        for room_id in room_ids:
            path = "/chat/client/r0/rooms/%s/state/m.room.member/%s" % (
                room_id,
                self.user_id,
            )
    ...
    def test_join_local_ratelimit_idempotent(self):
        ...
        paths_to_test = [
            "/chat/client/r0/rooms/%s/join",
            "/chat/client/r0/join/%s",
        ]
...
class PublicRoomsRestrictedTestCase(unittest.HomeserverTestCase):
    ...
    def make_homeserver(self, reactor, clock):
        ...
        self.url = b"/chat/client/r0/publicRooms"
...
class PublicRoomsTestRemoteSearchFallbackTestCase(unittest.HomeserverTestCase):
    ...
    def test_simple(self):
        ...
        channel = self.make_request(
            "POST",
            b"/chat/client/r0/publicRooms?server=testserv",
...
class PerRoomProfilesForbiddenTestCase(unittest.HomeserverTestCase):
    ...
    def prepare(self, reactor, clock, homeserver):
        ...
        channel = self.make_request(
            "PUT",
            "/chat/client/r0/profile/%s/displayname" % (self.user_id,),
            request_data,
    ...
    def test_per_room_profile_forbidden(self):
        ...
        channel = self.make_request(
            "PUT",
            "/chat/client/r0/rooms/%s/state/m.room.member/%s"
        ...
        channel = self.make_request(
            "GET",
            "/chat/client/r0/rooms/%s/event/%s" % (self.room_id, event_id),
            access_token=self.tok,
        )
        self.assertEqual(channel.code, 200, channel.result)
...
class RoomMembershipReasonTestCase(unittest.HomeserverTestCase):
    ...
    def test_join_reason(self):
        ...
        channel = self.make_request(
            "POST",
            f"/chat/client/r0/rooms/{self.room_id}/join",
    ...
    def test_leave_reason(self):
        ...
        channel = self.make_request(
            "POST",
            f"/chat/client/r0/rooms/{self.room_id}/leave",
            content={"reason": reason},
            access_token=self.second_tok,
        )
        self.assertEqual(channel.code, 200, channel.result)
    ...
    def test_kick_reason(self):
        ...
        channel = self.make_request(
            "POST",
            f"/chat/client/r0/rooms/{self.room_id}/kick",
    ...
    def test_ban_reason(self):
        ...
        channel = self.make_request(
            "POST",
            f"/chat/client/r0/rooms/{self.room_id}/ban",
            content={"reason": reason, "user_id": self.second_user_id},
    ...
    def test_unban_reason(self):
        ...
        channel = self.make_request(
            "POST",
            f"/chat/client/r0/rooms/{self.room_id}/unban",
            content={"reason": reason, "user_id": self.second_user_id},
            access_token=self.creator_tok,
        )
        self.assertEqual(channel.code, 200, channel.result)
    ...
    def test_invite_reason(self):
        ...
        channel = self.make_request(
            "POST",
            f"/chat/client/r0/rooms/{self.room_id}/invite",
            content={"reason": reason, "user_id": self.second_user_id},
            access_token=self.creator_tok,
        )
        self.assertEqual(channel.code, 200, channel.result)
    ...
    def _check_for_reason(self, reason):
        channel = self.make_request(
            "GET",
            "/chat/client/r0/rooms/{}/state/m.room.member/{}".format(
...
class RoomAliasListTestCase(unittest.HomeserverTestCase):
    ...
    def _get_aliases(self, access_token: str, expected_code: int = 200) -> JsonDict:
        ...
        channel = self.make_request(
            "GET",
            "/chat/client/r0/rooms/%s/aliases" % (self.room_id,),
    ...
    def _set_alias_via_directory(self, alias: str, expected_code: int = 200):
        url = "/chat/client/r0/directory/room/" + alias
