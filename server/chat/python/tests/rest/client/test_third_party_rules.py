...
from chat.rest.client import login, room
from chat.types import Requester, StateMap
from chat.util.frozenutils import unfreeze
...
class ThirdPartyRulesTestCase(unittest.HomeserverTestCase):
    ...
    def test_third_party_rules(self):
        ...
        channel = self.make_request(
            "PUT",
            "/chat/client/r0/rooms/%s/send/foo.bar.allowed/1" % self.room_id,
            {},
        ...
        channel = self.make_request(
            "PUT",
            "/chat/client/r0/rooms/%s/send/foo.bar.forbidden/2" % self.room_id,
            {},
            access_token=self.tok,
        )
    ...
    def test_cannot_modify_event(self):
        ...
        channel = self.make_request(
            "PUT",
            "/chat/client/r0/rooms/%s/send/modifyme/1" % self.room_id,
            {"x": "x"},
            access_token=self.tok,
        )
    ...
    def test_message_edit(self):
        ...
        channel = self.make_request(
            "PUT",
            "/chat/client/r0/rooms/%s/send/m.room.message/2" % self.room_id,
            {
        ...
        channel = self.make_request(
            "GET",
            "/chat/client/r0/rooms/%s/event/%s" % (self.room_id, orig_event_id),
            access_token=self.tok,
        )
        self.assertEqual(channel.result["code"], b"200", channel.result)
        ev = channel.json_body
        ...
        channel = self.make_request(
            "GET",
            "/chat/client/r0/rooms/%s/event/%s" % (self.room_id, edited_event_id),
            access_token=self.tok,
        )
        self.assertEqual(channel.result["code"], b"200", channel.result)
        ev = channel.json_body
    ...
    def test_legacy_check_event_allowed(self):
        ...
        channel = self.make_request(
            "PUT",
            "/chat/client/r0/rooms/%s/send/m.room.message/1" % self.room_id,
            {
                "msgtype": "m.text",
                "body": "Original body",
            },
