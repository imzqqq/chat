...
from chat.rest.client import login, room, sync
...
class ConsentNoticesTests(unittest.HomeserverTestCase):
    ...
    def test_get_sync_message(self):
        ...
        channel = self.make_request(
            "GET", "/chat/client/r0/sync", access_token=self.access_token
        )
        ...
        channel = self.make_request(
            "POST",
            "/chat/client/r0/rooms/" + room_id + "/join",
        ...
        self.assertTrue(
            messages[0]["content"]["body"].startswith(
                "consent https://example.com/chat/consent"
            )
        )
