...
from chat.http.federation.matrix_federation_agent import MatrixFederationAgent
from chat.http.federation.srv_resolver import Server
from chat.http.federation.well_known_resolver import (
    WELL_KNOWN_MAX_SIZE,
    WellKnownResolver,
    _cache_period_from_headers,
)
from chat.logging.context import SENTINEL_CONTEXT, LoggingContext, current_context
from chat.util.caches.ttlcache import TTLCache
...
class MatrixFederationAgentTests(unittest.TestCase):
    ...
    def _do_get(self):
        ...
        test_d = self._make_get_request(b"matrix://testserv:8880/foo/bar")
        ...
        self.assertEqual(
            request.requestHeaders.getRawHeaders(b"host"), [b"testserv:8880"]
        )
        self.assertEqual(
            request.requestHeaders.getRawHeaders(b"user-agent"), [b"test-agent"]
        )
    ...
    def _do_get_via_proxy(
        ...
        self.assertEqual(request.path, b"testserv:8880")
    ...
    def test_idna_servername(self):
        ...
        self.mock_resolver.resolve_service.assert_called_once_with(
            b"chat._tcp.xn--bcher-kva.com"
        )

        # We should fall back to port 8880
        clients = self.reactor.tcpClients
        self.assertEqual(len(clients), 2)
        (host, port, client_factory, _timeout, _bindAddress) = clients[1]
        self.assertEqual(host, "1.2.3.4")
