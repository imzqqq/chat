...
from chat.api.errors import Codes, RedirectException, SynapseError
from chat.config.server import parse_listener_def
from chat.http.server import DirectServeHtmlResource, JsonResource, OptionsResource
...
class JsonResourceTests(unittest.TestCase):
    ...
    def test_handler_for_request(self):
        ...
        res.register_paths(
            "GET",
            [re.compile("^/chat/foo/(?P<room_id>[^/]*)$")],
            _callback,
            "test_servlet",
        )

        make_request(
            self.reactor, FakeSite(res), b"GET", b"/chat/foo/%E2%98%83?a=%E2%98%83"
        )
    ...
    def test_callback_direct_exception(self):
        ...
        res = JsonResource(self.homeserver)
        res.register_paths(
            "GET", [re.compile("^/chat/foo$")], _callback, "test_servlet"
        )

        channel = make_request(self.reactor, FakeSite(res), b"GET", b"/chat/foo")
    ...
    def test_no_handler(self):
        """
        If there is no handler to process the request, Chat will return 400.
        """
        ...
        channel = make_request(self.reactor, FakeSite(res), b"GET", b"/chat/foobar")
    ...
    def test_head_request(self):
        ...
        channel = make_request(self.reactor, FakeSite(res), b"HEAD", b"/chat/foo")
