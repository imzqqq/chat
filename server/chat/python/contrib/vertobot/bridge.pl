            ...
            if ($event->{type} eq 'm.room.membership') {
                ...
                if ($membership eq 'invite') {
                    ...
                    my ( $response ) = $http->do_request(
                        ...
                        uri => URI->new(
                            $CONFIG{"matrix"}->{server}.
                                "/chat/client/api/v1/rooms/$room_id/join?".
...
Future->needs_all(
    $http->do_request(
            method => "POST",
            uri => URI->new( $CONFIG{"matrix"}->{server}."/chat/appservice/v1/register" ),
            content_type => "application/json",
            content => <<EOT
{
    "as_token": "$as_token",
            ...
            if (($json->{method} eq 'verto.answer' && $json->{params}->{sdp}) ||
                ...
                if ($json->{params}->{sdp}) {
                    $http->do_request(
                        ...
                        uri => URI->new(
                            $CONFIG{"matrix"}->{server}.
                                "/chat/client/api/v1/send/m.call.answer?".
                ...
                my ($response) = $http->do_request(
                    ...
                    uri => URI->new(
                        $CONFIG{"matrix"}->{server}.
                            "/chat/client/api/v1/join/$room_id?".
                            "access_token=$as_token&user_id=$caller_user"
                    ),
                    content_type => "application/json",
                    content => '{}',
                ...
                $http->do_request(
                    ...
                    uri => URI->new(
                        $CONFIG{"matrix"}->{server}.
                            "/chat/client/api/v1/send/m.call.invite?".
                            "access_token=$as_token&user_id=$caller_user"
                    ),
                    content_type => "application/json",
                    content => JSON->new->encode({
                ...
                $http->do_request(
                    ...
                    uri => URI->new(
                        $CONFIG{"matrix"}->{server}.
                            "/chat/client/api/v1/send/m.call.hangup?".
                            "access_token=$as_token&user_id=$caller_user"
                    ),
                    content_type => "application/json",
                    content => JSON->new->encode({
