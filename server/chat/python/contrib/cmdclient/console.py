...
""" Starts a Chat server client console. """
import argparse
import cmd
import getpass
import json
...
class SynapseCmd(cmd.Cmd):
    ...
    """Basic chat command-line processor.
    ...
    def __init__(self, http_client, server_url, identity_server_url, username, token):
        ...
        self.path_prefix = "/chat/client/api/v1"
    ...
    def _do_emailrequest(self, args):
        ...
        url = (
            ...
            + "/chat/identity/api/v1/validate/email/requestToken"
        )
    ...
    def _do_emailvalidate(self, args):
        ...
        url = (
            self._identityServerUrl()
            + "/chat/identity/api/v1/validate/email/submitToken"
        )
    ...
    def _do_3pidbind(self, args):
        # TODO: Update to use v2 Identity Service API endpoint
        url = self._identityServerUrl() + "/chat/identity/api/v1/3pid/bind"
    ...
    def _do_invite(self, roomid, userstring):
        if not userstring.startswith("@") and self._is_on("complete_usernames"):
            # TODO: Update to use v2 Identity Service API endpoint
            url = self._identityServerUrl() + "/chat/identity/api/v1/lookup"
            ...
            if "mxid" in json_res and "signatures" in json_res:
                ...
                url = (
                    self._identityServerUrl()
                    + "/chat/identity/api/v1/pubkey/ed25519"
                )
...
def main(server_url, identity_server_url, username, token, config_path):
    print("Chat command line client")
    print("===========================")
    ...
    # load chat.json config from a previous session
    global CONFIG_JSON
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("Starts a chat client.")
    parser.add_argument(
        "-s",
        "--server",
        dest="server",
