...
from chat.config.emailconfig import EmailSubjectConfig
from chat.events import EventBase
from chat.push.presentable_names import (
    calculate_room_name,
    descriptor_from_member_events,
    name_from_member_event,
)
from chat.storage.state import StateFilter
from chat.types import StateMap, UserID
...
# From matrix-react-sdk/blob/master/src/HtmlUtils.js
ALLOWED_TAGS = [
...
class Mailer:
    ...
    async def send_password_reset_mail(
        ...
        link = (
            ...
            + "_chat/client/password_reset/email/submit_token?%s"
    ...
    async def send_registration_mail(
        ...
        link = (
            self.hs.config.public_baseurl
            + "chat/client/unstable/registration/email/submit_token?%s"
            % urllib.parse.urlencode(params)
        )
    ...
    async def send_add_threepid_mail(
        ...
        link = (
            self.hs.config.public_baseurl
            + "chat/client/unstable/add_threepid/email/submit_token?%s"
            % urllib.parse.urlencode(params)
        )
    ...
    def _add_text_message_vars(
        ...
        """
        Potentially add a sanitized message body to the message variables.
    ...
    def _make_unsubscribe_link(
        ...
        return "%schat/client/unstable/pushers/remove?%s" % (
            self.hs.config.public_baseurl,
            urllib.parse.urlencode(params),
        )
        ###
