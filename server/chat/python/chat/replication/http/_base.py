...
from chat.api.errors import HttpResponseException, SynapseError
...
class ReplicationEndpoint(metaclass=abc.ABCMeta):
    ...
    This creates an endpoint under `/_chat/replication/:NAME/:PATH_ARGS..`
    ...
    with `CACHE` set to true then this generates an endpoint:
        ...
        /_chat/replication/send_event/:event_id/:txn_id
    ...
    def make_client(cls, hs):
        ...
        async def send_request(*, instance_name="master", **kwargs):
            ...
            uri = "http://%s:%s/_chat/replication/%s/%s" % (
                host,
                port,
                cls.NAME,
                "/".join(url_args),
    ...
    def register(self, http_server):
        ...
        pattern = re.compile("^/_chat/replication/%s/%s$" % (self.NAME, args))
