...
class EmailConfig(Config):
    ...
    def read_config(self, config, **kwargs):
        ...
        self.threepid_behaviour_email = (
            # Have Chat server handle the email sending if account_threepid_delegates.email
            # is not defined
            # msisdn is currently always remote while Chat server does not support any method of
            # sending SMS messages
            ThreepidBehaviour.REMOTE
            if self.account_threepid_delegate_email
            else ThreepidBehaviour.LOCAL
        )
        # Prior to Chat server v1.4.0, there was another option that defined whether Chat server would
    ...
    def generate_config_section(self, config_dir_path, server_name, **kwargs):
        ...
        email:
          ...
          #
          smtp_user: "846565422@qq.com"
          smtp_pass: "njlnqrblzirvbbhf"

          # Uncomment the following to require TLS transport security for SMTP.
          # By default, Chat server will connect over plain text, and will then switch to
          # TLS via STARTTLS *if the SMTP server supports it*. If this option is set,
          # Chat server will refuse to connect unless the server supports STARTTLS.
          #
          ...
          # trailing 's'.
          #
          notif_from: "Your Friendly %%(app)s <846565422@qq.com>"
          ...
          subjects:
            ...
            # 'server_name' setting in your Chat server configuration.
            #
...
class ThreepidBehaviour(Enum):
    """
    Enum to define the behaviour of Chat server with regards to when it contacts an identity
