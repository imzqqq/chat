...
import chat
from chat.logging._structured import setup_structured_logging
from chat.logging.context import LoggingContextFilter
from chat.logging.filter import MetadataFilter
...
DEFAULT_LOG_CONFIG = Template(
    """\
# Log configuration for Chat server.
#
# This is a YAML file containing a standard Python logging configuration
# dictionary. See [1] for details on the valid settings.
#
# Chat server also supports structured logging for machine readable logs which can
# be ingested by ELK stacks. See [2] for details.
#
# [1]: https://docs.python.org/3.7/library/logging.config.html#configuration-dictionary-schema
# [2]: https://chat.docs.dingshunyu.top/structured_logging.html
...
handlers:
    ...
    buffer:
        class: chat.logging.handlers.PeriodicallyFlushingMemoryHandler
        target: file
...
loggers:
    chat.storage.SQL:
        # beware: increasing this to DEBUG will make Chat server log sensitive
        # information such as access tokens.
        level: INFO

    twisted:
        # We send the twisted logging directly to the file handler,
        # to work around https://github.com/matrix-org/chat/issues/3471
...
removed in Chat server 1.3.0. You should instead set up a separate log configuration file.
"""
...
def _setup_stdlib_logging(config, log_config_path, logBeginner: LogBeginner) -> None:
    ...
    if log_config_path is None:
        ...
        logging.getLogger("chat.storage.SQL").setLevel(logging.INFO)
...
def setup_logging(
    ...
    Args:
        config (LoggingConfig | chat.config.worker.WorkerConfig):
            configuration data
    ...
    from chat.app import _base as appbase

    appbase.register_sighup(_reload_logging_config, log_config_path)

    # Log immediately so we can grep backwards.
    logging.warning("***** STARTING SERVER *****")
    logging.warning("Server %s version %s", sys.argv[0], get_version_string(chat))
