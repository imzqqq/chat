...
Chat server process before they can be run in a separate worker.
...
class WorkerConfig(Config):
    """The workers are processes run separately to the main Chat server process.
    They have their own pid_file and listener configuration. They use the
    replication_url to talk to the main Chat server process."""
    ...
    def read_config(self, config, **kwargs):
        ...
        if self.worker_app == "chat.app.homeserver":
        ...
        # The host used to connect to the main chat
        self.worker_replication_host = config.get("worker_replication_host", None)

        # The port on the main chat for TCP replication
        self.worker_replication_port = config.get("worker_replication_port", None)

        # The port on the main chat for HTTP replication endpoint
        self.worker_replication_http_port = config.get("worker_replication_http_port")

        # The shared secret used for authentication when connecting to the main chat.
        ...
        #      `chat.app.federation_sender` worker app.
        ...
        if federation_sender_instances is None:
            ...
            if self.worker_app == "chat.app.federation_sender":
                if send_federation:
        ...
        if pusher_instances is None:
            ...
            if self.worker_app == "chat.app.pusher":
                if start_pushers:
