...
from chat.util.stringutils import parse_and_validate_server_name
...
class ServerConfig(Config):
    ...
    def read_config(self, config, **kwargs):
        ...
        if bind_port:
            ...
            unsecure_port = config.get("unsecure_port", bind_port - 800)
            if unsecure_port:
                self.listeners.append(
                    ListenerConfig(
                        port=unsecure_port,
    ...
    def generate_config_section(
        ...
        # configure this correctly before you start Chat server. It should be all
        # lowercase and may contain an explicit port.
        # Examples: chat.org, localhost:8080
        #
        ...
        # The absolute URL to the web client which /chat/client will redirect
        # to if 'webclient' is configured under the 'listeners' configuration.
        #
        # This option can be also set to the filesystem path to the web client
        # which will be served at /chat/client/ if 'webclient' is configured
        # under the 'listeners' configuration, however this is a security risk:
        # https://github.com/matrix-org/chat#security-note
        #
        #web_client_location: https://client.example.com/

        # The public-facing base URL that clients use to access this Homeserver (not
        # including chat/...). This is the same URL a user might enter into the
        # 'Custom Homeserver URL' field on their client. If you use Chat server with a
        # reverse proxy, this should be the URL to reach Chat server via the proxy.
        ...
        #
        # This option replaces federation_ip_range_blacklist in Chat server v1.25.0.
        #
...
%(ip_range_blacklist)s
        ...
        # List of ports that Chat server should listen on, their purpose and their
        # configuration.
        #
        ...
        #       X-Forwarded-For header as the client IP. Useful when Chat server is
        ...
        #
        #   client: the client-server API (/chat/client), and the Chat server admin
        ...
        #       'media', 'keys', 'openid'
        #
        #   keys: the key discovery API (/chat/keys).
        #
        ...
        listeners:
          ...
          # will also need to give Chat server a TLS key and certificate: see the TLS section
          # below.)
          #
        ...
        # forward extremities reaches a given threshold, Chat server will send an
        ...
        #
        # If this feature is enabled, Chat server will regularly look for and purge events
        # which are older than the room's maximum retention period. Chat server will also
        ...
        retention:
          ...
          # Default retention policy. If set, Chat server will apply it to rooms that lack the
          # 'm.room.retention' state event. Currently, the value of 'min_lifetime' doesn't
          # matter much because Chat server doesn't take it into account yet.
          #
          #default_policy:
          #  min_lifetime: 1d
          #  max_lifetime: 1y
          ...
          # 'max_lifetime' that's out of these bounds, Chat server will cap the room's policy
          ...
          # Chat server's database (which is done using the range specified in a purge job's
          # configuration).
          #
          #purge_jobs:
        ...
        #
        templates:
          # Directory in which Chat server will try to find template files to use to generate
...
Chat server no longer includes a web client. To enable a web client, configure
