...
from chat.crypto.context_factory import FederationPolicyForHTTPS
from chat.http import proxyagent
from chat.http.client import BlacklistingAgentWrapper, BlacklistingReactorWrapper
from chat.http.connectproxyclient import HTTPConnectProxyEndpoint
from chat.http.federation.srv_resolver import Server, SrvResolver
from chat.http.federation.well_known_resolver import WellKnownResolver
from chat.http.proxyagent import ProxyAgent
from chat.logging.context import make_deferred_yieldable, run_in_background
from chat.types import ISynapseReactor
...
class MatrixHostnameEndpoint:
    ...
    async def _resolve_server(self) -> List[Server]:
        ...
        if port or _is_ip_literal(host):
            return [Server(host, port or 8880)]

        logger.debug("Looking up SRV record for %s", host.decode(errors="replace"))
        ### MARK - FIXME:
        server_list = await self._srv_resolver.resolve_service(b"chat._tcp." + host)
        ###
        ...
        # No SRV records, so we fallback to host and 8880
        logger.debug("No SRV records for %s", host.decode(errors="replace"))
        return [Server(host, 8880)]
