...
import chat
import chat.api.auth
import chat.types
from chat.api.constants import APP_SERVICE_REGISTRATION_TYPE, LoginType
...
from chat.config.captcha import CaptchaConfig
from chat.config.consent import ConsentConfig
from chat.config.emailconfig import ThreepidBehaviour
from chat.config.ratelimiting import FederationRateLimitConfig
from chat.config.registration import RegistrationConfig
from chat.config.server import is_threepid_reserved
from chat.handlers.auth import AuthHandler
from chat.handlers.ui_auth import UIAuthSessionDataConstants
...
from chat.metrics import threepid_send_requests
...
from chat.util.stringutils import assert_valid_client_secret, random_string
from chat.util.threepids import (
    canonicalise_email,
    check_3pid_allowed,
    validate_email,
)
...
class EmailRegisterRequestTokenRestServlet(RestServlet):
    ...
    async def on_POST(self, request):
        ...
        # in chat/rest/client/account.py)
        try:
        ...
        if self.config.threepid_behaviour_email == ThreepidBehaviour.REMOTE:
            ...
            # Send registration emails from Chat
...
class RegisterRestServlet(RestServlet):
    ...
    async def _do_guest_registration(self, params, address=None):
        ...
        device_id = chat.api.auth.GUEST_DEVICE_ID
        initial_display_name = params.get("initial_device_display_name")
        (
            device_id,
            access_token,
