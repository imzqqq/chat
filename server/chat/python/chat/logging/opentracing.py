...
"""
============================
Using OpenTracing in Chat
============================

Python-specific tracing concepts are at https://opentracing.io/guides/python/.
Note that Chat wraps OpenTracing in a small module (this one) in order to make the
...
no span objects from OpenTracing are exposed in Chat's code. This allows
OpenTracing to be easily disabled in Chat and thereby have OpenTracing as
...
In Chat it is not possible to start a non-active span. Spans can be started
...
.. code-block:: python
   ...
   from chat.logging.opentracing import start_active_span
...
.. code-block:: python
   ...
   from chat.logging.opentracing import trace
...
.. code-block:: python
   ...
   from chat.logging.opentracing import tag_args
...
from chat.util import json_decoder, json_encoder
...
try:
    ...
    from chat.logging.scopecontextmanager import LogContextScopeManager
except ImportError:
...
class SynapseBaggage:
    FORCE_TRACING = "chat-force-tracing"
...
def inject_header_dict(
    ...
    Note:
        ...
        chat or twisted. If we're still using jaeger these headers would be those
        here:
...
def inject_response_headers(response_headers: Headers) -> None:
    ...
    if trace_id is not None:
        response_headers.addRawHeader("Chat-Trace-Id", f"{trace_id:x}")
