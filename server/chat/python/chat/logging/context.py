""" Thread-local-alike tracking of log contexts within chat

This module provides objects and utilities for tracking contexts through
chat code, so that log lines can include a request identifier, and so that
...
if TYPE_CHECKING:
    from chat.logging.scopecontextmanager import _LogContextScope
...
class LoggingContext:
    ...
    def current_context(cls) -> LoggingContextOrSentinel:
        ...
        """
        warnings.warn(
            "chat.logging.context.LoggingContext.current_context() is deprecated "
            "in favor of chat.logging.context.current_context().",
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def set_current_context(
        ...
        """
        warnings.warn(
            "chat.logging.context.LoggingContext.set_current_context() is deprecated "
            "in favor of chat.logging.context.set_current_context().",
            DeprecationWarning,
            stacklevel=2,
        )
...
def make_deferred_yieldable(deferred):
    """Given a deferred (or coroutine), make it follow the Chat logcontext
    rules:
...
def defer_to_thread(reactor, f, *args, **kwargs):
    ...
    The result deferred follows the Chat logcontext rules: you should `yield`
    on it.
