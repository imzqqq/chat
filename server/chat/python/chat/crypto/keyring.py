...
from chat.config.key import TrustedKeyServer
from chat.events import EventBase
from chat.events.utils import prune_event_dict
from chat.logging.context import make_deferred_yieldable, run_in_background
from chat.storage.keys import FetchKeyResult
from chat.types import JsonDict
from chat.util import unwrapFirstError
from chat.util.async_helpers import yieldable_gather_results
from chat.util.batching_queue import BatchingQueue
...
class BaseV2KeyFetcher(KeyFetcher):
    ...
    async def process_v2_response(
        ...
        GET /chat/key/v2/server, or a single entry from the list returned by
        POST /chat/key/v2/query.

        Checks that each signature in the response that claims to come from the origin
        server is valid, and that there is at least one such signature.

        Stores the json in server_keys_json so that it can be used for future responses
        to /chat/key/v2/query.

        Args:
            from_server: the name of the server producing this result: either
                the origin server for a /chat/key/v2/server request, or the notary
                for a /chat/key/v2/query.
