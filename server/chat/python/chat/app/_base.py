...
import chat
from chat.api.constants import MAX_PDU_SIZE
from chat.app import check_bind_error
from chat.app.phone_stats_home import start_phone_stats_home
from chat.config.homeserver import HomeServerConfig
from chat.crypto import context_factory
from chat.events.presence_router import load_legacy_presence_router
from chat.events.spamcheck import load_legacy_spam_checkers
from chat.events.third_party_rules import load_legacy_third_party_event_rules
from chat.logging.context import PreserveLoggingContext
from chat.metrics.background_process_metrics import wrap_as_background_process
from chat.metrics.jemalloc import setup_jemalloc_stats
from chat.util.caches.lrucache import setup_expire_lru_cache_entries
from chat.util.daemonize import daemonize_process
from chat.util.rlimit import change_resource_limit
...
def start_worker_reactor(appname, config, run_command=reactor.run):
    ...
    Args:
        ...
        config (chat.config.Config): config object
...
def listen_metrics(bind_addresses, port):
    ...
    """
    from chat.metrics import RegistryProxy, start_http_server
...
def listen_manhole(bind_addresses: Iterable[str], port: int, manhole_globals: dict):
    ...
    from chat.util.manhole import manhole
...
def refresh_certificate(hs):
    """
    Refresh the TLS certificates that Chat server is using by re-reading them from
...
async def start(hs: "HomeServer"):
    ...
    # Start the tracer
    chat.logging.opentracing.init_tracer(hs)  # type: ignore[attr-defined] # noqa
    ...
    # It is now safe to start your Chat.
    hs.start_listening()
...
def setup_sentry(hs):
    ...
    sentry_sdk.init(dsn=hs.config.sentry_dsn, release=get_version_string(chat))

    # We set some default tags that give some context to this instance
    with sentry_sdk.configure_scope() as scope:
        scope.set_tag("matrix_server_name", hs.config.server_name)

        app = hs.config.worker_app if hs.config.worker_app else "chat.app.homeserver"
