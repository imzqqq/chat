...
import chat.metrics
from chat.api.presence import UserPresenceState
from chat.events import EventBase
from chat.federation.sender.per_destination_queue import PerDestinationQueue
from chat.federation.sender.transaction_manager import TransactionManager
from chat.federation.units import Edu
...
from chat.metrics.background_process_metrics import (
    run_as_background_process,
    wrap_as_background_process,
)
from chat.types import JsonDict, ReadReceipt, RoomStreamToken
...
# Time (in s) after Chat's startup that we will begin to wake up destinations
# that have catch-up outstanding.
CATCH_UP_STARTUP_DELAY_SEC = 15

# Time (in s) to wait in between waking up each destination, i.e. one destination
# will be woken up every <x> seconds after Chat's startup until we have woken
...
class FederationSender(AbstractFederationSender):
            ...
            while True:
                ...
                async def handle_event(event: EventBase) -> None:
                    ...
                    if destinations:
                        ...
                        chat.metrics.event_processing_lag_by_event.labels(
                            "federation_sender"
                        ).observe((now - ts) / 1000)
                ...
                if events:
                    ...
                    chat.metrics.event_processing_lag.labels(
                        "federation_sender"
                    ).set(now - ts)
                    chat.metrics.event_processing_last_ts.labels(
                        "federation_sender"
                    ).set(ts)
                ...
                chat.metrics.event_processing_positions.labels(
                    "federation_sender"
                ).set(next_token)
    ...
    async def send_read_receipt(self, receipt: ReadReceipt) -> None:
        ...
        #
        # For even more background, see https://github.com/matrix-org/chat/issues/4730.
