...
from chat.util.caches import register_cache
...
class ResponseCache(Generic[KV]):
    ...
    def get(self, key: KV) -> Optional[defer.Deferred]:
        ...
        Returns a new Deferred (which also doesn't follow the chat
    ...
    def _set(
        ...
        *deferred* should run its callbacks in the sentinel logcontext (ie,
        you should wrap normal chat deferreds with
        chat.logging.context.run_in_background).

        Returns a new Deferred (which also doesn't follow the chat logcontext rules).
    ...
    async def wrap(
        ...
        **kwargs: Any,
        ...
        follow the chat logcontext rules and returns it.

        Otherwise, makes a call to *callback(*args, **kwargs)*, which should
        follow the chat logcontext rules, and adds the result to the cache.
