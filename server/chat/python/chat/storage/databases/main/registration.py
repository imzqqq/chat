...
from chat.api.errors import Codes, StoreError, SynapseError, ThreepidValidationError
from chat.metrics.background_process_metrics import wrap_as_background_process
from chat.storage.database import DatabasePool, LoggingDatabaseConnection
...
from chat.storage.types import Connection, Cursor
from chat.storage.util.id_generators import IdGenerator
from chat.storage.util.sequence import build_sequence_generator
from chat.types import UserID, UserInfo
...
class RegistrationWorkerStore(CacheInvalidationWorkerStore):
    ...
    async def is_server_admin(self, user: UserID) -> bool:
        """Determines if a user is an admin of this chat server.
    ...
    async def set_server_admin(self, user: UserID, admin: bool) -> None:
        """Sets whether a user is an admin of this chat server.
    ...
    async def count_all_users(self):
        """Counts all users registered on the chat server."""
    ...
    async def count_daily_user_type(self) -> Dict[str, int]:
        ...
        Counts 1) native non guest users
               ...
               3) bridged users
        who registered on the chat server in the past 24 hours
        """
    ...
    async def count_real_users(self):
        """Counts all users without a special user_type registered on the chat server."""
    ...
    async def user_get_bound_threepids(self, user_id: str) -> List[Dict[str, Any]]:
        """Get the threepids that a user has bound to an identity server through the chat server
        The chat server remembers where binds to an identity server occurred. Using this
