...
from chat.storage.database import LoggingDatabaseConnection
from chat.storage.engines import BaseDatabaseEngine
from chat.storage.engines.postgres import PostgresEngine
from chat.storage.schema import SCHEMA_COMPAT_VERSION, SCHEMA_VERSION
...
OUTDATED_SCHEMA_ON_WORKER_ERROR = (
    "Expected database schema version %i but got %i: run the main chat process to "
    "upgrade the database schema before starting worker processes."
)

EMPTY_DATABASE_ON_WORKER_ERROR = (
    "Uninitialised database: run the main chat process to prepare the database "
    "schema before starting worker processes."
)

UNAPPLIED_DELTA_ON_WORKER_ERROR = (
    "Database schema delta %s has not been applied: run the main chat process to "
    "upgrade the database schema before starting worker processes."
)
...
class _SchemaState:
    ...
    """The SCHEMA_VERSION of the oldest version of Chat server for this database
...
def _upgrade_existing_database(
    ...
    There can be multiple delta files per version. Chat server will keep track of
    ...
    if not is_empty and "main" in databases:
        from chat.storage.databases.main import check_database_before_upgrade

        assert config is not None
        check_database_before_upgrade(cur, database_engine, config)

    # update schema_compat_version before we run any upgrades, so that if chat
    ...
    for v in range(start_ver, SCHEMA_VERSION + 1):
        ...
        for entry in directory_entries:
            ...
            if ext == ".py":
                ...
                module_name = "chat.storage.v%d_%s" % (v, root_name)
...
def _apply_module_schema_files(
    ...
    Args:
        cur: database cursor
        database_engine: chat database engine class
