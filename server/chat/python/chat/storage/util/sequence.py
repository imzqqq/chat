...
from chat.storage.engines import (
    BaseDatabaseEngine,
...
table '%(table)s'. This can happen if Chat server has been downgraded and
then upgraded again, or due to a bad migration.

To fix this error, shut down Chat server (including any and all workers)
...
https://github.com/matrix-org/chat.

A temporary workaround to fix this error is to shut down Chat server (including
...
class SequenceGenerator(metaclass=abc.ABCMeta):
    ...
    def check_consistency(
        ...
        sync with the table, e.g. if Chat server gets rolled back to a previous
