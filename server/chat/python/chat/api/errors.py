...
class SynapseError(CodeMessageException):
    """A base exception type for Chat server errors which have an errcode and error
    message (as well as an HTTP status code).

    Attributes:
        errcode: Chat server error code e.g 'M_FORBIDDEN'
    """

    def __init__(self, code: int, msg: str, errcode: str = Codes.UNKNOWN):
        """Constructs a Chat server error.

        Args:
            code: The integer error code (an HTTP response code)
            msg: The human-readable error message.
            errcode: The Chat server error code e.g 'M_FORBIDDEN'
        """
...
class ProxiedRequestError(SynapseError):
    """An error from a general Chat server endpoint, eg. from a proxied Chat server API call.
...
class HttpResponseException(CodeMessageException):
    ...
    def to_synapse_error(self):
        ...
        are used as the errcode and error message in the new chat error.
