...
from chat.api.errors import Codes, NotFoundError, RedirectException, SynapseError
...
from chat.http.server import respond_with_html, respond_with_redirect
...
class SsoIdentityProvider(Protocol):
    ...
    Chat supports various implementations of IdPs, including OpenID Connect, SAML,
    and CAS.
    ...
    `/chat/client/r0/login/sso/redirect` can be correctly dispatched.
    """
...
class SsoHandler:
    ...
    def render_error(
        ...
        be found under `chat/res/templates/sso_error.html`.
    ...
    async def handle_redirect_request(
        ...
        return "/_chat/client/pick_idp?" + urlencode(
    ...
    async def _redirect_to_username_picker(
        ...
        e = RedirectException(b"/_chat/client/pick_username/account_details")
        e.cookies.append(
            b"%s=%s; path=/"
    ...
    async def complete_sso_ui_auth_request(
        ...
        recentish chat versions, but may not be for older users.
    ...
    async def handle_submit_username_request(
        ...
        if self._consent_at_registration:
            redirect_url = b"/_chat/client/new_user_consent"

        # otherwise, redirect to the completion page
        else:
            redirect_url = b"/_chat/client/sso_register"
    ...
    async def handle_terms_accepted(
        ...
        respond_with_redirect(request, b"/_chat/client/sso_register")
