...
from chat.http import RequestTimedOutError
from chat.http.client import SimpleHttpClient
from chat.http.site import SynapseRequest
from chat.types import JsonDict, Requester
from chat.util import json_decoder
from chat.util.hash import sha256_and_url_safe_base64
from chat.util.stringutils import (
...
class IdentityHandler(BaseHandler):
    ...
    async def threepid_from_creds(
        ...
        url = id_server + "/chat/identity/api/v1/3pid/getValidated3pid"
    ...
    async def bind_threepid(
        ...
        if use_v2:
            bind_url = "https://%s/chat/identity/v2/3pid/bind" % (id_server,)
            headers["Authorization"] = create_id_access_token_header(id_access_token)  # type: ignore
        else:
            bind_url = "https://%s/chat/identity/api/v1/3pid/bind" % (id_server,)
    ...
    async def try_unbind_threepid_with_id_server(
        ...
        url = "https://%s/chat/identity/api/v1/3pid/unbind" % (id_server,)
        url_bytes = b"/chat/identity/api/v1/3pid/unbind"
    ...
    async def requestEmailToken(
        ...
        try:
            data = await self.http_client.post_json_get_json(
                id_server + "/chat/identity/api/v1/validate/email/requestToken",
                params,
            )
            return data
        except HttpResponseException as e:
    ...
    async def requestMsisdnToken(
        ...
        try:
            data = await self.http_client.post_json_get_json(
                id_server + "/chat/identity/api/v1/validate/msisdn/requestToken",
                params,
            )
        except HttpResponseException as e:
            logger.info("Proxied requestToken failed: %r", e)
        ...
        data["submit_url"] = (
            ...
            + "chat/client/unstable/add_threepid/msisdn/submit_token"
        )
        return data
    ...
    async def proxy_msisdn_submit_token(
        ...
        try:
            return await self.http_client.post_json_get_json(
                id_server + "/chat/identity/api/v1/validate/msisdn/submitToken",
                body,
            )
        except RequestTimedOutError:
            raise SynapseError(500, "Timed out contacting identity server")
    ...
    async def _lookup_3pid_v1(
        ...
        try:
            data = await self.blacklisting_http_client.get_json(
                "%s%s/chat/identity/api/v1/lookup" % (id_server_scheme, id_server),
            ...
            if "mxid" in data:
                ...
                # https://github.com/matrix-org/chat/issues/5253#issuecomment-666246950
                return data["mxid"]
        except RequestTimedOutError:
            raise SynapseError(500, "Timed out contacting identity server")
        except OSError as e:
    ...
    async def _lookup_3pid_v2(
        ...
        try:
            hash_details = await self.blacklisting_http_client.get_json(
                "%s%s/chat/identity/v2/hash_details" % (id_server_scheme, id_server),
        ...
        try:
            lookup_results = await self.blacklisting_http_client.post_json_get_json(
                "%s%s/chat/identity/v2/lookup" % (id_server_scheme, id_server),
                {
    ...
    async def ask_id_server_for_third_party_invite(
        ...
        data = None
        base_url = "%s%s/chat/identity" % (id_server_scheme, id_server)

        if id_access_token:
            key_validity_url = "%s%s/chat/identity/v2/pubkey/isvalid" % (
                id_server_scheme,
                id_server,
            )
        ...
        if data is None:
            key_validity_url = "%s%s/chat/identity/api/v1/pubkey/isvalid" % (
                id_server_scheme,
                id_server,
            )
