...
from chat.config.oidc import OidcProviderClientSecretJwtKey, OidcProviderConfig
from chat.handlers.sso import MappingException, UserAttributes
from chat.http.site import SynapseRequest
from chat.logging.context import make_deferred_yieldable
from chat.types import JsonDict, UserID, map_username_to_mxid_localpart
from chat.util import Clock, json_decoder
from chat.util.caches.cached_call import RetryOnExceptionCachedCall
...
class OidcHandler:
    ...
    async def handle_oidc_callback(self, request: SynapseRequest) -> None:
        """Handle an incoming request to /_chat/client/oidc/callback
        ...
        if b"error" in request.args:
            ...
            # either the provider misbehaving or Chat being misconfigured.
...
class OidcProvider:
    ...
    def __init__(
        ...
        self._callback_path_prefix = (
            public_baseurl_path.encode("utf-8") + b"_chat/client/oidc"
        )
    ...
    async def handle_redirect_request(
        ...
        parameters:
          ...
          - ``response_type``: ``code``
          - ``redirect_uri``: the callback URL ; ``{base url}/_chat/client/oidc/callback``
    ...
    async def handle_oidc_callback(
        ...
        By this time we have already validated the session on the chat side, and
    ...
    def _remote_id_from_userinfo(self, userinfo: UserInfo) -> str:
        ...
        # Some OIDC providers use integer IDs, but Chat expects external IDs
        # to be strings.
...
class OidcSessionTokenGenerator:
    ...
    def generate_oidc_session_token(
        ...
        When Chat initiates an authorization flow, it creates a random state
