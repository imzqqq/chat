...
from chat.api.constants import LoginType
...
from chat.handlers._base import BaseHandler
from chat.handlers.ui_auth import (
    INTERACTIVE_AUTH_CHECKERS,
    UIAuthSessionDataConstants,
)
from chat.handlers.ui_auth.checkers import UserInteractiveAuthChecker
from chat.http import get_request_user_agent
from chat.http.server import finish_request, respond_with_html
from chat.http.site import SynapseRequest
from chat.logging.context import defer_to_thread
from chat.metrics.background_process_metrics import run_as_background_process
from chat.module_api import ModuleApi
from chat.storage.roommember import ProfileInfo
from chat.types import JsonDict, Requester, UserID
from chat.util import stringutils as stringutils
from chat.util.async_helpers import maybe_awaitable
from chat.util.macaroons import get_value_from_macaroon, satisfy_expiry
from chat.util.msisdn import phone_number_to_msisdn
from chat.util.stringutils import base62_encode
from chat.util.threepids import canonicalise_email

if TYPE_CHECKING:
    from chat.rest.client.login import LoginResponse
...
def login_id_phone_to_thirdparty(identifier: JsonDict) -> Dict[str, str]:
    ...
    if "country" not in identifier or (
        # The specification requires a "phone" field, while Chat used to require a "number"
...
class AuthHandler(BaseHandler):
    ...
    async def check_ui_auth(
        ...
        chat.rest.client._base.interactive_auth_handler as a
        decorator.
        ...
        if not sid:
            ...
            # chat.rest.client.register.RegisterRestServlet.on_POST
            ...
            if session.clientdict != clientdict:
                logger.warning(
                    ...
                    "authentication session. A future version of Chat "
    ...
    async def validate_login(
        ...
        # method which uses it (https://github.com/matrix-org/chat/issues/8836)
        #
        ...
        if identifier_dict["type"] == "m.id.thirdparty":
            ...
            # (See add_threepid in chat/handlers/auth.py)
            if medium == "email":
                try:
    ...
    async def add_threepid(
        ...
        # on accounts, so in future Chat will gain knowledge of specific
        # types (mediums) of threepid. For now, we still use the existing
        # infrastructure, but this is the start of chat gaining knowledge
    ...
    def _complete_sso_login(
        ...
        This exists purely for backwards compatibility of chat.module_api.ModuleApi.
        """
