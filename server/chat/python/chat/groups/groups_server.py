...
from chat.handlers.profile import MAX_AVATAR_URL_LEN, MAX_DISPLAYNAME_LEN
from chat.types import GroupID, JsonDict, RoomID, UserID, get_domain_from_id
...
def _parse_join_policy_dict(join_policy_dict: JsonDict) -> str:
    ...
    if join_policy_type not in ("invite", "open"):
        raise SynapseError(400, "Chat only supports 'invite'/'open' join rule")
...
def _parse_visibility_dict(visibility: JsonDict) -> bool:
    ...
    if vis_type not in ("public", "private"):
        raise SynapseError(400, "Chat only supports 'public'/'private' visibility")
