...
class LdapAuthProvider(object):
    ...
    async def check_auth(self, username, login_type, login_dict):
        ...
        # Local part of Chat ID which will be used in registration process
        localpart = username
    ...
    async def register_user(self, localpart, name, email_address):
        """Register a Chat server user, first checking if they exist.
        ...
        # create account
        # check if we're running a version of chat that supports binding emails
        # from password providers
        if parse_version(chat.__version__) <= parse_version("0.99.3"):
            ...
            # If Chat server has support, bind emails
            user_id, access_token = (
                await self.account_handler.register(
    ...
    async def _map_login_to_upn(self, username):
        ...
        Returns:
            ...
            Active Directory domain and local part of Chat ID.
        """
