#!/usr/bin/env python
#
# This script reads environment variables and generates a shared Chat worker,
...
WORKERS_CONFIG = {
    ...
    "federation_sender": {
        "app": "chat.app.federation_sender",
        "listener_resources": [],
        "endpoint_patterns": [],
    ...
    "federation_reader": {
        ...
        "endpoint_patterns": [
            "^/chat/federation/(v1|v2)/event/",
            "^/chat/federation/(v1|v2)/state/",
            "^/chat/federation/(v1|v2)/state_ids/",
            "^/chat/federation/(v1|v2)/backfill/",
            "^/chat/federation/(v1|v2)/get_missing_events/",
            "^/chat/federation/(v1|v2)/publicRooms",
            "^/chat/federation/(v1|v2)/query/",
            "^/chat/federation/(v1|v2)/make_join/",
            "^/chat/federation/(v1|v2)/make_leave/",
            "^/chat/federation/(v1|v2)/send_join/",
            "^/chat/federation/(v1|v2)/send_leave/",
            "^/chat/federation/(v1|v2)/invite/",
            "^/chat/federation/(v1|v2)/query_auth/",
            "^/chat/federation/(v1|v2)/event_auth/",
            "^/chat/federation/(v1|v2)/exchange_third_party_invite/",
            "^/chat/federation/(v1|v2)/user/devices/",
            "^/chat/federation/(v1|v2)/get_groups_publicised$",
            "^/chat/key/v2/query",
        ],
        "shared_extra_conf": {},
        "worker_extra_conf": "",
    ...
    "federation_inbound": {
        "app": "chat.app.generic_worker",
        "listener_resources": ["federation"],
        "endpoint_patterns": ["/chat/federation/(v1|v2)/send/"],
        "shared_extra_conf": {},
        "worker_extra_conf": "",
    },
    "event_persister": {
    ...
    "frontend_proxy": {
        "app": "chat.app.frontend_proxy",
        "listener_resources": ["client", "replication"],
        "endpoint_patterns": ["^/chat/client/(api/v1|r0|unstable)/keys/upload"],
        "shared_extra_conf": {},
        "worker_extra_conf": (
...
def generate_base_homeserver_config():
    """Starts Chat and generates a basic homeserver config, which will later be
...
def generate_worker_files(environ, config_path: str, data_dir: str):
    ...
    Args:
        ...
        config_path: Where to output the generated Chat main worker config file.
        data_dir: The location of the chat data directory. Where log and
            user-facing config files live.
    """
    # Note that yaml cares about indentation, so care should be taken to insert lines
    # into files at the correct indentation below.

    # shared_config is the contents of a Chat config file that will be shared amongst
    # the main Chat process as well as all workers.
    ...
    #
    # This config file will be passed to all workers, included Chat's main process.
    shared_config = {"listeners": listeners}

    # The supervisord config. The contents of which will be inserted into the
    # base supervisord jinja2 template.
    #
    # Supervisord will be in charge of running everything, from redis to nginx to Chat
    ...
    convert(
        "/conf/nginx.conf.j2",
        "/etc/nginx/conf.d/chat-server.conf",
...
def main(args, environ):
    ...
    # Start supervisord, which will start Chat, all of the configured worker
