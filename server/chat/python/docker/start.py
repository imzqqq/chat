...
def generate_config_from_template(config_dir, config_path, environ, ownership):
    ...
    log("Generating chat config file " + config_path)
    ...
    args = [
        ...
        "--config-path",
        config_path,
        # tell chat to put generated keys in /data rather than /compiled
        "--keys-directory",
        config_dir,
        "--generate-keys",
    ]
...
def run_generate_config(environ, ownership):
    """Run Chat server with a --generate-config param to generate a template config file
    ...
    if ownership is not None:
        # make sure that chat has perms to write to the data dir.
...
def main(args, environ):
    ...
    synapse_worker = environ.get("SYNAPSE_WORKER", "chat.app.homeserver")
    ...
    # if there are no config files passed to Chat server, try adding the default file
...
The Chat server docker image no longer supports generating a config file on-the-fly
...
"""
    ...
    log("Starting Chat server with args " + " ".join(args))
