 ...
 * A user should only have to agree to each version of each document once for
   their Chat ID, ie. having agreed to a set of terms in one client, they
...
API in question, ie. `/chat/identity/v2` for the IS API and
`/chat/integrations/v1` for the IM API.
 ...
 * `POST /chat/client/r0/account/3pid/unbind` endpoints on the client/server
   API
...
All current endpoints within `/chat/identity/api/v1/` will be duplicated
into `/chat/identity/v2`, noting that MSC2134 changes the behaviour of
...
Any request to any endpoint within `/chat/identity/v2`, with the exception
of:
 * `/chat/identity/v2`
 * `/chat/identity/v2/pubkey/*`
 * The new `$prefix/account/register` endpoint
 * The new `GET /chat/identity/v2/terms`
...
The existing endpoints under `/chat/identity/api/v1/` continue to be
...
and `/chat/client/r0/rooms/{roomId}/invite` requests and in the
...
ie.  `/chat/client/r0/account/deactivate` and
`/chat/client/r0/account/3pid/delete`, in which case the unbind will be
authenticated by a signed request from the Homeserver.

### HS Register API

The `bind_email` and `bind_msisdn` options to `/chat/client/r0/register` in
the client/server API will be removed. Due to the fact that
`/chat/identity/v2/3pid/bind` requires authentication, it will no longer be
 ...
 * `/chat/identity/v2/account`
 * `/chat/identity/v2/account/logout`
...
Any request to any endpoint in the IM API, and the `/chat/identity/v2/`
namespace of the IS API, with the exception of `/chat/identity/v2` itself,
may return:
...
The `/chat/identity/v2/3pid/unbind` endpoint must not return either of these
...
### `POST /chat/client/r0/account/3pid/unbind`
...
`POST /chat/client/r0/account/3pid/delete`, ie. `id_server`, `medium`,
...
Returns the same as `POST /chat/client/r0/account/3pid/delete`.

Clients may add IS bindings for 3PIDs that already exist on the user's
Homeserver account by using the `POST /chat/client/r0/account/3pid`
to re-add the 3PID.
...
of re-using existing primitives that already exist in the Chat ecosystem.
Custom HTTP headers are not used anywhere else within Chat. This also gives a
