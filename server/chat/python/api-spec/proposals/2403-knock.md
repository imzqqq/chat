...
/chat/client/r0/knock/{roomIdOrAlias}`. This allows the client to state
their intent to knock on a room.

Additionally, extensions to the `GET /chat/client/r0/sync` endpoint are
...
Request:
```
GET /chat/client/r0/sync HTTP/1.1
Content-Type: application/json
```
...
### `GET /chat/federation/v1/make_knock/{roomId}/{userId}`
...
Note that `GET /chat/federation/v1/make_join/{roomId}/{userId}` does not make `ver`
...
### `PUT /chat/federation/v1/send_knock/{roomId}/{eventId}`
...
```json
PUT /chat/federation/v1/send_knock/%21abc123%3Amatrix.org/%24abc123%3Aexample.org HTTP/1.1
Content-Type: application/json
...
* `POST /chat/client/knock/{roomIdOrAlias}`
* `POST /chat/client/unstable/xyz.amorgan.knock/knock/{roomIdOrAlias}`

S-S make_knock:

* `GET /chat/federation/v1/make_knock/{roomId}/{userId}`
* `GET /chat/federation/unstable/xyz.amorgan.knock/make_knock/{roomId}/{userId}`

S-S send_knock:

* `PUT /chat/federation/v1/send_knock/{roomId}/{eventId}`
* `PUT /chat/federation/unstable/xyz.amorgan.knock/send_knock/{roomId}/{eventId}`
