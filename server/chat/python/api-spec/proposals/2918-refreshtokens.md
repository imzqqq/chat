...
In Chat, requests to the Client-Server API are currently authenticated using non-expiring, revocable access tokens.
...
The Chat server can revoke the old access token right away, but does not have to since its lifetime is short enough that it will expire anyway soon after.

`POST /chat/client/r0/refresh`
...
The current spec states that "Chat servers should record which device each access token is assigned to".
...
This can however make the deployment of CLI tools for Chat a bit harder, since the credentials can't be statically defined anymore.
...
The refresh token endpoint should be served and used using the unstable prefix: `POST /chat/client/unstable/org.matrix.msc2918/refresh`.
...
If it also includes the refresh token, it is useless *if* the token was refreshed before (which will happen if the user just opens their Chat client in between).
...
Since all clients are "public" in the Chat world, there are no client-bound credentials that could be used, hence the rotation of refresh tokens.

---

The other kind of scenario where this change makes sense is to help further changes in the homeservers.
A good, recent example of this, is in Chat server v1.34.0 [they moved away from macaroons for access tokens](https://github.com/matrix-org/chat/pull/5588) to random, shorter, saved in database tokens, similar to [what GitHub did recently](https://github.blog/2021-04-05-behind-githubs-new-authentication-token-formats/).

Because there is no refresh token mechanism in the C2S API, most Chat server instances now have a mix of the two formats of tokens, and for a long time.
It makes it impossible to enforce the new format of tokens without invalidating all existing sessions, making it impossible to roll out changes like a web-app firewall in front of Chat server that verifies the shape and checksums of tokens even before reaching Chat server.

---

Lastly, expiring tokens already exist in Chat server (via the `session_lifetime` configuration parameter).
