...
specific Chat user ID. The `rule_id` of Sender rules is always the
Chat user ID of the user whose messages they'd apply to.
...
part of the user's Chat ID, separated by word boundaries.
...
```json
{
    ...
    "default": true,
    "enabled": true,
    "pattern": "[the local part of the user's Chat ID]",
    "actions": [
        "notify",
        {
            "set_tweak": "sound",
...
`!dj234r78wl45Gh4D:chat.dingshunyu.top`:

    curl -X PUT -H "Content-Type: application/json" "https://example.com/chat/client/v3/pushrules/global/room/%21dj234r78wl45Gh4D%3Amatrix.org?access_token=123456" -d \
    '{
       "actions" : ["dont_notify"]
     }'

To suppress notifications for the user `@spambot:chat.dingshunyu.top`:

    curl -X PUT -H "Content-Type: application/json" "https://example.com/chat/client/v3/pushrules/global/sender/%40spambot%3Amatrix.org?access_token=123456" -d \
    '{
       "actions" : ["dont_notify"]
     }'
...
specific sound (with a rule\_id of `SSByZWFsbHkgbGlrZSBjYWtl`):
    ...
    curl -X PUT -H "Content-Type: application/json" "https://example.com/chat/client/v3/pushrules/global/content/SSByZWFsbHkgbGlrZSBjYWtl?access_token=123456" -d \
    '{
       "pattern": "cake",
...
'cake' but ending with 'lie', superseding the previous rule:
    ...
    curl -X PUT -H "Content-Type: application/json" "https://example.com/chat/client/v3/pushrules/global/content/U3BvbmdlIGNha2UgaXMgYmVzdA?access_token=123456&before=SSByZWFsbHkgbGlrZSBjYWtl" -d \
    '{
       "pattern": "cake*lie",
       "actions" : ["notify"]
     }'
...
than the room, sender and content rules):
    ...
    curl -X PUT -H "Content-Type: application/json" "https://example.com/chat/client/v3/pushrules/global/override/U2VlIHlvdSBpbiBUaGUgRHVrZQ?access_token=123456" -d \
    '{
       "conditions": [
