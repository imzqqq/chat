...
async function discoverHomeserverUrl(serverName) {
     ...
     * or 8880 if no port is given. The target server must present a valid certificate for the IP address.
    ...
    if (delegatedHostname) {
         ...
         * <delegated_port> or 8880 if no port is provided. The target server must present a valid TLS certificate
        ...
        if (validateDomain(parsed.hostname)) {
             ...
             * looked up for chat._tcp.<delegated_hostname>. This may result in another hostname (to be resolved
            ...
            let records;
            try {
                records = await resolver.resolveSrv(`chat._tcp.${delegatedHostname}`);
            } catch (error) {
                // Pass
            }
             ...
             * made to the resolve IP address and a port of 8880, using a Host header of <delegated_hostname>.
             * The target server must present a valid certificate for <delegated_hostname>.
             */
            return {
                homeserverUrl: `https://${delegatedHostname}:8880`,
                serverName: delegatedHostname,
            };
        }
    }
     ...
     * for chat._tcp.<hostname>. This may result in a hostname (to be resolved using AAAA or A records) and port.
     * Requests are made to the resolved IP address and port, using 8880 as a default port, with a Host
     * header of <hostname>. The target server must present a valid certificate for <hostname>.
     */
    let records;
    try {
        records = await resolver.resolveSrv(`chat._tcp.${hostname}`);
    } catch (error) {
        // Pass
    }
    if (records && records.length > 0) {
        let record = records[0];
        return {
            homeserverUrl: `https://${record.name}:${record.port || '8880'}`,
            serverName: hostname,
        };
    }
     ...
     * port 8880 and a Host header containing the <hostname>.
     * The target server must present a valid certificate for <hostname>.
     */
    return {
        homeserverUrl: `https://${hostname}:8880`,
        serverName: hostname,
    };
}
