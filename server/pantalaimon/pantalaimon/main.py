...
async def init(data_dir, server_conf, send_queue, recv_queue):
    ...
    app.add_routes(
        [
            web.post("/chat/client/r0/login", proxy.login),
            web.get("/chat/client/r0/sync", proxy.sync),
            web.get("/chat/client/r0/rooms/{room_id}/messages", proxy.messages),
            web.put(
                r"/chat/client/r0/rooms/{room_id}/send/{event_type}/{txnid}",
                proxy.send_message,
            ),
            web.post(
                r"/chat/client/r0/rooms/{room_id}/send/{event_type}",
                proxy.send_message,
            ),
            web.post("/chat/client/r0/user/{user_id}/filter", proxy.filter),
            web.post("/.well-known/matrix/client", proxy.well_known),
            web.get("/.well-known/matrix/client", proxy.well_known),
            web.post("/chat/client/r0/search", proxy.search),
            web.options("/chat/client/r0/search", proxy.search_opts),
            web.get(
                "/chat/media/v1/download/{server_name}/{media_id}", proxy.download
            ),
            web.get(
                "/chat/media/v1/download/{server_name}/{media_id}/{file_name}",
                proxy.download,
            ),
            web.get(
                "/chat/media/r0/download/{server_name}/{media_id}", proxy.download
            ),
            web.get(
                "/chat/media/r0/download/{server_name}/{media_id}/{file_name}",
                proxy.download,
            ),
            web.post(
                r"/chat/media/r0/upload",
                proxy.upload,
            ),
            web.put(
                r"/chat/client/r0/profile/{userId}/avatar_url",
                proxy.profile,
            ),
        ]
    )
