...
shutdown () {
  if [ $_success = false ]; then
    printf "Your Live installation did not complete successfully.\n"
    printf "Please report your issue at https://github.com/imzqqq/issues\n"
  fi
...
main () {
  printf "${PURPLE}${BOLD}Live Installer v%s ${NC}\n\n" "$OWNCAST_VERSION"
  ...
  OWNCAST_URL="https://github.com/imzqqq/releases/download/v${OWNCAST_VERSION}/owncast-${OWNCAST_VERSION}-${PLATFORM}-${OWNCAST_ARCH}.zip"
  ...
  # Download release
  printf "${BLUE}Downloading${NC} Live v${OWNCAST_VERSION} for ${PLATFORM}"
  curl -s -L ${OWNCAST_URL} --output "${OWNCAST_TARGET_FILE}" &
  spinner $!
  printf "${GREEN}Downloaded${NC} Live v${OWNCAST_VERSION} for ${PLATFORM}  [${GREEN}✓${NC}]\n"
