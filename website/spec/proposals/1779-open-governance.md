# Proposal for Open Governance of Chat.org
...
This whole document is the proposed constitution proposal for Chat.org, and
...
**This MSC is now formalised in the official Rules of the Chat.org Foundation,
maintained at https://docs.google.com/document/d/1MhqsuIUxPc7Vf_y8D250mKZlLeQS6E39DPY6Azpc2NY**

## Introduction

Historically the core team of Chat has been paid to work on it by the same
employer (currently New Vector; the startup incorporated to hire the core
team in Aug 2017).  Whilst convenient in initially getting Chat built, we
recognise that this could create a potential conflict of interest between the
core team’s responsibilities to neutrally support the wider Chat.org ecosystem
...
This proposal seeks to establish a new open governance process for Chat.org,
such that once the specification has finally been ‘born’ and reached an initial
‘r0’ release across all APIs, control of Chat.org can be decoupled from New
...
The Spec Core Team are the technical experts who curate and edit the Chat
Specification from day to day, and so steer the evolution of the protocol by
having final review over which Chat Spec Changes (MSCs) are merged into the
core spec.
...
Team) keeps on mission and neutrally protects the development of Chat.
Guardians are typically independent of the commercial Chat ecosystem and may
even not be members of today’s Chat community, but are deeply aligned with the
...
formal part of the final Articles of Association of the Chat.org Foundation.
...
The Chat.org Foundation exists to act as a neutral custodian for Chat and to
...
For clarity: the Chat ecosystem is defined as anyone who uses the Chat
protocol. This includes (non-exhaustively):

 * End-users of Chat clients.
 * Chat client developers and testers.
 * Spec developers.
 * Server admins.
 * Chat packagers & maintainers.
 * Companies building products or services on Chat.
 * Bridge developers.
 * Bot developers.
 * Widget developers.
 * Server developers.
 * Chat room and community moderators.
 * End-users who are using Chat indirectly via bridges.
 * External systems which are bridged into Chat.
 * Anyone using Chat for data communications.

"Greater benefit" is defined as maximising:

 * the number of Chat-native end-users reachable on the open Chat network.
 * the number of regular users on the Chat network (e.g. 30-day retained federated users).
 ...
 * the signal-to-noise ratio of the content on the open Chat network (i.e. minimising spam).
 * the ability for users to discover content on their terms (empowering them to select what to see and what not to see).
 * the quality and utility of the Chat spec (as defined by ease and ability
...
rather than closed deployments. For example, if WhatsApp adopted Chat it
wouldn’t be a complete win unless they openly federated with the rest of the
Chat network.

### Values

As Chat evolves, it's critical that the Spec Core Team and Guardians are
...
The contents and direction of the Chat Spec is governed by the Spec Core Team;
a set of experts from across the whole Chat community, representing all
aspects of the Chat ecosystem.  The Spec Core Team acts as a subcommittee of
the Foundation.

Members of the Spec Core Team pledge to act as a neutral custodian for Chat on
behalf of the whole ecosystem and uphold the Guiding Principles of the project
as outlined above.  In particular, they agree to drive the adoption of Chat as
 ...
 * Reviewing Chat Spec Change proposals and Spec PRs.

 * Contributing to and reviewing reference implementations of Chat Spec Change
   proposals.

 * Shepherding Chat Spec Changes on behalf of authors where needed.

 * Triaging Chat Spec issues.
...
consensus in the event of failing to get agreement on a Chat Spec Change.
...
spec and is updated using the Chat Spec Change process.  However, changes to
...
The Guardians form the legal Board of Directors of The Chat.org Foundation CIC
 ...
 * As well as the Spec Core Team committee, they may also oversee committees for
   other areas such as marketing Chat.org, registering custom event types,
   or "Made for Chat" certification.
...
Guardians are chosen typically to be independent of the commercial Chat
ecosystem (and especially independent from New Vector), and may even not be
members of today’s Chat community. However, they should be deeply aligned with
...
The intention is for Matthew & Amandine (the original founders of Chat) to
 ...
 * Ensuring that all projects follow the Chat spec.
 ...
 * Owns the core Chat IP in an asset lock, which shall be transferred from New Vector:
   ...
   * Owns the Chat.org marketing swag (t-shirts, stickers, exhibition stands etc).
 * Responsible for finding someone to run the Chat.org homeserver (currently New Vector).
 * Publishes the spec.
 ...
 * Publishes the website (including ensuring This Week In Chat and similar exist to promote independent projects).
 * Manages any future IANA-style allocations for Chat, such as:
   * mx:// URI scheme.
   * TCP port 8880.
   * .well-known URIs
* Ensures that Chat promotion is happening (e.g. ensuring that meetups &
  events & community activity is supported).

In future:

 * Contracts entities to work on Chat if such contracts help the Foundation to
   ...
   kits).
 * Manages a "Made for Chat" certification process? (to confirm that products
   are actually compatible with Chat).
...
> Chat as an openly standardised secure decentralised communication protocol and
> network, open to all, and available to the public for no charge;

> 4.1.2  build and develop an appropriate governance model for Chat through the
> Foundation, in order to drive the adoption of Chat as a single global
...
We are currently planning to release r0 of the Chat Spec at the end of Jan 2019, and
finalise the Foundation's articles of association shortly afterwards based on the
contents of this MSC once passed FCP.

This will coincide with the formal asset transfer of Chat.org's assets from
