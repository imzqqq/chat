---
date: '2019-06-28'
title: 'This Week in Chat 2019-06-28'
categories:
  - This Week in Chat
author: Ben Parsons
...
Matthew & Amandine talk to Ross Schulman; new Chat Foundation Guardian!
...
New Vector won [Hottest GovTech Startup in Europe](https://theeuropas2019.thepathfounder.com/page/1409435/finalists) at The Europas tech awards last night for work on rolling out Chat for France and elsewhere!
...
### Rust Embedded community is moving to Chat

They held a public vote, and Chat won [very convincingly](https://github.com/rust-embedded/wg/issues/357).
...
> With 1.0 shipped we are now starting to take a closer look at Chat performance more generally and this will be a theme for us over the coming months. We want to improve not only large scale deployments such as Chat but also optimise for smaller instances.
>
...
>
> Other than that, based on 1.0 feedback, we have been working on improving the Chat upgrade path and expect to put out a new release next week containing the tweaks. Specifically this means improving configuration for Docker installs, and configuration management for sending emails.
>
> We’re also implementing open tracing into Chat, initially to help with e2ee debugging, but it will make tracking down strange behaviour easier more generally.
>
...
>
> Note: 1.0 is the last release to support python 2 and postgres 9.4, the next Chat release will drop support for both, see <https://matrix.org/blog/2019/04/08/synapse-deprecating-postgres-9-4-and-python-2-x> for more.
...
> This week was spent working on a big revamp of [ruma-events], the library that defines Rust types for the "events" used in Chat.  
> After some discussion in [#ruma:matrix.org], I decided to make a move towards treating ruma-events as a higher-level library. Previously, ruma-events has more or less offered Rust types that are exact representations of the JSON structures used by Chat. However, by representing events this way, it would be possible for users to easily create values that, while valid JSON, would be invalid events according to the specification.  
...
> [Jeon] 0.9.0 release. It is a release candidate for Jeon 1.0.0 which complies with the Chat stable release 1.0.  
> Not a lot changes, just added missing endpoints and events.  
> Also I started to work on [JeonServer], a Chat server written on java.

[Jeon] is a set of Java interfaces to Chat APIs, [JeonServer] is a proposed homeserver.
...
### GSOC project: Chat Visualisations 🎓

[Eisha] appeared with this terrific GSOC update:

> The GSoC project “Chat Visualisations” has made good progress during this first period:
>
...
> * A (server-side) backend has been implemented so that the application can directly talk to the PostgreSQL database of Chat. You can find it on [this repo](https://github.com/Kagamihime/matrix-visualisations-backend).
>
> Note that the support of the Federation API has been postponed so I could work on this Chat database backend.
...
![Chat Visualisations](/blog/img/2019-06-28-vis.png)
...
>
> * Some characters in IRC usernames (such as “|”) would cause IRC user’s messages to not appear on the Chat side.
...
There is already [substantial documentation available](https://github.com/Sorunome/mx-puppet-bridge/blob/master/bridge.md), and a Chat room at [#mx-puppet-bridge:sorunome.de].
...
>
> You can download the iOS app via TestFlight soon, join [#app:pattle.im](https://to.chat.dingshunyu.top/#/#app:pattle.im) to get the link immediately when it's available!
>
> #### Other changes
>
...
> #### Follow Pattle
>
> Follow development in [#app:pattle.im](https://to.chat.dingshunyu.top/#/#app:pattle.im)!
>
> #### Support me
>
...
This is a read-only Chat client, which takes the contents of a public room and "enacts" it, that is, it performs it using the Web Audio API in your browser
...
> Besides this, with Chat v1.0 already out, we've taken the opportunity to simplify the installation instructions a bit.  
...
### Update from the *Chat on Debian* team

<https://matrix-team.pages.debian.net/blogue/2019/06/26/june-2019-matrix-on-debian-update/>

> This is an update on the state of Chat-related software in Debian
...
### Link existing unix accounts with accounts on a Chat homeserver

Cos created an article [detailing how to link existing unix accounts with accounts on a Chat homeserver](https://coshacks.blogspot.com/2019/06/matrix-synapse-authentication-from-unix.html).
...
> I made a Chat bot that uses reactions to do polls! The code is available at <https://github.com/babolivier/matrix-poll-bot> (with a screenshot of how it works) and you can invite [@poll-bot:abolivier.bzh] in your room if you want to try it out 🙂
...
kernel.org have set up an ActivityPub instance (see <https://people.kernel.org/about>.) Not strictly Chat but interesting that they decided to move to a federated platform.

First GSOC evaluation submissions were due this week, all four Chat projects are proceeding well. See Eisha's update above, and [the other three last week](https://matrix.org/blog/2019/06/21/this-week-in-matrix-2019-06-21).
