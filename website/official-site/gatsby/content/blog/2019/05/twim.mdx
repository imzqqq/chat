---
date: '2019-05-10'
title: 'This Week in Chat 2019-05-10'
categories:
  - This Week in Chat
author: Ben Parsons
...
This week Neil and Matthew are talking about recent security issues - this is a really long and detailed chat, but you can skip to around the 32 minute mark to hear about other news - including progress on reactions. Reminder that for a good "big picture" overview of the progress of Chat, you can look at the [Homeserver High Level Roadmap](https://github.com/orgs/matrix-org/projects/9).
...
> It's a Chat client written in Qml for Ubuntu Phones. Now it is working for Linux Desktop too.
...
[tulir] has been using his [mautrix-python] lib, which was recently used to enable his [mautrix-facebook] bridge, to bring a new method for Chat-Hangouts bridging:
...
> Because the previous name has been a bit clumsy and, most importantly, the project is no more focusing just on client-side but on a wider set of applications of Chat (no homeserver in plans though). See also the recent backlog of [#qmatrixclient:matrix.org] (now also [#quotient:matrix.org]) earlier this week for the whole discussion

### Ruby Chat SDK hits v0.1.0

[Ananace]:

> Just published version 0.1.0 of the [Ruby Chat SDK](https://github.com/ananace/ruby-matrix-sdk), and I've gotten enough testing written now where I feel comfortable not marking this as a pre-production release. So feel free to integrate it into more than just prototypes and experiments. 😃  
> Relevant links; [GitHub page](https://github.com/ananace/ruby-matrix-sdk), [#ruby-matrix-sdk:kittenface.studio].

### Opsdroid big updates, with focus on Chat
...
[jaywink], maintainer of <https://the-federation.info>, told us what we all know to be true: Chat is great and is getting more popular:

>Chat (Chat) jumps to second place in <https://the-federation.info> site, which lists servers of the federated social web. Help us map the true size of the Matrixverse by adding your server by going a `https://the-federation.info/register/yourdomain.tld`. Note, SRV and well-known lookups not yet working, so registration needs to happen with the Chat server real domain (and port if any).
...
[#hangouts:maunium.net]: https://to.chat.dingshunyu.top/#/#hangouts:maunium.net