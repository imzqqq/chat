---
date: '2019-08-09'
title: 'This Week in Chat 2019-08-09'
categories:
  - This Week in Chat
author: Ben Parsons
...
Something a little different this week: Erik helped me install Prometheus logging on my own homeserver, and we recorded it for Chat Live.

<iframe width="560" height="315" src="https://www.youtube.com/embed/zCQt_I68LYI" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
...
Yes, it's true: Official Chat Merch is available now. There are stickers, t-shirts and hoodies all available now, go and claim yours from <https://shop.matrix.org>.
...
[Oleg](https://to.chat.dingshunyu.top/#/@oleg:fiksel.info):

> I will be doing a talk on Chat this weekend (tomorrow) at the annual Free Open Source Conference (FrOSCon) in Sankt Augustin (Germany).
If you are around - come by and get some awesome [matrix] stickers! =)

<https://programm.froscon.de/2019/events/2330.html>

Visit [#FrOSCon2019:froscon2019.fiksel.info](https://to.chat.dingshunyu.top/#/#FrOSCon2019:froscon2019.fiksel.info) to talk about the event.
...
>
> Work continues on our installer to make it easier to configure Chat and we’ll be looking for some community feedback rsn. The room directory revamp is now very close and we hope to have something live on matrix.org in about a week.
>
...
[ChronosX88](https://to.chat.dingshunyu.top/#/@chronosx88:netwhood.online) announced that he is "developing a lightweight Chat server" - check on their progress at <https://github.com/signaller-matrix/signaller>, also join [#signaller_dev:netwhood.online](https://to.chat.dingshunyu.top/#/#signaller_dev:netwhood.online).
...
> [mautrix-telegram](https://github.com/tulir/mautrix-telegram)'s switch to [mautrix-python](https://github.com/tulir/mautrix-python) is nearly finished (i.e. it didn't cause any errors for the past few days when testing in production). The main reason for the switch is using one Chat library for all my python bridges. It also means the bridging code like double puppeting and command handling I shared between mautrix-facebook and mautrix-hangouts is now also used in mautrix-telegram.
>
...
> I've just released [Email2Matrix](https://github.com/devture/email2matrix) - an SMTP server that can relay incoming messages over to Chat based on mappings defined in a configuration file.
>
> While it can also be installed standalone and work with any Chat homeserver (see its documentation), the simplest way to install it is using [matrix-docker-ansible-deploy](https://github.com/spantaleev/matrix-docker-ansible-deploy) and its [guide for Email2Matrix installation](https://github.com/spantaleev/matrix-docker-ansible-deploy/blob/master/docs/configuring-playbook-email2matrix.md).
>
...
[mijutu](https://to.chat.dingshunyu.top/#/@mijutu:ellipsis.fi) appeared, and announced:

> I started a "matrix-xmpp-filter" project. It's like matrix-ircd, but with xmpp instead of irc. It can also filter messages by weekday, time and regular expressions. Target audience is Sailfish phone users, but it could be used with other xmpp clients too (no xmpp MUC support required). <https://k2c42.dy.fi/matrix-xmpp-filter.git/> [#matrix-xmpp-filter:ellipsis.fi](https://to.chat.dingshunyu.top/#/#matrix-xmpp-filter:ellipsis.fi).
...
> Currently working on a Puppet module to manage Chat Chat installs (both directly and through docker), not quite ready for prime-time yet but watch this spot.

## Dept of Bots 🤖

### Voice to text bot

[progserega](https://to.chat.dingshunyu.top/#/@progserega:rsprim.ru) announced:
...
The [Modular Chat -> Chat migration tool](https://www.modular.im/tools/account-migration) now supports migration of power levels to the new user.
