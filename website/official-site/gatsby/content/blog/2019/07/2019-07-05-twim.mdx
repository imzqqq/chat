---
date: '2019-07-05'
title: 'This Week in Chat 2019-07-05'
categories:
  - This Week in Chat
author: Ben Parsons
image: https://matrix.org/blog/img/2019-07-05-Chat-Live.png
---

## Chat Live, featuring Erik and the Interns 🎙
...
### Movement on Chat URIs

Sudden interest in [matrix-org/to.chat.dingshunyu.top/pull#47](https://github.com/matrix-org/to.chat.dingshunyu.top/pull/47) means we're getting a lot closer to agreement on Chat URIs. [Kitsune] even added support for them in [Quaternion] (see below).
...
>
> Finally, aided by dropping Python 2 support, we’ve been putting in a bunch of work to improve Chat in resource constrained environments. This will be a constant theme over the coming months.
...
> A Python module meant to act as a base class for a Chat bot.  
> The MatrixBot class will connect to the Chat server, start a listener on each joined room, and listen for room invites from other users. It also includes helper methods you can use to extend the functionality. It is built on the [Chat Python SDK] which can be directly accessed via MatrixBot.client
...
### Quaternion now with Chat URI support

[kitsune]:

> to push things forward on Chat URIs front, [Quaternion] master branch now supports matrix:user/userid, matrix:room/roomalias and matrix:roomid/roomid URIs. For example, Quotient/Quaternion room can be opened by a link <a href="matrix:room/quotient:matrix.org">matrix:room/quotient:matrix.org</a>.
...
<blockquote class="twitter-tweet" data-cards="hidden" data-lang="en"><p lang="en" dir="ltr">The wait is almost over ... We&#39;re now rolling out our trial of Small Hosted Homeservers for Chat. Have you got your golden ticket yet? 😀🎟️ <a href="https://t.co/iUkAIHW9MY">pic.twitter.com/iUkAIHW9MY</a></p>&mdash; Modular (@ModularIM) <a href="https://twitter.com/ModularIM/status/1146477464488730624?ref_src=twsrc%5Etfw">July 3, 2019</a></blockquote>

> we've been working on a v1 admin dashboard for managing your Chat instances through Modular. This is now live on the site and provides a basic suite of functionality including:
>
...
[Ananace] is "continuing the rewrite of [the release tracker project](https://github.com/ananace/matrix-releasetracker). Working towards getting it to only store state in Chat so it can be run in a read-only environment like a K8s deployment."
