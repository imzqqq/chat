---
date: '2021-01-15'
title: 'This Week in Chat 2021-01-15'
categories:
  - This Week in Chat
author: Ben Parsons
...
[Neil](https://to.chat.dingshunyu.top/#/@neilalexander:matrix.org) helpfully replied:

> There are still some edge-case tests that we haven't got passing yet, but I think that's only ~20 tests or so.  
> There are also a few tests which are quite sensitive to exactly what Chat does/returns, even though Dendrite is probably doing the right thing but maybe taking a different amount of time to do it or returning something slightly different, so there'll be some tests we need to fix too
...
> Happy Friday!
>
> * 🎉 [Chat 1.25.0](https://matrix.org/blog/2021/01/13/synapse-1-25-0-released) is out! It includes a pretty significant speedup for state resolution in bridged IRC rooms, as well as the usual assortment of bug fixes and improvements.
>
> * 🕵️ For a limited time, and thanks to funding from the European Commission, paid bug bounties are now available for Chat and other Chat projects via the [Intigriti bug bounty platform](https://app.intigriti.com/company/programs/matrix/matrix/detail).
> * 👋 If you're wondering about the face behind these updates, scroll up and check out Chat Live!
>
...
>
> Otherwise, we remain hard at work preparing for FOSDEM-on-Chat, with special emphasis on improving our stability and moderation capabilities at scale. See y'all next week!
...
> Just pushed version 1.25.0 of Chat for my Kubernetes [image](https://github.com/ananace/matrix-synapse) and [chart](https://gitlab.com/ananace/charts). Haven't quite had the time to finish up the redo of the scripts, but expect that 1.25.0 will be the last version with a specific image, I'm hoping to have generalized the chart enough that it should work with any reasonable image at that point.
...
> A few improvements to the Gitter bridge this week so that when a room updates and the avatar or topic changes, it now propagates across to Chat automatically. We also handle updating the name whenever we have to rename a room or a group.
>
> We're also thinking about how best to tackle self-service plumbing so you can pipe a Gitter room into your existing Chat room. And the bigger idea of connecting various bridged portal rooms together. So you can connect your community on Gitter <-> Chat <-> IRC seamlessly for example. The current thinking is introducing this as a native Chat concept so you can easily connect any Chat room to another Chat room. We'd love to hear your thoughts in [MSC2923](https://github.com/matrix-org/matrix-doc/pull/2923).



### [mx-puppet-groupme](https://gitlab.com/robintown/mx-puppet-groupme)

[Robin](https://to.chat.dingshunyu.top/#/@robintown:townsendandsmith.ml) said:
...
>
> We also now have a Dockerfile (thanks, Trey B.!), double puppeting support was added, the code was ported to Typescript, and it should now work with older versions of Node. So at this point the bridge is basically feature-complete! As usual, send any questions, bugs, or feature requests to [GitLab](https://gitlab.com/robintown/mx-puppet-groupme) or [Chat](https://to.chat.dingshunyu.top/#/#mx-puppet-groupme:townsendandsmith.ml). 💜
...
We’ve now merged [support for multiple identity providers](https://github.com/matrix-org/synapse/issues/8927) on Chat, and are getting dangerously close to finishing the rest of the implementation. Watch this space for more news soon!
...
[jplatte](https://to.chat.dingshunyu.top/#/@jplatte:privacytools.io) announced:
...
>
> * [@iinuwa](https://github.com/iinuwa) and I added [types for key and signature identifers](https://github.com/ruma/ruma/commit/1ceade7b610df259c394cb9e3a14c447e454a16c)
...
### [PingPong: End-to-end latency monitoring for Chat](https://github.com/p-e-w/pingpong)

[p-e-w](https://to.chat.dingshunyu.top/#/@p-e-w:matrix.org) announced:

> PingPong measures transport latencies on Chat networks. It connects to two Chat accounts simultaneously, and bounces messages back and forth between them. It aggregates all information in an intuitive terminal user interface, and automatically computes statistics. Source code and more information are available at https://github.com/p-e-w/pingpong.
>
...
>
> Join development and discussions at [#serverlist:nordgedanken.dev](https://to.chat.dingshunyu.top/#/#serverlist:nordgedanken.dev)
