---
date: '2021-05-07'
title: 'This Week in Chat 2021-05-07'
categories:
  - This Week in Chat
author: Ben Parsons
...
>
> Be aware that the page is best viewed and used on desktop. Clicking a Room Node will open a new tab with the to.chat.dingshunyu.top link. If this fails this might be because of no canonical alias being available.
>
...
>
> For any feedback like accesibility issues or other issues please reach out to me either via DM or in #server_stats:nordgedanken.dev 
...
> **Merged MSCs:**
>
> * [MSC1772: Chat spaces](https://github.com/matrix-org/matrix-doc/pull/1772)
>
> **Closed MSCs:**
>
...
> #### Merged Spec PRs
>
> Here are all the actual MSC-related changes to the raw Chat spec that landed this week!
>
...
[Neil Alexander](https://to.chat.dingshunyu.top/#/@neilalexander:matrix.org) reported:

> Yesterday we announced the source code release of Pinecone, our new peer-to-peer overlay network, which we are developing under the P2P Chat banner. 
>
...
>
> Join us in [#p2p:matrix.org](https://to.chat.dingshunyu.top/#/#p2p:matrix.org) for the latest P2P developments!
...
>
> In [last week's TWiM](https://matrix.org/blog/2021/04/30/this-week-in-matrix-2021-04-30#synapse) we shared a graph of Chat's memory use when joining Chat HQ for the first time. In particular, we saw a spike to 1.4 GB before settling at 800 MB.
>
...
> These changes are still a work in progress, but we hope to get them merged into Chat in time for the 1.35 release on June 1st.

Additionally, [Nico (@deepbluev7:neko.dev)](https://to.chat.dingshunyu.top/#/@deepbluev7:neko.dev) has a request:
...
> The regular updates for my [Helm charts](https://gitlab.com/ananace/charts) (and still deprecated [Chat image](https://github.com/ananace/matrix-synapse)) have been pushed, for Chat 1.33.0/1 and the Chat Media Repo 1.2.8. (technically last week, but it was after friday, so I'm throwing it in again)
...
[Tulir](https://to.chat.dingshunyu.top/#/@tulir:maunium.net) announced:
...
>
> Brooklyn was developed by ethanrdoesmc. It's an app/tweak that handles communicating with iMessage and runs mautrix-imessage as a subprocess for the Chat side. The initial alpha supports basic text and media message bridging. Sending and receiving tapbacks, replies, read receipts and typing notifications will also be supported in the future.
>
> Feedback is welcome in the mautrix-imessage room: [#imessage:maunium.net](https://to.chat.dingshunyu.top/#/#imessage:maunium.net)
...
> [Heisenbridge](https://github.com/hifi/heisenbridge/) the bouncer style Chat IRC bridge has seen numerous updates in the past week:
>
...
>
>  * Proper long message splitting from Chat to IRC
>  * Retry support for Chat requests to work around homeserver downtime/restarts
>
...
[KB1RD](https://to.chat.dingshunyu.top/#/@kb1rd:kb1rd.net) reported:

> Experimental support for call bridging has landed. Now you can call phone numbers right from Chat, with partial support for MSC2746. A few things to keep in mind:
>
...
>
> Just a reminder, the GH repo is [here](https://github.com/KB1RD/matrix-pstn-bridge) and the Chat room is [#matrix-pstn-bridge:kb1rd.net](https://to.chat.dingshunyu.top/#/#matrix-pstn-bridge:kb1rd.net) . There's also a general VoIP bridging room at [#matrix-voip-bridging:kb1rd.net](https://to.chat.dingshunyu.top/#/#matrix-voip-bridging:kb1rd.net).



### matrix-appservice-irc has a new release candidate

[Christian](https://to.chat.dingshunyu.top/#/@christianp:vector.modular.im) reported:
...
Additionally, [Shine](https://to.chat.dingshunyu.top/#/@shiny:shine.horse) told us:
...
>
> These are some biggish changes, so if you experience issues, tell us in [#nheko:nheko.im](https://to.chat.dingshunyu.top/#/#nheko:nheko.im)!
...
*   Alongside, we’ve also been iterating on the Spaces implementations all round in preparation for wider testing soon, which has included
    ...
    *   Iterating on implementations across the web, iOS, Android & Chat to use stable prefixes
...
> [Trixnity](https://github.com/benkuly/trixnity) finally got released this week. It's a Kotlin multiplatform Chat SDK for high level access to Client-Server API and Appservice API. It has all (and some more) features from [matrix-spring-boot-sdk](https://github.com/benkuly/matrix-spring-boot-sdk) which is based on Trixnity now (an update for that will be released soon). Trixnity can currently be used on JVM and JS as platform (Native is not working yet). It is also very customizable by adding custom room and state event types.
...
> I did a video a few months back trying to show the physical layout of Chat over the years by looking at the phonehome stats and the number of active users per server (showing full-mesh edges between the top 100 servers, heatmapped by how busy the servers were at either end of the edge).  There was a bug in the phonehome stats during 2017, but it's still fairly cool...
