...
and curate sets of users and rooms in Chat. [Back in Nov
...
and users, like [+matrix:matrix.org](https://to.chat.dingshunyu.top/#/+matrix:matrix.org).
...
Element Android) and Chat
[1.34.0](https://matrix.org/blog/2021/05/17/synapse-1-34-0-released) - so head
over to your nearest Element, make sure it's connected to the latest Chat
(and that Chat has Spaces enabled in its config) and find some Space to
explore! [#community:matrix.org](https://to.chat.dingshunyu.top/#/#community:matrix.org)
...
This is also a bit of a turning point in Chat's architecture: we are now
using rooms more and more as a generic way of modelling new features in
Chat.  For instance, rooms could be used as a structured way of storing
...
this pushes us towards ensuring rooms are as lightweight as possible in Chat -
...
global decentralised filesystem hierarchy on top of Chat - grouping the
...
Thanks for flying Chat into Space;

Matthew & the whole Spaces (and Chat) team.