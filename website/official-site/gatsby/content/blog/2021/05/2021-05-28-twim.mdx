---
date: '2021-05-28'
title: 'This Week in Chat 2021-05-28'
categories:
  - This Week in Chat
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/4vr9RRYNWWE" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
...
[anoa](https://to.chat.dingshunyu.top/#/@andrewm:amorgan.xyz) gifted us with:

> Here's your weekly spec update! The heart of Chat is the specification - and this 
> is modified by Chat Spec Change (MSC) proposals. Learn more about how the process works at https://spec.matrix.org/unstable/proposals.
>
>
> ## MSC Status
>
...
> We have interns! In addition to Chat's participation in Google Summer of Code, Element has funded two Outreachy interns to work on backend projects, and we're overjoyed to welcome the following folks to Chat:
>
> * Shay (Outreachy) is focused on modernizing [Sydent](https://github.com/matrix-org/sydent) and [Sygnal](https://github.com/matrix-org/sygnal), our reference implementations of a Chat Identity Server and a Chat Push Gateway respectively.
...
>
> We're also working to [unify and standardize the module API](https://github.com/matrix-org/synapse/pull/10062) which is used to extend Chat, and your feedback would be very welcome. Similarly, if you have any experience with adding custom modules to Docker images or Debian packages and have thoughts on how we should best support that (or just examples of projects which do that well), please weigh in at [#9944](https://github.com/matrix-org/synapse/issues/9944).
>
> Lastly, we're looking forward to the release of Chat 1.35 which will bring significant improvements to memory use during room joins, but that's for next week. 😉
...
[f0x](https://to.chat.dingshunyu.top/#/@f0x:pixie.town) announced:
...
> Media still gets forwarded to the `<local>` component where it gets stored in Chat's media folder with an accompanying database entry, so all media is still stored long-term in the way Chat expects it to be, so the proxy can be removed at any point. 
>
...
[Conduit](https://serverstats.nordgedanken.dev) is a Chat homeserver written in Rust.

[timokoesters](https://to.chat.dingshunyu.top/#/@timokoesters:fachschaften.org) reported the latest 
...
multi-implementation Chat homeserver landscape really beginning to take shape!
...
[Half-Shot](https://to.chat.dingshunyu.top/#/@Half-Shot:half-shot.uk) tells us:
...
`#<channel_name>:libera.chat`. You can also add the homeserver to your room directory in your Chat client to search all the bridged rooms.
>
> We're also hanging out in [#libera-matrix:libera.chat](https://to.chat.dingshunyu.top/#/#libera-matrix:libera.chat), if you have any questions. Of course, the bridge is proving to be very popular so please be patient :)

Definitely one of the most sought-after Chat bridges lately.
Props to Half-Shot, Libera.Chat and everyone else for getting
it all together in such short notice.

### Heisenbridge Updates

[hifi](https://to.chat.dingshunyu.top/#/@hifi:vi.fi) told us:

> #### Plumbing IRC channels 🧑‍🔧
>
> Somewhat experimental feature that has landed this week is the ability to use single puppeting in a public Chat room to bridge IRC channels.
>
> This support comes through a new `PLUMB` admin command for network rooms that will ask the bridge bot to join a public Chat room and an IRC channel.
>
> On IRC side there will be only a single user/bot that will relay messages from Chat users. This is a stop-gap between fully puppeting Chat users on IRC and doesn't need explicit permission from an IRC network to use by channel operators as long as bots are allowed.
>
> On Chat side puppets will join and leave based on their presence on the IRC channel. The bridge bot does not need any special permissions in a room as it doesn't do permission synchronization so it can be kept without any power levels.
>
...
[NeoChat](https://apps.kde.org/neochat/) is a Chat client written using KDE 
technologies, most notably the Kirigami UI framework.

[Carl Schwan](https://to.chat.dingshunyu.top/#/@carl:kde.org) reported:
...
[AppAraat](https://to.chat.dingshunyu.top/#/@AppAraat:matrix.org) told us:
...
[krille](https://to.chat.dingshunyu.top/#/@krille:janian.de) announced:
...
[Fractal](https://wiki.gnome.org/Apps/Fractal) is a Chat client written in Rust, and 
...
[GoMatrixHosting](https://www.gomatrixhosting.com/) is a SaaS Chat hosting platform.

[Michael](https://to.chat.dingshunyu.top/#/@michael:perthchat.org) reported:

> We are finally open for business. Start your own Chat service today with GoMatrixHosting!
>
...
>
> Or come say hello at: [#general:gomatrixhosting.com](https://to.chat.dingshunyu.top/#/#general:gomatrixhosting.com)
...
[Asbjørn](https://to.chat.dingshunyu.top/#/@asbjorn:olli.ng) reported:
...
[Beeper](https://www.beeper.com/) is a Chat client and service that makes it easy to 
connect all your digital communications in one place.

[Eric Migicovsky](https://to.chat.dingshunyu.top/#/@eric:beeper.com) told us:
...
Eric's Chat ID is [@eric:beeper.com](https://to.chat.dingshunyu.top/#/@eric:beeper.com). Reach 
...
[Robin](https://to.chat.dingshunyu.top/#/@robintown:townsendandsmith.ml) announced:
...
>
> It requires zero configuration—just invite the bot to your space and all of the rooms and subspaces within it, make sure it has invite permissions, and you're good to go! You're welcome to use my hosted version [@airlock:townsendandsmith.ml](https://to.chat.dingshunyu.top/#/@airlock:townsendandsmith.ml), or [run it yourself](https://gitlab.com/robintown/airlock#running-the-bot-yourself).

It's great to see people building useful tools to bridge the gap as Spaces inches closer out of beta.

## Dept of Interesting Projects 🛰️

### Host your blog on Chat

[evol](https://to.chat.dingshunyu.top/#/@evol:evolved.systems) told us:

> About a month ago I came up with the idea that it would be interesting to use Chat as an API for a blog. Sometime later it turned out that it's possible, and one thing led to another, and now I have a blog hosted on Chat 😃
>
> Long story short, the blog is a space and each post is a room within that space. This allows you to read the posts in your Chat client of choice and also have discussions right underneath the content. 
>
> You can read more details on how it's done on the blog itself (incl. links to ugly code on my Github): https://evolved.systems/hosting-a-blog-on-matrix/. If you want to chat about this, join [#blog:evolved.systems](https://to.chat.dingshunyu.top/#/#blog:evolved.systems) !

Longtime readers may remember Luke Barnard's take on the concept of Blogs built on 
Chat back in 2019 with
...
statistics about the Chat network neatly catalogued and browsable.
...
end-to-end encrypted social network built on top of Chat.

[farribeiro](https://to.chat.dingshunyu.top/#/@farribeiro:matrix.org) announced:
...
I love seeing Chat projects dipping into the social networking sphere. It's an 
untapped market!
