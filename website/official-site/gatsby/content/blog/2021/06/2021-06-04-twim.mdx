---
date: '2021-06-04'
title: 'This Week in Chat 2021-06-04'
categories:
  - This Week in Chat
author: Ben Parsons
...
Wired UK have published a feature on Chat in their print edition this month. We'll be sure to link to it when it's made available online!

![Wired Article](/blog/img/2021-06-04-wired.png)

### German-universities poll

[jfkimmes](https://to.chat.dingshunyu.top/#/@jfkimmes:matrix.org) shared:

> I just learned that in a poll of 89 universities in Germany, Chat ranked third place in the chat category already. 
>
...
[Oleg](https://to.chat.dingshunyu.top/#/@oleg:fiksel.info) clarified:

> The evaluation was "which solution are you using".

[Florian](https://to.chat.dingshunyu.top/#/@florian:dsn.tm.kit.edu) added:

> The Instant Messaging part starts at slide 15, the first chart on that slide is "which solution do you use", the second is "How content are you with the solution?", with Chat having the best average of all solutions, namely ~8.8/10.

also [JCG](https://to.chat.dingshunyu.top/#/@jan.christian:gruenhage.xyz):
...
>
> Finally [MSC3231](https://github.com/matrix-org/matrix-doc/pull/3231) is a (currently draft status) MSC from Callum, one of Chat's [GSoC students](https://matrix.org/blog/2021/05/20/google-summer-of-code-2021) this year! His project aims to allow native token-based registration to homeservers (the idea is so that you can generate a few tokens from your registration-disabled homeserver and hand them out to a few trusted friends and family members).
>
...
> Hello from the Chat team! A short update for a short workweek (thanks, bank holidays!):
>
> * Chat 1.35 [was released](https://matrix.org/blog/2021/06/01/synapse-1-35-0-released) this week! The Spaces flag is on by default, a bunch of bugs were fixed, and we've landed many of the prerequisites to eliminating RAM spikes on room joins.
>
...
>
> While they both implement Chat' media endpoints, they serve rather different niches, where matrix-media-repo fully decouples the media repo aspect, my proxy cooperates with Chat's filesystem and database, to speed up operation while ultimately making it a seamless drop-in and removal process.
>
...
> And another weekly installment of Kubernetes [Helm Chart](https://gitlab.com/ananace/charts) (and deprecated [Docker image](https://github.com/ananace/matrix-synapse)) updates, tracking the Chat releases this week (1.35.0/1).
...
[hifi](https://to.chat.dingshunyu.top/#/@hifi:vi.fi) shared this great demonstation video of [Heisenbridge](https://github.com/hifi/heisenbridge/):
...
[Half-Shot](https://to.chat.dingshunyu.top/#/@Half-Shot:half-shot.uk) hit us late with a pair of updates:
...
### Thunderbird Chat support

[freaktechnik](https://to.chat.dingshunyu.top/#/@freaktechnik:humanoids.be) announced:

> [Thunderbird](https://thunderbird.net) now has Chat support based on `matrix-js-sdk` enabled in the Nightly builds.
>
> The star feature is probably that we support multiple Chat accounts in the same client. Right now all your unencrypted rooms with text messages should work fine. While we think we won't destroy your account's state, it's still recommended to use a testing account with it. During account setup, it will ask you for a password, even if the homeserver supports SSO. If you intend to log in through SSO, just leave the password field blank.
>
> We're not quite at the point where we support all the things you love about chatting with Chat. Many of the missing features and polish to make communication successful are tracked in [this meta bug](https://bugzilla.mozilla.org/show_bug.cgi?id=1712710). The goal for that milestone is to enable Chat in our Beta builds.
>
> You can get a Thunderbird Nightly build at the bottom of [thunderbird.net](https://thunderbird.net/#channel) by switching from "Beta Channel" to "Nightly Channel". If you run into bugs with the Chat integration, please report them through [this form](https://bugzilla.mozilla.org/enter_bug.cgi?product=Chat%20Core&component=Chat). When filing a bug, please include debug logs. You can copy the debug logs for the account by going to the "Show Accounts" dialog, right clicking the account and selecting "Copy Debug Log". Note that the debug log may contain information from any of your conversations, so you might want to check the contents before posting it anywhere.

Also, check out Chat Live!
...
> Callum, our GSoC student, after spending some time on Chat, had now his first go at Nheko's codebase. He implemented, that you can now just enter the server name on registration instead of the full URL. This means entering conduit.rs or matrix.org works now nicely, since those servers are actually hosted at a different URL. He's now working on the Token Registration MSC, which he will implement in Chat and Nheko, so exciting times ahead!
>
...
[Bruno](https://to.chat.dingshunyu.top/#/@bwindels:matrix.org) announced:
...
[tusooa](https://to.chat.dingshunyu.top/#/@tusooa:tusooa.xyz) reported on kazv:
...
>
> As German Chat community grows this is a great opportunity to meet each other and hack together.
>
> #### Chat Dev Room
>
> We are planing to do a virtual Dev Room this year. The idea is to exchange on the latest Chat development and projects, get to know each other and drink `<your_favorite_beverage>` (virtually) together. 😉
>
> #### To make it happen we need your help!
> #### Dev Room is living from talks and workshops - this is your chance to present your Chat project or to do a workshop!
>
...
>
> #### Chat Open Source booth
>
> It was a great place to chit-chat and to get your in-depth answers regarding Chat at FOSDEM this year. 👍️
>
> We also planing to have a virtual booth at FrOSCon.
>
> **We need your support in answering questions about Chat or just to have a good time.**
>
> #### Get in touch
> #### If you want to take part please contact [@oleg:fiksel.info](https://to.chat.dingshunyu.top/#/@oleg:fiksel.info) (or oleg@fiksel.info) ASAP to add you to the Dev Room participants list.
>
> BTW: we also have a [#FrOSCon:fiksel.info room](https://to.chat.dingshunyu.top/#/#FrOSCon:fiksel.info)
