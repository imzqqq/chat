---
date: '2021-02-12'
title: 'This Week in Chat 2021-02-12'
categories:
  - This Week in Chat
author: Ben Parsons
...
This week we have Neil presenting P2P Chat over Bluetooth. It's titled "P2P Chat Bluetooth Low Energy demo", but I thought Neil and Matthew brought a pretty good energy to the video!
...
> Famedly (Chat based application for German hospitals and clinics) seems to be getting positive feedback so far: https://medtech-zwo.de/aktuelles/nachrichten/nachrichten/famedly-ueberzeugt-mit-messenger-dienst-im-krankenhaus.html
>
...
>
> * They actually use `[matrix]–Protokoll` to refer to the Chat protocol. The open and standardized Chat protocol seems to be a big selling factor.
...
> #### New Spec Platform
>
> [Will Bamberg](https://github.com/wbamberg) has done an incredible job with getting both an entire redesign and an internal refactoring to the Chat Spec. Remaining work includes ironing out some remaining minor blockers (https://github.com/matrix-org/matrix-doc/pull/3002, https://github.com/matrix-org/matrix-doc/pull/3003), merging changelog representation with respect to the current idea of a global Chat Spec version number (https://github.com/matrix-org/matrix-doc/pull/2992) and then finally hooking up all the CI/CD to deploy it all (my job!).
>
...
> We survived FOSDEM! Last week was relatively slower in Chat land as we paused to catch our breath, but we do have news!
>
> First, releases. Chat 1.27.0rc2 is out now ([changelog](https://github.com/matrix-org/synapse/blob/v1.27.0rc2/CHANGES.md#synapse-1270rc2-2021-02-11)), and we anticipate a full 1.27 release early next week. This is basically the branch that FOSDEM ran, so we think it's a good one.
>
...
[Pierre](https://to.chat.dingshunyu.top/#/@pierre:mamieserv.fr) told us:
...
>
> [Chat](https://github.com/YunoHost-Apps/synapse_ynh/tree/master) integration had been updated to 1.25.0 (1.26.0 available in branch `testing`)
>
...
[qg](https://to.chat.dingshunyu.top/#/@qg:supercable.onl) offered:
...
>
> Thanks to [@michi7801](https://to.chat.dingshunyu.top/#/@michael7801:rapgru.com) we can now provide builds for macOS!! 🍎
>
...
[jplatte](https://to.chat.dingshunyu.top/#/@jplatte:privacytools.io) offered:

> In the last two weeks,
>
> * [@DevinR528] fixed [the push rule precedence for iterating a Ruleset](https://github.com/ruma/ruma/pull/403)
>
> * [@jsparber] added [an unstable workaround for Chat sometimes requiring authentication in the profile API (the spec says no authentication is required)](https://github.com/ruma/ruma/pull/408)
...
> Thanks to [@Peetz0r](https://github.com/Peetz0r), [matrix-docker-ansible-deploy](https://github.com/spantaleev/matrix-docker-ansible-deploy) can now install a bunch of tools for monitoring your Chat server: the [Prometheus](https://prometheus.io) time-series database server, the Prometheus [node-exporter](https://prometheus.io/docs/guides/node-exporter/) host metrics exporter, and the [Grafana](https://grafana.com/) web UI.
>
> To install these services, follow our [Enabling metrics and graphs (Prometheus, Grafana) for your Chat server](https://github.com/spantaleev/matrix-docker-ansible-deploy/blob/master/docs/configuring-playbook-prometheus-grafana.md) docs page.
...
>
> Feel free to join [#space-hub:nordgedanken.dev](https://to.chat.dingshunyu.top/#/#space-hub:nordgedanken.dev) for discussions or have an eye on https://github.com/matrix-spaces-generator where at the time of writing this only an example of the spaces folder structure exists.
...
[sorunome](https://to.chat.dingshunyu.top/#/@sorunome:sorunome.de) shared:
