---
date: '2021-10-08'
title: 'This Week in Chat 2021-10-08'
categories:
  - This Week in Chat
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/hj-W53PSqbg" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
...
>
> From page 6 of the open MSCs there's [MSC2723](https://github.com/matrix-org/matrix-doc/pull/2723). The MSC is relatively small, but solves a fairly important problem with forwarded messages in Chat: where were they forwarded from? From a glance, it has minor technical feedback and could do with an implementation (psst: client authors, this should be easy and fit well into a North American Long Weekend project 😇).
>
...
[Neil Alexander](https://to.chat.dingshunyu.top/#/@neilalexander:matrix.org) said:
...
>
> While there is still some work to be done to ensure the Pinecone network gracefully handles large-scale mobility events, protocol development is slowly progressing and we will soon be looking at how Chat federation can be made more efficient by using routing intelligence from the overlay network.
>
> If you have a passion for routing protocols and/or peer-to-peer systems, we're looking to hire engineers to work in the P2P Chat space — get in touch by [emailing your resume to us](mailto:elementio@jobs.workablemail.com)!
>
> We also have our very own [P2P Chat](https://to.chat.dingshunyu.top/#/#p2p:matrix.org) room where you can follow along with the latest discussions and also occasionally pick up experimental iOS and Android builds of Element with an embedded P2P-enabled Dendrite homeserver.
...
[callahad](https://to.chat.dingshunyu.top/#/@callahad:matrix.org) told us:

> We [released Chat 1.44](https://matrix.org/blog/2021/10/05/synapse-1-44-0-released) this week! The release was mainly focused on bug fixes and internal cleanups. In particular, we've made significant strides toward enforcing static type annotations across several more Chat modules, we continue to remove "magic" code from our configuration handler, and we've landed significant refactors to both the user directory and federated event authentication. These changes pay off significant technical debt, reduce Chat's maintenance cost, and ease future bug fixes.
>
...
[Ananace](https://to.chat.dingshunyu.top/#/@ace:kittenface.studio) said:
...
[Pascal](https://to.chat.dingshunyu.top/#/@lascapiriot:matrix.org) reported:

> Hi! Rocket.chat is working on a bridge with Chat !!!! So nice 😊 
>
...
[kitsune](https://to.chat.dingshunyu.top/#/@kitsune:matrix.org) told us:
...
[ajbura](https://to.chat.dingshunyu.top/#/@ajbura:matrix.org) said:
...
>
> Join the matrix room: [#cinny:matrix.org](https://to.chat.dingshunyu.top/#/#cinny:matrix.org)



### SchildiChat

[qg](https://to.chat.dingshunyu.top/#/@qg:supercable.onl) said:
...
> * ... and many more (read the release announcements on [GitHub](https://github.com/SchildiChat/schildichat-desktop/releases) or join our [announcement room (#schildichat-web-announcements:matrix.org)](https://to.chat.dingshunyu.top/#/#schildichat-web-announcements:matrix.org) to stay informed)
>
...
>
> Feel free to join the [SchildiChat space (#schildichat:matrix.org)](https://to.chat.dingshunyu.top/#/#schildichat:matrix.org), and visit our discussion rooms!
...
[gen3](https://to.chat.dingshunyu.top/#/@gen3:blackline.xyz) said:

> Hello! Halcyon is a Chat bot library created with the intention of being easy to install and use. My goal is to lower the barrier of entry currently required to write and maintain a bot. I do not intend you to need a database, or have to juggle encryption keys.
>
...
> Please check out the example in https://github.com/WesR/Halcyon and join us over in [#halcyon:blackline.xyz](https://to.chat.dingshunyu.top/#/#halcyon:blackline.xyz)



### jOlm

[brevilo](https://to.chat.dingshunyu.top/#/@brevilo:matrix.org) said:
...
[Sumner](https://to.chat.dingshunyu.top/#/@sumner:sumnerevans.com) said:
...
> If you are interested in learning more, the source code is here: https://sr.ht/~sumner/standupbot and you can join the development matrix room [#standupbot-dev:sumnerevans.com](https://to.chat.dingshunyu.top/#/#standupbot-dev:sumnerevans.com)
...
### Newgrounds MVS (Chat adVancement Server)

[mr_johnson22](https://to.chat.dingshunyu.top/#/@mr_johnson22:matrix.org) offered:

> #### An (unofficial) Chat server for the Newgrounds community.
>
...
>
> * https://matrix.ngmvs.one: the homeserver itself (Chat). It has open registration, but only for [Newgrounds Supporters](https://www.newgrounds.com/supporter) (ie. patrons) via SSO logins with Newgrounds accounts. But Newgrounds doesn't have its own "Log in with Newgrounds" capability, so that leads me to the next component:
> * [ng-saml-idp](https://gitlab.com/ngmvs/ng-saml-idp): middleware built on [PySAML2](https://pysaml2.readthedocs.io/en/latest/) for using Newgrounds logins as a SAML2 Identity Provider. The code is tailored for my Chat installation, but it can easily be tweaked to allow Newgrounds SSO logins to any SAML2-compatible service!
>
> * [W-Bot](https://gitlab.com/ngmvs/wbot): a Chat appservice/bot built on [mautrix-python](https://github.com/tulir/mautrix-python) that serves two purposes:
> 	1. Creates Chat rooms for Newgrounds movie/game submission pages (ie. maps `#portal_view_<ID>:ngmvs.one` to `https://www.newgrounds.com/portal/view/<ID>`), and adds a widget containing the submission itself to these rooms (but only if it's a movie--widgets for games will only have a link)
>
...
> 	
>
>   Point 2 also means that anyone on the Chat network can make a room that's restricted to Newgrounds Supporters. Just make sure the bot has a high enough power level to kick users!
>
>   Public (and federation-accessible) instance is at [@w-bot:ngmvs.one](https://to.chat.dingshunyu.top/#/@w-bot:ngmvs.one).
>
>   (PS: The "W" stands for "double"--because of its job of "copying" rooms into Chat.)
>
> * [MVSX](https://addons.mozilla.org/en-US/firefox/addon/newgrounds-mvsx/): a Firefox extension that, when visiting a Newgrounds movie/game submission page, puts a button in the URL bar that sends you to the Chat room created for that submission by W-Bot.
>
> **This week's updates:**
>
...
>
> * The MVSX extension has been updated to allow joining rooms with Chat clients other than [app.ngmvs.one](https://app.ngmvs.one/). Right-clicking the URL bar icon now shows a drop-down list of alternative clients, and lets you set which one to use by default (on left-clicks of the icon).
>
> Discussion: [#ngmvs-public-discussion:ngmvs.one](https://to.chat.dingshunyu.top/#/#ngmvs-public-discussion:ngmvs.one)
