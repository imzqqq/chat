---
date: '2021-04-23'
title: 'This Week in Chat 2021-04-23'
categories:
  - This Week in Chat
author: Ben Parsons
...
[Neil](https://to.chat.dingshunyu.top/#/@neilj:matrix.org) announced:
...
[Jonathan](https://to.chat.dingshunyu.top/#/@jboi:jboi.nl) noticed:
...
[Nico (@deepbluev7:neko.dev)](https://to.chat.dingshunyu.top/#/@deepbluev7:neko.dev) said:

> #### What is the Chat URI scheme?
>
...
>
> In Chat you have similar identifiers, most notably users (@user:example.org) and rooms ([#room:example.org](https://to.chat.dingshunyu.top/#/#room:example.org)). The matrix URI scheme now allows you to open such identifiers in your favourite Chat clients. You can now just click on matrix:u/user:example.org or matrix:r/room:example.org and start chatting with the user or join the room. (There is a bit more to it, you actually need an action after the identifier for it, otherwise it will just open the user profile or show the room, if you are joined to it, but that is the idea. For more details, check https://github.com/matrix-org/matrix-doc/blob/master/proposals/2312-matrix-uri.md )
>
> As you can see the format of the matrix: scheme is a bit different that standard Chat identifiers. This is mostly because they need to start with `scheme:`, where `scheme` is different depending on the application, but some characters are also treated differently in URIs, which is why the matrix scheme decided to avoid them where possible.
>
> #### What does it mean, that the matrix: scheme is now part of the HTML standard?
>
> In general desktop and mobile clients could already implement the Chat scheme before, FluffyChat, Quaternion, gomuks and Nheko have even done so already. This means if you are using those clients, you can already use the Chat scheme today!
>
...
>
> 1. Browsers need to implement the HTML standard change and allow web apps to register the Chat scheme.
>
...
>
> You may be wondering about to.chat.dingshunyu.top, which was used to share matrix identifiers before. In the long term matrix URIs will probably replace it. Watch the next few TWIMs to find out what happens and/or subscribe to the following issues: https://github.com/matrix-org/to.chat.dingshunyu.top/issues/191 https://github.com/matrix-org/to.chat.dingshunyu.top/issues/192
>
> #### Closing words
>
> As you can see, this was a big step for Chat as a whole. This of course couldn't have been possible without [Alexey](https://to.chat.dingshunyu.top/#/@kitsune:matrix.org) writing such an awesome proposal and [MayeulC](https://to.chat.dingshunyu.top/#/@mayeul:mayeul.net) pushing for the changes in browsers and the HTML standards. A lot of other people were also involved in implementing the scheme in their clients, sending the [RFC to IANA](https://www.iana.org/assignments/uri-schemes/prov/matrix) or working on to.chat.dingshunyu.top, which was our solution until now and will probably bridge the gap into the future. The matrix scheme being in the HTML standard may look small, but a lot of stuff was blocked on it, that can now start making progress. Exciting, isn't it?
...
[kitsune](https://to.chat.dingshunyu.top/#/@kitsune:matrix.org) added:
...
> #### Spec Updates
>
> Folks from the Chat Core team continue to chip away at the various Spaces-related MSCs in order to further the initial implementation across all three Element clients. The Spec Core Team is cleaning up some deprecated APIs that have been hanging around for way too long now (Identity Service v1 APIs), as well as continuing to focus on getting various, implemented end-to-end encryption MSCs documented and into the spec.
>
...
[matrix-media-repo](https://github.com/turt2live/matrix-media-repo) is a highly customizable multi-domain media repository for Chat
...
>
> See https://github.com/turt2live/matrix-media-repo/releases/tag/v1.2.7 for more details, and visit #media-repo:t2bot.io on Chat for questions and support.

### Chat

[callahad](https://to.chat.dingshunyu.top/#/@callahad:matrix.org) announced:

> [Chat 1.32 is out](https://matrix.org/blog/2021/04/22/synapse-1-32-2-released)! This release focused on internal cleanups and is our first release without support for Python 3.5, so nothing too groundbreaking, but we're pleased with the improvements we're making.
>
...
> My regularly scheduled [chart](https://gitlab.com/ananace/charts) updates for Chat 1.3.1 (the deprecated image got a bump too) and matrix-media-repo 1.2.7 have been pushed.

and then

> Got the Chat 1.32.2 update up on my [chart](https://gitlab.com/ananace/charts) too, replacing the pulled 1.32.1 version.

### YunoHost

[Mamie](https://to.chat.dingshunyu.top/#/@mamie:mamieserv.fr) reported:
...
>
> [Chat](https://github.com/YunoHost-Apps/synapse_ynh/tree/master) integration had been updated to 1.31.0 (1.32.0 available in branch `testing`)
>
...
[Half-Shot](https://to.chat.dingshunyu.top/#/@Half-Shot:half-shot.uk) announced:

> Hello folks! Today at 17:41, we sunset the matrix.org gitter bridge after many years of active service. It started out as a [tiny little project, forked from another gitter<->IRC bridge](https://github.com/matrix-org/matrix-appservice-gitter/commit/c641e6eb4916c1addbff5ba50d0bf0a5f2084690) before eventually becoming part of the matrix-appservice-bridge family of bridges (thanks Leo for putting in the hard work there!). It never quite gained all the nice features that the IRC and Slack bridges enjoyed, in part due to it's dependence on the use of one bot account (serving as a relay). However, it was still the entrypoint for Chat users into the likes of NeoVIM and Typescript and continued to run for several years.
>
> Years later, [gitter.im gained the ability to speak Chat](https://matrix.org/blog/2020/12/07/gitter-now-speaks-matrix),  so the old bridge can finally be laid to rest. Thanks to everyone that used and supported the bridge, and we hope you enjoy using the new one!
>
> Thanks from the bridge team 🌉!

### matrix-puppeteer-line

[Fair](https://to.chat.dingshunyu.top/#/@fair:miscworks.net) announced:
...
[Nico (@deepbluev7:neko.dev)](https://to.chat.dingshunyu.top/#/@deepbluev7:neko.dev) reported:
...
>
> I hope you'll have a wonderful time with this release and if you find any bugs, bug us in [#nheko:nheko.im](https://to.chat.dingshunyu.top/#/#nheko:nheko.im) or open an issue on our [bug tracker](https://github.com/Nheko-Reborn/nheko/issues)!

### Fractal

[Alexandre Franke](https://to.chat.dingshunyu.top/#/@afranke:matrix.org) offered:
...
>
> But as interesting as all that is, it pales in comparison to our bigger announcement. We are rewriting Fractal! Julian, Alexandre and Thib have documented the thought process behind this decision and the work done so far. For more details, have a look at [Julian’s blog](https://blogs.gnome.org/jsparber/2021/04/23/getting-fractal-up-to-speed/). The summary is that even though such rewrites are usually frowned upon (for good reasons), two major technological shifts (from a homegrown backend to matrix-rust-sdk and from GTK3 to GTK4) made it an option that we needed to consider… and so we did! This should result in a cleaner codebase, easier to maintain in the long run, and improvements that we can share with the rest of the Chat community.
>
...
*   On Spaces, we’ve been continuing to implement [MSC3083](https://github.com/matrix-org/matrix-doc/pull/3083) (Restricting room membership based on space membership) on the Web, Android & Chat, while also iterating on iOS.
...
[Bruno](https://to.chat.dingshunyu.top/#/@bwindels:matrix.org) commented:
...
[Bruno](https://to.chat.dingshunyu.top/#/@bwindels:matrix.org) offered:
...
[Ruma](https://www.ruma.io/) is a set of Rust library crates around Chat

[Johannes Becker](https://to.chat.dingshunyu.top/#/@jbecker:famedly.de) told us:
...
[Asbjørn](https://to.chat.dingshunyu.top/#/@asbjorn:olli.ng) told us:

> Cactus Comments is a federated comment system for the open web built on Chat.
>
...
>
> Join our Chat room: [#cactus:cactus.chat](https://to.chat.dingshunyu.top/#/#cactus:cactus.chat)

### Chat World from the point of view of my Chat

[MTRNord](https://to.chat.dingshunyu.top/#/@mtrnord:nordgedanken.dev) told us:
...
>
> For my server at point of writing this results in 2348 Servers of which are 2269 running Chat, 70 running Dendrite, 8 running Conduit and 1 running Construct.
>
> #### Tech used
>
...
Very cool! Reminder to also check out this [set of graphs showing Chat versions over time](https://graph.settgast.org/d/z1nplqXik/matrix?orgId=1) from [Chris](https://to.chat.dingshunyu.top/#/@chris:settgast.org).

Warning!

[Timo from Conduit](https://to.chat.dingshunyu.top/#/@timokoesters:fachschaften.org) announced:
