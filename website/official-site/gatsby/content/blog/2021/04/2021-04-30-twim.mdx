---
date: '2021-04-30'
title: 'This Week in Chat 2021-04-30'
categories:
  - This Week in Chat
author: Ben Parsons
...
[Oda Nobunaga](https://to.chat.dingshunyu.top/#/@odanobunaga:matrix.org) said:

> The German IT platform Heise has conducted an interview with the university of Innsbruck which chose Chat/Element over Microsoft Teams. Reasons: it's free, decentralized/federated, expandable and secure.
>
...
Heise article, naturally, is in German. Element have [some content in English discussing the deployment](https://element.io/case-studies/university-of-innsbruck), and [how well-suited Chat is for univerities](https://element.io/blog/universal-universities/). Finally, the best kind of feedback: an Innsbruck professor cheers the utility of federation:
...
[Nico](https://to.chat.dingshunyu.top/#/@deepbluev7:neko.dev) offered:
...
[Lazalatin](https://to.chat.dingshunyu.top/#/@lazalatin:kif.rocks) added:
...
[anoa](https://to.chat.dingshunyu.top/#/@andrewm:amorgan.xyz) offered:
...
>
> * [MSC1772: Chat spaces](https://github.com/matrix-org/matrix-doc/pull/1772) (merge) (5 days remaining)
>
> **Merged MSCs:**
>
...
> #### Spec Updates
>
> We've opened up a new room for MSC authors to come and ask about the MSC process, as well as for review and feedback on their MSC ideas over at [#sct-office:matrix.org](https://to.chat.dingshunyu.top/#/#sct-office:matrix.org). If you're unsure what the next steps for your MSC are, or aren't sure whether an idea would make sense for inclusion in the Chat Specification, then this room is the perfect place to ask about it. Come on by!
>
...
[Neil Alexander](https://to.chat.dingshunyu.top/#/@neilalexander:matrix.org) announced:

> We've just released the first public P2P Chat demo using the experimental Pinecone protocol. There are builds available for Android right now and iOS is following shortly via Testflight as soon as it passes Apple's beta review:
>
...
>
> Join [#p2p:matrix.org](https://to.chat.dingshunyu.top/#/#p2p:matrix.org) for more chat and to provide feedback!
...
>
> We managed to work on a few features the last two weeks, but there are some very annoying issues with major Chat clients:
>
...
> We expect to release 1.33 on Tuesday; more on that next week. But generally speaking, much of our development focus has been on profiling the memory usage of small homeservers joining large, highly-federated rooms for the first time. For example, we see a graph like this when joining Chat HQ:
...
[hifi](https://to.chat.dingshunyu.top/#/@hifi:vi.fi) said:

> Announcing a new bouncer style Chat IRC bridge to _bridge_ the gap between IRC and Chat in a way that's easy to configure and manage by homeserver administrators for small scale deployment.
>
...
>
> Intended target audience are people who run their own homeserver and would like to transition from a combination like ssh+irssi to pure Chat (with weechat-matrix, of course!) but still keep IRC close without losing the "fine details" of the protocol that happens when plumbing a room properly.
>
> As the bridge is written by someone who doesn't really know Python help is much appreciated in the form of filing issues, sending pull requests or just discussing about it on Chat to tell how you would like to use it!
>
> Development and support in [#heisenbridge:vi.fi](https://to.chat.dingshunyu.top/#/#heisenbridge:vi.fi)
>
> Thanks!
...
[Fair](https://to.chat.dingshunyu.top/#/@fair:miscworks.net) offered:
...
>     * To capture this in Chat, instead of sending read receipts for a group chat message, the bridge puts a reaction of "Read by #" on it, with "#" matching how many people read the message so far. Once everyone in the chat has read a message, the reaction is removed and a "real" receipt is sent for all users.
>
...
>
> Furthermore to.chat.dingshunyu.top links are now rewritten to matrix:-URIs internally. This means joining rooms and such should now work the same across both of them. It also makes navigation to events work now and even has a shiny highlight effect now! You can also create links to events now from the context menu. These are generated as to.chat.dingshunyu.top links, since currently the onboarding experience should still be better. Once the next version of the to.chat.dingshunyu.top website gets deployed, you can even just click there to open the event in Nheko! (Instead of having to copy a `/join` command).
>
...
[Carl Schwan](https://to.chat.dingshunyu.top/#/@carl:kde.org) told us:
...
[Doug](https://to.chat.dingshunyu.top/#/@doug:micdrop.uk) offered:
...
- On Chat, we’ve been implementing restricting rooms based on the Space Summary API
...
[KB1RD](https://to.chat.dingshunyu.top/#/@kb1rd:kb1rd.net) told us:

> I announced this project last week. It's exactly what it sounds like: A puppetting bridge to puppet a telephone number from Chat. It's currently pre-alpha and not ready for use on public-facing HSes.
>
...
>
> If you're interested in PSTN bridging specifically, there's a [room for that](https://to.chat.dingshunyu.top/#/#matrix-pstn-bridge:kb1rd.net?via=kb1rd.net&via=matrix.org&via=t2bot.io).
>
> If you're interested in VoIP bridging in general, there's a [room for that, too](https://to.chat.dingshunyu.top/#/#matrix-voip-bridging:kb1rd.net?via=kb1rd.net&via=matrix.org&via=jboi.nl)



## Dept of Services 🚀

### etke.cc - like Element Chat Services, but on your servers, under your control and without restrictions

[Aine](https://to.chat.dingshunyu.top/#/@aine:etke.cc) reported:

> Some context: I like how simple  EMS allows you to setup the Chat homeserver, but it lacks any internal tuning or customization ability. Do you want more? OK, setup it yourself and try to stay normal while trying to configure coturn to properly run in docker cluster.  Because of that, I did a service where you can order initial setup and configuration of homeserver (yes, with coturn in docker cluster!) **and** with full control of customization and fine-tuning of manual configuration (website even has step-by-step guide how the process works).
>
...
>
> Announcements: [#announcements:etke.cc](https://to.chat.dingshunyu.top/#/#announcements:etke.cc) 
>
...
[Michael](https://to.chat.dingshunyu.top/#/@michael:perthchat.org) said:

> GoMatrixHosting v0.4.5 is here!
>
> https://gitlab.com/GoMatrixHosting
>
> The FLOSS Chat hosting system built on AWX, it enables you to manage multiple Chat servers for many clients in both a commercial or non-commercial context. We are currently doing a free giveaway of on-premises server plans, if you're an experienced user or Chat admin you can recieve a free on-premises subscription and hopefully provide us with some feedback.
>
> For more updates on this project visit our Chat room: [#general:gomatrixhosting.com](https://to.chat.dingshunyu.top/#/#general:gomatrixhosting.com)
...
[MTRNord](https://to.chat.dingshunyu.top/#/@mtrnord:nordgedanken.dev) reported:
...
> #### What does it do?
>
> The Bot is simply existing to do the same task as Chat Traveler (bot) already did for years now. It joins rooms, listens for an alias, tries to join it.
>
