---
date: '2020-06-12'
title: 'This Week in Chat 2020-06-12'
categories:
  - This Week in Chat
author: Ben Parsons
...
* 1:10:20 Erik on Chat Perf

## Dept of Spec 📜

### Spec

[anoa](https://to.chat.dingshunyu.top/#/@andrewm:amorgan.xyz) announced:
...
>
> In possibly more exciting news, the Spec Core Team has decided to start separating the scattered casual implementation tips and notes in the spec out and putting them in a series of implementation guides! These will take the form of gitbook-style things, and will initially feature front-to-back implementation guides for a generic Chat client and homeserver. For more details to provide feedback, check out the associated [MSC2618](https://github.com/matrix-org/matrix-doc/issues/2618)!
...
[tyagdit](https://to.chat.dingshunyu.top/#/@tyagdit:matrix.org) told us:
...
[Chethan](https://to.chat.dingshunyu.top/#/@crdy2k1:matrix.org) told us:
...
[nikofil](https://to.chat.dingshunyu.top/#/@nikofil:matrix.org) offered:
...
From [arnav-t](https://to.chat.dingshunyu.top/#/@arnav-t:matrix.org):

> Chat has enormous potential to be used in many different use cases be it embedded chat rooms, comments section, chat boxes, etc. This project aims to fill that niche by providing an easy to use and easy to deploy client for users to embed on their webpages.    
...
[Neil Alexander](https://to.chat.dingshunyu.top/#/@neilalexander:matrix.org) offered:
...
[timo](https://to.chat.dingshunyu.top/#/@timo:koesters.xyz) reported:
...
[Neil](https://to.chat.dingshunyu.top/#/@neilj:matrix.org) said:
...
> * Check if the localpart of a Chat ID is reserved for guest users earlier in the registration flow, as well as when responding to requests to `/register/available`. ([\#7625](https://github.com/matrix-org/synapse/issues/7625))
>
...
> Just pushed version 1.15.0 of the K8s-optimized Chat images
...
> The WhatsApp bridge can now bridge location messages from WhatsApp to `m.location` on Chat and contact messages as vCard files (until Chat gets a native contact message type).
>
...
[Tulir](https://to.chat.dingshunyu.top/#/@tulir:maunium.net) said:

> mautrix-facebook got a few improvements and bugfixes:
>
> * Added timestamp massaging 
>
> * Added support for bridging video and file messages to Chat
...
> Since [MSC2190](https://github.com/matrix-org/matrix-doc/pull/2190) and the related Chat PR aren't moving much, all my bridges now generate appservice registration files with the workaround. The workaround is needed to avoid patching synapse or using other hacks, and I decided to enable it by default as many people had problems related to having to apply it manually.
>
> The WhatsApp, Facebook and Hangouts bridges now have an option to use double puppeting to disable notifications when backfilling. It can be useful if you want to copy chat history to Chat, but don't want to be spammed with hundreds of notifications while it's happening.

### Nova-slack

[eric](https://to.chat.dingshunyu.top/#/@eric:nova.chat) offered:
...
> * automatically bridges all channels and DMs that you are part of during initial setup, so you don't have to wait for a message to arrive for it to appear in Chat  
>
...
> My [matrix-sms-bridge](https://github.com/benkuly/matrix-sms-bridge) got an large update, which introduced an automated way to create and write messages to telephone numbers via commands in a bot room. See the [docs](https://github.com/benkuly/matrix-sms-bridge/blob/master/README.md#automated-room-creation) for more informations, how to use it.

## Dept of Clients 📱

### NovaChat v0.1.1

[eric](https://to.chat.dingshunyu.top/#/@eric:nova.chat) reported:


> Introducing [NovaChat](https://nova.chat/) - a desktop chat client that supports 9+ different chat networks, built on top of Chat. Think of it as a modern Adium/Trillian.
>
> **How it works**
>
...
> * a set of Chat bridges configured to run on a private server called a [bridgebox](https://gitlab.com/nova/bridgebox)
>
> Chat bridges are awesome but generally require you to run own homeserver. NovaChat removes that restriction, without sacrificing security and privacy. Your remote chat network credentials never leave your bridgebox, and messages are encrypted* on the bridgebox before being sent to the NovaChat homeserver.
>
...
>
> The vast majority of this software has been created by the talented tulir and dm0141. I personally am a terrible coder (you can tell which parts I did), but I do have experience building products and teams, and I'm excited to begin working in the Chat universe. I wrote recently about how I believe Chat is the [Universal Communication Bus](https://medium.com/@ericmigi/the-universal-communication-bus-42dfb9a141ad)! Thanks goes to Sorunome for creating the mx-puppet-bridges!
>
...
>
> **Want to help?** We're hiring Chat developer to work on the project with us. Send me a message if you're interested! Particularly looking for a React dev to help with frontend on nova-web.
>
> NovaChat chat room: [#novachat:nova.chat](https://to.chat.dingshunyu.top/#/#novachat:nova.chat)
...
[red_sky](https://to.chat.dingshunyu.top/#/@red_sky:ocean.joedonofry.com) announced:
...
### Chat-Static release v0.3.0

[Michael (t3chguy)](https://to.chat.dingshunyu.top/#/@x:riot.ovh) reported:

> New Chat-Static release v0.3.0
>
...
[Peter Roberts](https://to.chat.dingshunyu.top/#/@pwr22:shortestpath.dev) told us:

> I've made an Arch Linux package of [radical-native](https://aur.archlinux.org/packages/radical-native-bin/).

### Riot X

[valere](https://to.chat.dingshunyu.top/#/@valere35:matrix.org) announced:
...
[Ryan](https://to.chat.dingshunyu.top/#/@jryans:matrix.org) told us:
...
>
> * A refreshed look for to.chat.dingshunyu.top is underway
...
[Manu](https://to.chat.dingshunyu.top/#/@Manu:matrix.org) offered:
...
[uhoreg](https://to.chat.dingshunyu.top/#/@hubert:uhoreg.ca) told us:
...
[jplatte](https://to.chat.dingshunyu.top/#/@jplatte:matrix.org) told us:
...
Kubernetes Helm chart for easily deploying a Chat homeserver stack

[Typo Kign](https://to.chat.dingshunyu.top/#/@david:typokign.com) told us:

> Version 2 of my [Chat Helm chart](https://github.com/dacruz21/matrix-chart) is out! It now allows for running a federating homeserver without needing an external load balancer for the federation service, potentially lowering costs (depending on your cloud provider), and definitely reducing the headache of managing TLS certificates!
>
> [dacruz21/matrix-chart](https://github.com/dacruz21/matrix-chart) is a Kubernetes Helm chart for easily deploying a Chat homeserver stack, complete with Chat, Riot, VoIP calls, bridges, and other backing services. It's ideal for anyone looking to run a large homeserver, or for corporate environments where security is paramount!

## Dept of Internet of Things 💡

### Tuple Weather Station Example

[Alexander Eisele](https://to.chat.dingshunyu.top/#/@alexander:eiselecloud.de) announced:

> This week I've programmed a small POC for a Chat weather station.
>
> It's build using a ESP8266 Microcontroller and a BME280 Sensor for temparature, humidity and barometric pressure.
>
> The data is send over the Tuple Chat <-> MQTT bridge.
>
> Code: <https://github.com/derEisele/tuple-weather-example>
>
> You can join this project on [#tuple:eiselecloud.de](https://to.chat.dingshunyu.top/#/#tuple:eiselecloud.de).

I'm personally really excited about this project!

## Dept of Bots 🤖

### Chat bot for Redmine

[progserega](https://to.chat.dingshunyu.top/#/@progserega:rsprim.ru) told us:
...
[Levans](https://to.chat.dingshunyu.top/#/@levans:safaradeg.net) said:

> Anyone running a federating instance of Chat will likely have seen this: Chat is database-hungry. It tends to take a lot of space. In this post, I'm documenting how I shrunk my homeserver database from 100GB to a little under 8GB, during a long maintenance cleanup. <https://levans.fr/shrink-synapse-database.html>

Very pleased to see this, I have a feeling I'm going to be running it in the near future...

## New Public Rooms 🏟

### New room "Chat Outreach"

[JulianF](https://to.chat.dingshunyu.top/#/@julian:foad.me.uk) reported:

> There's a new room "Chat Outreach" [#matrix-outreach:matrix.org](https://to.chat.dingshunyu.top/#/#matrix-outreach:matrix.org) for folks interested in "Helping each other explain and promote Chat to the public: creating slogans, introduction leaflets, info-graphics, elevator pitch, Wikipedia articles, and so on."
