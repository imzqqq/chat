---
date: '2020-09-04'
title: 'This Week in Chat 2020-09-04'
categories:
  - This Week in Chat
author: Ben Parsons
...
[Alexander Olofsson](https://to.chat.dingshunyu.top/#/@aleol57:liu.se) reported:

> The Chat service at Linköping University has now passed its soft launch state and headed straight into full and proper deployment territory. (Though we're still not being overly vocal about it, don't want to crash our deployment on day one after all.)
...
> And just to add another slight update: already we have at least one conference planning on trying out the new Chat server as well, so the user count might just end up exploding soon anyway.

\o/

## Dept of Spec 📜

[sorunome](https://to.chat.dingshunyu.top/#/@sorunome:sorunome.de) asks for attention on MSC2545:
...
>
> Brendan is polishing his unread counts implementations, while Rich is writing up the push rules replacement tech in an MSC. Rich has also been optimizing the device code in Chat to both simplify the code, as well as cut down on unnecessary device data being sent over federation.
>
> Patrick has finished converting Chat's database code to be fully async! This has the benefit of making profiling the matrix.org Chat process much more sane, and we're already starting to find a few little places where Chat is burning CPU unnecessarily! Now, Chat won't be fully async'd until Twisted itself is, and we drop support for older Twisted versions that aren't, which is not *soon*. However the majority of the code that developers touch and that is executed being async has massive benefits either way 🙂
>
> Andrew has been working on having a confirmation pop up after users click the link in their email, to help prevent phishing scams. He's also trying to finally make Chat spec-compliant in accepting identifier dictionaries during user-interactive authentication.
>
> Olivier has been continuing to help Chat recover missed messages better after it has been offline for an extended period of time, as well as various fixes to push. 
>
...
>
> If contributing to Dendrite sounds like something you would be interested in, please take a look at these issues and join us in [#dendrite-dev:matrix.org](https://to.chat.dingshunyu.top/#/#dendrite-dev:matrix.org)! There's also [#dendrite:matrix.org](https://to.chat.dingshunyu.top/#/#dendrite:matrix.org) for general Dendrite chat and updates.
...
>
> [Chat](https://github.com/YunoHost-Apps/synapse_ynh/tree/master) integration had been updated to 1.18.0 (1.19.1 available in branch `testing`)
>
...
[Lieuwe](https://to.chat.dingshunyu.top/#/@lieuwe:lieuwe.xyz) told us:

> Introducing a new bridge to the family, tomsg-matrix!
>
> A friend of mine preferred using [his own chat protocol](https://git.tomsmeding.com/tomsg/), but having a chat protocol is useless without having people to talk to. Since I prefer using Chat I thought it would be a fun exercise to try and create a bridge using Rust.
>
...
>
> Come chat to us over at [#tomsg:lieuwe.xyz](https://to.chat.dingshunyu.top/#/#tomsg:lieuwe.xyz), hoping that it doesn't break. :P

Making your own chat protocol in a world of well established options seems, frankly- oh right.

### matrix-sms-bridge

[Benedict](https://to.chat.dingshunyu.top/#/@benedict:imbitbu.de) told us:
...
[progserega](https://to.chat.dingshunyu.top/#/@progserega:rsprim.ru) said:
...
Ditto is a Chat client for iOS & Android

[Annie](https://to.chat.dingshunyu.top/#/@annie:ditto.chat) offered:
...
> Links:
>
> [Chat Room](https://to.chat.dingshunyu.top/#/#ditto:ditto.chat)
...
YESSSS - see Chat Live for a demo.

![2020-09-04-t4hyh-image.png](/blog/img/2020-09-04-t4hyh-image.png)

### gomuks

[gomuks](https://maunium.net/go/gomuks) is a terminal based Chat client written in Go. [Source on GitHub](https://github.com/tulir/gomuks)
...
[eric](https://to.chat.dingshunyu.top/#/@eric:nova.chat) told us:

> [NovaChat](https://nova.chat/) is a new Chat-based desktop client that aggregates all your chat networks into one app. We now have integrated bridges for Whatsapp, Twitter, Telegram, Hangouts, Slack, Messenger, Android Messages (SMS), Skype, Discord, Instagram and IRC.
>
> #### Sept 4 Updates:
>
...
[SpiritCroc](https://to.chat.dingshunyu.top/#/@spiritcroc:matrix.org) offered:
...
> We have just merged a few big PRs to develop: add emails and phone numbers to Chat account, rework of upload attachment management, and other stuff related to 1-1 calls. It will live on `develop` for the week-end and we will prepare the release v1.0.6 at the beginning of next week.
...
>
> Links: [Docs (start here!)](https://rnmatrix.com) | [Chat Room](https://to.chat.dingshunyu.top/#/#rnmatrix:ditto.chat)

NB, this update should be read together with the Ditto update above

### Ruma

[iinuwa](https://to.chat.dingshunyu.top/#/@iinuwa:matrix.org) said:
...
> [matrix-docker-ansible-deploy](https://github.com/spantaleev/matrix-docker-ansible-deploy) can now help you set up [matrix-registration](https://github.com/ZerataX/matrix-registration) - an application that lets you keep your Chat server's registration private, but still allow certain users (those having a unique registration link) to register by themselves.
>
...
### Chat talk @ FrOSCon 2020 (in German)

[Oleg](https://to.chat.dingshunyu.top/#/@oleg:fiksel.info) reported:

> I did a talk on Chat bridges at the annual Free Open Source Conference.
>
...
> The interest was high. Some didn't know, that bridges exist in Chat.
