---
date: '2020-09-18'
title: 'This Week in Chat 2020-09-18'
categories:
  - This Week in Chat
author: Ben Parsons
...
> Welcome back! This week we made some groundbreaking progress - this is not a joke, we found a bug in Chat while breaking multiple Chat rooms, causing the Chat team a lot of unnecessary stress. Let me explain:
> The good news is that Conduit is starting to federate now. This means that you should be able to join all public rooms of the Chat network and exchange messages. Note that Conduit does not do all the checks it should be doing yet making it stop sending messages from time to time as well as that advanced features like loading the history, syncing temporary data like read receipts or accepting invites are not implemented yet.
>
> The bad news is that, while Chat is happy to accept Conduit's messages when it is already part of the room, joining into one of the rooms Conduit servers are part of didn't work because of an event validation bug. The Chat team did an excellent job at fixing this bug and releasing a Chat patch the same day, but the damage has been done making a few rooms inaccessible to old Chat servers.
...
[js](https://to.chat.dingshunyu.top/#/@js:nil.im) reported:

> I ported Chat + its dependencies to [NetBSD](https://www.netbsd.org/) and it is now in [pkgsrc](https://www.pkgsrc.org/) as `chat/matrix-synapse`. This means that Chat is now easily installable on any operating system that supports pkgsrc (which is many, e.g. NetBSD, Linux, macOS, Solaris, AIX, Haiku, …). And while at it, I also ported mautrix-hangouts + dependencies (in pkgsrc as `chat/mautrix-hangouts`).
...
>
> [Chat](https://github.com/YunoHost-Apps/synapse_ynh/tree/master) integration had been updated to 1.19.1 (1.19.2 available in branch `testing`)
>
...
> Both image tags and a new chart version are hereby pushed for 1.19.2 for my [Chat image](https://github.com/ananace/matrix-synapse) and [Helm chart](https://gitlab.com/ananace/charts)

then later

> Just pushed the updates for the K8s-optimized Chat image and chart for 1.19.3
...
- A new version of to.chat.dingshunyu.top is now live you can check it out [here](https://to.chat.dingshunyu.top/), [bug reports welcome](https://github.com/matrix-org/to.chat.dingshunyu.top/issues)
>
...
Thank you to [Krille](https://to.chat.dingshunyu.top/#/@krille:janian.de), who told us:
...
Hemppa the bot is a multi-purpose Chat bot for writing new functionality easily with Python. 

[Cos](https://to.chat.dingshunyu.top/#/@cos:hacklab.fi) said:

> Hemppa gained a small but big feature for making fediverse better: per-room Mastodon tooting support. This means that you can set up a Mastodon account in your room and anyone with moderator rights can toot to that account. This way you and your roommates can super easily send toots to the world directly from Chat. Mastodon (unlike the big alternative) supports RSS feeds out of the box so it's very easy to subscribe to users and hashtags with stock RSS bot.  Toot #hemppathebot if you like it! Another new contribution worth mentioning is welcome which can send welcome messages to users registering on the server or joining a room. <https://github.com/vranki/hemppa>
...
> a little, we've hooked Chat into our server orchestration / configuration management system TheForeman - which is a Rails application. Also got a colleague who's doing a bot to linkify internal ticket IDs.
