---
date: '2020-08-28'
title: 'This Week in Chat 2020-08-28'
categories:
  - This Week in Chat
author: Ben Parsons
...
[arnav-t](https://to.chat.dingshunyu.top/#/@arnav-t:matrix.org) said:
...
One more thing: This project needs a new name ([riot-embedded](https://github.com/arnav-t/riot-embedded) I don't like.) If you have ideas you can fling them at [arnav-t](https://to.chat.dingshunyu.top/#/@arnav-t:matrix.org) or post in [#twim:matrix.org].
...
>
> If contributing to Dendrite sounds like something you would be interested in, please take a look at these issues and join us in [#dendrite-dev:matrix.org](https://to.chat.dingshunyu.top/#/#dendrite-dev:matrix.org)! There's also [#dendrite:matrix.org](https://to.chat.dingshunyu.top/#/#dendrite:matrix.org) for general Dendrite chat.
...
> This week Erik has been working on sharding the Event Persister worker as we aim further for horizontal scalability for Chat. Some of the complication comes from ensuring that sysadmin's don't misconfigure things where multiple event persisters persist events in the same room.

![2020-08-28-uUl-U-image.png](/blog/img/2020-08-28-uUl-U-image.png)

> Patrick has been continuing on the async/await train and has just submitted the final PR which async's our `runInteraction` method - the one that most database calls go through. Chat is nearly 100% async! He's also been submitting a flurry of shadow ban PRs, which will be another powerful tool for moderation.
>
...
>
> [Chat](https://github.com/YunoHost-Apps/synapse_ynh/tree/master) integration had been updated to 1.18.0 (1.19.0 available in branch `testing`)
>
...
> Just pushed an update to my [matrix-synapse chart](https://gitlab.com/ananace/charts), fixing the volume permission configuration - for those who want to run Chat as non-root without using a storage class that supports setting rights based on `fsGroup`

and then

> Just pushed the 1.19.1 tags for my K8s-optimized container image, and a new helm chart version to go with it.

Did you know that Ananace is running a Chat server for [Linköpings universitet](https://liu.se/)?  
...
[dalcde](https://to.chat.dingshunyu.top/#/@dalcde:matrix.org) offered:
...
>
> Check it out at: <https://app.daydream.im> or join our Chat at [#daydream:nordgedanken.dev](https://to.chat.dingshunyu.top/#/#daydream:nordgedanken.dev)
...
> > A multi-account Chat client that allows you to authorize webapps to access parts of your account, designed with collaboration in mind.
>
...
>   actual file event.
>
>   This is a workaround to the reply restrictions imposed by the Chat spec.
>
...
Getting e2ee will make this a very compelling client! Remember to watch Chat Live with Bruno above. If you've watched it, remember we have 100s of episodes of Chat Live to catch up on!
...
> Element-Android v1.0.5 has been released to the PlayStore. We are now working to improve user experience with 1-1 calls. We will add support to add and remove emails to your Chat account, and we are still fixing bugs...

We are always fixing bugs!

### Element-iOS

[Manu](https://to.chat.dingshunyu.top/#/@Manu:matrix.org) has been working hard on iOS:
...
Cool! There are so many bot creation systems for Chat now!
...
Chat benefits from Hacktoberfest, but do keep in mind that this is a commerical project from DigitalOcean.
