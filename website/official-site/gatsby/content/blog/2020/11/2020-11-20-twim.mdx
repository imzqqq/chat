---
date: '2020-11-20'
title: 'This Week in Chat 2020-11-20'
categories:
  - This Week in Chat
author: Ben Parsons
...
1. Oleg presents his Chat home-automation setup, which uses Opsdroid, home-assistant and the reminder maubot.
...
6. Bala from Noteworthy presents *Deploy a Chat home server in 2 minutes with Noteworthy* (set to royalty free music!)
...
> The Chat deployment for the Bundeswehr (German Armed Forces) has gone live - details (in German) at <https://messenger.bwi.de/> and <https://www.bwi.de/news-blog/news/artikel/open-source-matrix-ist-einheitlicher-messenger-standard-fuer-die-bundeswehr> and further coverage at <https://www.egovernment-computing.de/bwi-und-bundeswehr-setzen-auf-open-source-a-980033/>

[Oleg](https://to.chat.dingshunyu.top/#/@oleg:fiksel.info) (same fellow from the video - wow!) added:

> Heise (German news portal) also quoted the news: <https://www.heise.de/news/Chat-steht-als-Messenger-fuer-Soldaten-und-zivile-Angehoerige-zur-Verfuegung-4963211.html>

[David Mehren](https://to.chat.dingshunyu.top/#/@davebloggt:kif.rocks) linked to Element's own coverage:
...
[wbamberg](https://to.chat.dingshunyu.top/#/@wbamberg:matrix.org) said:
...
>
> Spec compliance is unchanged, although some Chat-specific tests have been removed and some new tests have been added:
>
> * Client-server APIs: 58%, same as last week
>
> * Server-server APIs: 83%, same as last week
>
> As always, feel free to join us in [#dendrite:matrix.org](https://to.chat.dingshunyu.top/#/#dendrite:matrix.org) for general Dendrite chat or [#dendrite-dev:matrix.org](https://to.chat.dingshunyu.top/#/#dendrite-dev:matrix.org) for development discussion.

### Chat

[callahad](https://to.chat.dingshunyu.top/#/@callahad:matrix.org) offered:

> We released Chat 1.23.0 on Wednesday! Read all about it on the [Chat Blog](https://matrix.org/blog/2020/11/18/synapse-1-23-0-released). Otherwise, we'd like to highlight a few developments over the past week:
>
...
>
> Lastly, we'd like to take this opportunity to remind you to **please regularly upgrade your Chat.** Especially if you're not yet on 1.20.0, as we'll be disclosing a denial of service issue which affects older versions on Monday.

### maunium/synapse

[Tulir](https://to.chat.dingshunyu.top/#/@tulir:maunium.net) said:

> mewmew wanted to use my fork of Chat, so I made the changes there configurable (instead of hardcoding to my user ID) and even added a readme to list the features. It might also be useful for other people who want custom room IDs and other fun stuff.
>
...
> And just to be on time, I just pushed an updated tag and new version of my K8s [container image](https://github.com/ananace/matrix-synapse) and [chart](https://gitlab.com/ananace/charts) for Chat 1.23.0

### Dendrite

Dendrite is a next-generation homeserver written in Go

[TR_SLimey](https://to.chat.dingshunyu.top/#/@tr_slimey:an-atom-in.space) offered:
...
[sorunome](https://to.chat.dingshunyu.top/#/@sorunome:sorunome.de) told us:
...
> The native Gitter <-> Chat bridge is in production! 🚀🚀 We're only testing it out in a single room but feel free to come by and send a message 😀
>
> <https://gitter.im/gitter/testing-matrix-bridge> <-> [#5faa0809d73408ce4ff3ad8e:gitter.im](https://to.chat.dingshunyu.top/#/#5faa0809d73408ce4ff3ad8e:gitter.im)
>
> We also have edits from the Gitter side flowing to Chat and soon to support [edits from Chat](https://gitlab.com/gitterHQ/webapp/-/merge_requests/2058). You can track our full progress from the [GitLab epic for the native Gitter <-> Chat bridge](https://gitlab.com/groups/gitterHQ/-/epics/10).

![2020-11-20-YVtF0-editingmessages.gif](/blog/img/2020-11-20-YVtF0-editingmessages.gif)

### mx-puppet-vk

[Coma Grayce](https://to.chat.dingshunyu.top/#/@commagray:sunbutt.faith) told us:

> Hey, our team of colorful ponies proud to introduce you a new Chat bridge to VK.com chats. It's powered by mx-puppet-bridge and so far supports almost all of core features people want to see, like replies, message edits and file attachments.
>
...
[Nico (@deepbluev7:neko.dev)](https://to.chat.dingshunyu.top/#/@deepbluev7:neko.dev) announced:
...
> *   **Social login**
>
>     *   To make authentication easier, we’ve started initial implementations of SSO in Element, exploring how homeservers & Chat clients can support multiple SSO providers. Most of the work so far is captured in [MSC2858](https://github.com/matrix-org/matrix-doc/pull/2858).
>
>
...
> *   This week:
>
>     *   Release including a new way to invite friends to Chat and to Element.



### Hydrogen

[Bruno](https://to.chat.dingshunyu.top/#/@bwindels:matrix.org) reported:
...
[jplatte](https://to.chat.dingshunyu.top/#/@jplatte:privacytools.io) said:
...
> * [Alejandro Domínguez](https://github.com/aledomu) added support for deserializing string power levels (requires the `unstable-synapse-quirks` feature because these events are invalid according to the spec but Chat used to (?) accept them)
...
> Just released version 2.2.0 of the [Chat Ruby SDK](https://github.com/ananace/ruby-matrix-sdk) with the help of the community, with this release support for JRuby is improved - though still not perfect, in the higher-level abstraction direct messaging rooms are now exposed for all users - as well as all that the current user has, and the lower-level abstraction sees the addition of the CS API method to get an event context.
>
> And just to reiterate; if you're using the gem - or have questions/comments about it. please do drop into the discussion room at [#ruby-matrix-sdk:kittenface.studio](https://to.chat.dingshunyu.top/#/#ruby-matrix-sdk:kittenface.studio).



## Dept of Bots 🤖

You can see demos of both bots in this section by watching Chat Live \o/

### [home-assistant-bot v2.0.0 released](https://gitlab.com/olegfiksel/home-assistant-bot/-/releases/v2.0.0)

[Oleg](https://to.chat.dingshunyu.top/#/@oleg:fiksel.info) told us:
...
> #### Feedback
>
> * Join us at [#home-assistant-bot:fiksel.info](https://to.chat.dingshunyu.top/#/#home-assistant-bot:fiksel.info) [#opsdroid-general:matrix.org](https://to.chat.dingshunyu.top/#/#opsdroid-general:matrix.org)
...
>
> Fell free to join us at [#timetracking-bot:famedly.de](https://to.chat.dingshunyu.top/#/#timetracking-bot:famedly.de) . As soon as the repo gets opened up to public we will announce it in TWIM. Feel free to give suggestions or wishes in our room :)
>
