---
date: '2016-02-10'
title: Advanced Chat setup with Let's Encrypt
categories:
  - General
author: David Baker
---

So, you've installed an configured synapse and started chatting from your very own Chat home server? What's the next step? Well, right now you're probably accessing your new home server over plaintext HTTP, which is bad, particularly because you'll be sending your password over this connection when you log in. You could connect to Chat's secure HTTP port, but your browser won't trust it by default because you'd normally need to pay for a certificate that your browser would recognise. That is, until recently!

<a href="https://letsencrypt.org/" target="_blank">Let's Encrypt</a> is a new initiative that issues SSL certificates free of charge, in an effort to make SSL universal on the Internet. In this blog post, we'll be walking through an example of how we can use this service to get ourselves a securely hosted Chat.

We're going to assume you have a Chat installed and listening on the standard ports, 8008 and 8448. If not, follow the <a href="https://github.com/matrix-org/synapse/blob/master/README.rst" target="_blank">Chat README</a> and come back here when you're done. Everybody ready? Okay, good.

So, in order to get a certificate from Let's Encrypt, we need to prove that we own our domain. The simplest way to do this is to host some special files on our web server. Now, Chat won't do this. We could use a separate web server, but then we'd have to stop Chat and start the other web server every time we renewed our certificate, and that means downtime. Instead, let's put our Chat behind a proper web server and let that serve the files. This has added advantages, including that we can host our Chat home server on the standard port 443 without having to run Chat as root.
...
<p class="p1">Now, store those keys somewhere safe! After you've done that, let's generate a certificate request for our domain. Note that we're requesting one for both example.com and www.example.com: this isn't strictly necessary for Chat but could be useful if we want to host a website too.</p>
...
<p class="p1">Now, one more crucial thing we have to do before using our SSL is to give NGINX some Diffie Hellman parameters. This is a good thing to do for any SSL configuration (it will increase your score on <a href="https://www.ssllabs.com/ssltest/" target="_blank">SSL Labs</a>) but it's absolutely crucial for us because Chat will only negotiate forward secret connections, so otherwise other Chat home servers will refuse to talk to us! (Technically, Chat also support elliptic curve Diffie Hellman, which doesn't need DH parameters, but not all Synapses will support this.) You'll already have some Diffie Hellman parameters from you existing Chat, so you could use them:</p>
...
<p class="p1">Chat will automatically pick up the new certificate, but we'll need to tell NGINX to reload:</p>
...
<p class="p2">Don't restart NGINX just yet: we need to tell our Chat to stop listening on that port first, so lets do that and give it our new certificate:</p>
...
no_tls: True
  - port: 8008
    ...
    type: http
    # Set this so that Chat obeys nginx's X-Forwarded-For headers, then IP addresess will be correct in Chat's logs
    x_forwarded: true
    resources:
...
<p class="p2">Now let's restart Chat and our web server to swap over what's listening on our port 8448:</p>
