...
if grep -q "chat-desktop-$package_add" "$package_file"; then
    echo "Abort, $package_add already active"
    exit 0
fi

# Analog to Chat-Android's alternative_package.sh
logo_replace_color() {
    local file="$1"
    local color_shell="$2"
...
sed -i "s|Chat|Chat.$name_add|g" "$package_file"
sed -i "s|chat-desktop|chat-desktop-$package_add|g" "$package_file"
