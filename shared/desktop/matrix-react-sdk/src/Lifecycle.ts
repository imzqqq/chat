...
/**
 * Called at startup, to attempt to build a logged-in Chat session. It tries
 * a number of things:
 *
 ...
 *
 * Stops the existing Chat client (without clearing its data) and starts a
 * new one in its place. This additionally starts all other react-sdk services
 * which use the new Chat client.
 *
...
async function startMatrixClient(startSyncing = true): Promise<void> {
    ...
    // Now that we have a MatrixClientPeg, update the Meet info
    await Meet.getInstance().start();
