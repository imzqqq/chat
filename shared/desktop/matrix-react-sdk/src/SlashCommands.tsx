    ...
    new Command({
        ...
        runFn: function(roomId, args) {
            if (args) {
                ...
                //   /join https://to.chat.dingshunyu.top/#/!example:example.org?via=altserver.com
                // The command also supports event permalinks transparently:
                //   /join https://to.chat.dingshunyu.top/#/!example:example.org/$something:example.org
                //   /join https://to.chat.dingshunyu.top/#/!example:example.org/$something:example.org?via=altserver.com
                ...
                if (params[0].startsWith("http:") || params[0].startsWith("https:")) {
                    ...
                    // if we're using a Chat permalink handler, this will catch it before we get much further.
    ...
    new Command({
        ...
        runFn: function(roomId, widgetUrl) {
            ...
            if (WidgetUtils.canUserModifyWidgets(roomId)) {
                ...
                // Make the widget a Meet widget if it looks like a Meet widget
                const jitsiData = Meet.getInstance().parsePreferredConferenceUrl(widgetUrl);
                if (jitsiData) {
                    console.log("Making /addwidget widget a Meet conference");
            ...
            return success((async () => {
                if (isPhoneNumber) {
                    ...
                    if (!results || results.length === 0 || !results[0].userid) {
                        throw new Error("Unable to find Chat ID for phone number");
                    }
