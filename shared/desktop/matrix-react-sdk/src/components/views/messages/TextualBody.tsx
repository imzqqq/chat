...
export default class TextualBody extends React.Component<IBodyProps, IState> {
    ...
    private applyFormatting(): void {
        ...
        if (this.props.mxEvent.getContent().format === "org.matrix.custom.html") {
            ...
            const pres = (ReactDOM.findDOMNode(this) as Chat).getElementsByTagName("pre");
            if (pres.length > 0) {
            ...
            // Highlight code
            const codes = (ReactDOM.findDOMNode(this) as Chat).getElementsByTagName("code");
            if (codes.length > 0) {
    ...
    private activateSpoilers(nodes: ArrayLike<Chat>): void {
        let node = nodes[0];
        while (node) {
            ...
            if (node.childNodes && node.childNodes.length) {
                this.activateSpoilers(node.childNodes as NodeListOf<Chat>);
            }

            node = node.nextSibling as Chat;
        }
    }

    private findLinks(nodes: ArrayLike<Chat>): string[] {
        let links: string[] = [];
    ...
    private isLinkPreviewable(node: Chat): boolean {
