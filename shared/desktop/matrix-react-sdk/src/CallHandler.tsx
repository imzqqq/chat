...
export default class CallHandler extends EventEmitter {
    ...
    private async startCallApp(roomId: string, type: string) {
        ...
        if (jitsiWidget) {
            // If there already is a Meet widget pin it
            WidgetLayoutStore.instance.moveToContainer(room, jitsiWidget, Container.Top);
            return;
        }

        const jitsiDomain = Meet.getInstance().preferredDomain;
        const jitsiAuth = await Meet.getInstance().getJitsiAuth();
        let confId;
        if (jitsiAuth === 'openidtoken-jwt') {
            // Create conference ID from room ID
            // For compatibility with Meet, use base32 without padding.
            // More details here:
        ...
        WidgetUtils.setRoomWidget(roomId, widgetId, WidgetType.JITSI, widgetUrl, 'Meet', widgetData).then(() => {
            console.log('Meet widget added');
        }).catch((e) => {
    ...
    private terminateCallApp(roomId: string) {
        Modal.createTrackedDialog('Confirm Meet Terminate', '', QuestionDialog, {
