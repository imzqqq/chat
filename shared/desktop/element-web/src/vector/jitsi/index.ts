...
// We do not need all of React to render a Meet conference.
...
(async function() {
    try {
        ...
        // Set this up as early as possible because Chat will be hitting it almost immediately.
        ...
        console.error("Error setting up Meet widget", e);
        document.getElementById("widgetActionContainer").innerText = "Failed to load Meet widget";
    }
})();
...
function createJWTToken() {
    ...
    // Payload
    const payload = {
        // As per Meet token auth, `iss` needs to be set to something agreed between
        // JWT generating side and Prosody config. Since we have no configuration for
        // the widgets, we can't set one anywhere. Using the Meet domain here probably makes sense.
        iss: jitsiDomain,
        sub: jitsiDomain,
    ...
    // to transport data to Prosody in the Meet stack.
...
function joinConference() { // event handler bound in HTML
    ...
    console.warn(
        "[Meet Widget] The next few errors about failing to parse URL parameters are fine if " +
        "they mention 'external_api' or 'jitsi' in the stack. They're just Meet Meet trying to parse " +
