...
export default class DesktopReleaseBuilder {
    ...
    public async start(): Promise<void> {
        logger.info(`Starting Chat Desktop ${this.desktopBranch} release builder...`);
        this.building = false;
    ...
    private async writeElectronBuilderConfigFile(
        ...
        const productName = target.platform === 'linux' ? 'Chat' : pkg.productName;
    ...
    private async buildLocal(target: Target): Promise<void> {
        ...
        if (target.platform === 'darwin') {
            ...
            for (const f of await getMatchingFilesInDir(path.join(repoDir, 'dist'), /\.dmg$/)) {
                ...
                const latestInstallPath = path.join(this.appPubDir, 'install', 'macos', 'Chat.dmg');
    ...
    private async buildWithRunner(
        ...
        // This will fetch the Chat release from GitHub that matches the version
    ...
    private async buildWin(target: WindowsTarget): Promise<void> {
        ...
        try {
            ...
            // This will fetch the Chat release from GitHub that matches the
            ...
            for (const f of await getMatchingFilesInDir(path.join(repoDir, 'dist', squirrelDir), /\.exe$/)) {
                ...
                const latestInstallPath = path.join(this.appPubDir, 'install', 'win32', archDir, 'Chat Setup.exe');
                logger.info('Update latest symlink ' + latestInstallPath + ' -> ' + f);
                await fsProm.unlink(latestInstallPath);
                await fsProm.symlink(f, latestInstallPath, 'file');
            }
