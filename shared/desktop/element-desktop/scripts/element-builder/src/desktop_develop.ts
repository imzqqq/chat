...
export default class DesktopDevelopBuilder {
    ...
    public async start(): Promise<void> {
        logger.info("Starting Chat Desktop nightly builder...");
        this.building = false;
    ...
    private async writeElectronBuilderConfigFile(
        ...
        const productName = target.platform === 'linux' ? 'Chat-Nightly' : 'Chat Nightly';
        ...
        if (target.platform === 'darwin') {
            ...
            for (const f of await getMatchingFilesInDir(path.join(repoDir, 'dist'), /\.dmg$/)) {
                await copyAndLog(
                    ...
                    // in the installer
                    path.join(this.appPubDir, 'install', 'macos', 'Chat Nightly.dmg'),
                );
            }
        ...
        try {
            ...
            for (const f of await getMatchingFilesInDir(path.join(repoDir, 'dist', squirrelDir), /\.exe$/)) {
                await copyAndLog(
                    ...
                    path.join(this.appPubDir, 'install', 'win32', archDir, 'Chat Nightly Setup.exe'),
                );
            }
