...
// async to are initialized in setupGlobals()
let asarPath;
let resPath;
let vectorConfig;
let iconPath;
...
if (userDataPathInProtocol) {
    ...
    // let oldUserDataPath = path.join(app.getPath('appData'), app.getName().replace('Chat', 'Classic_Chat'));
    // if (argv['profile']) {
...
async function setupGlobals() {
    ...
    try {
        ...
        // config. This is to avoid a problem where Classic_Chat thinks there are multiple homeservers
        ...
        if (e instanceof SyntaxError) {
            dialog.showMessageBox({
                type: "error",
                title: `Your ${vectorConfig.brand || 'Chat'} is misconfigured`,
                message: `Your custom ${vectorConfig.brand || 'Chat'} configuration contains invalid JSON. ` +
                         `Please correct the problem and reopen ${vectorConfig.brand || 'Chat'}.`,
                detail: e.message || "",
            });
        }
    ...
    trayConfig = {
        icon_path: iconPath,
        brand: vectorConfig.brand || 'Chat',
    };

    // launcher
    launcher = new AutoLaunch({
        name: vectorConfig.brand || 'Chat',
        isHidden: true,
        mac: {
            useLaunchAgent: true,
        },
...
//     // Look for an auto-launcher under 'Classic_Chat' and if we find one, port it's
//     // enabled/disbaledp-ness over to the new 'Chat' launcher
//     if (!vectorConfig.brand || vectorConfig.brand === 'Chat') {
...
const warnBeforeExit = (event, input) => {
    ...
    if (shouldWarnBeforeExit && exitShortcutPressed) {
        const shouldCancelCloseRequest = dialog.showMessageBoxSync(mainWindow, {
            type: "question",
            buttons: [_t("Cancel"), _t("Close Chat")],
...
app.setAppUserModelId('com.squirrel.schildichat-desktop.Chat');
