import { defaultTheme, themes } from "../types/FlowTheme";
...
export async function getConfig(): Promise<Config | undefined> {
    try {
        ...
        if (
            ...
            console.info(
                "Location does not have a forward slash, so Flow has added it automatically."
            );
            resp.data.location = location + "/";
        }

        if (process.env.NODE_ENV === "development") {
            resp.data.location = "http://localhost:3001/";
            console.info("Location field has been updated to localhost:3001.");
        }

        return resp.data as Config;
    } catch (err) {
        console.error(
            "Couldn't configure Flow with the config file. Reason: " + err.name
        );
    }
}
