...
- (void)viewDidLoad
{
    ...
    self.defaultHomeServerUrl = ChatSettings.shared.homeserverUrlString;
    
    self.defaultIdentityServerUrl = ChatSettings.shared.identityServerUrlString;
...
- (void)showAuthenticationFallBackViewController:(NSString*)fallbackPage
{
    ...
    UINavigationController *navigationController = [[ChatNavigationController alloc] initWithRootViewController:authFallBackViewController];
...
- (void)authenticationViewController:(MXKAuthenticationViewController *)authenticationViewController didLogWithUserId:(NSString *)userId
{
    ...
    // Create DM with Chat-bot on new account creation.
    if (self.authType == MXKAuthenticationTypeRegister && botCreationEnabled)
    {
        MXRoomCreationParameters *roomCreationParameters = [MXRoomCreationParameters parametersForDirectRoomWithUser:@"@riot-bot:chat.dingshunyu.top"];
