...
class SpaceMenuViewModel: SpaceMenuViewModelType {
    ...
    init(session: MXSession, spaceId: String) {
        ...
        if spaceId != SpaceListViewModel.Constants.homeSpaceId {
            ...
            self.menuItems = [
                SpaceMenuListItemViewData(action: .showAllRoomsInHomeSpace, style: .toggle, title: VectorL10n.spaceHomeShowAllRooms, icon: nil, value: ChatSettings.shared.showAllRoomsInHomeSpace)
            ]
        }
    }
    ...
    private func processAction(with action: SpaceMenuListItemAction, at indexPath: IndexPath) {
        ...
        case .showAllRoomsInHomeSpace:
            ChatSettings.shared.showAllRoomsInHomeSpace.toggle()
            self.menuItems[indexPath.row].value = ChatSettings.shared.showAllRoomsInHomeSpace
