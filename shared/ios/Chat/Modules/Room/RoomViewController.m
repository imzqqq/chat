...
{
    BOOL callOptionAllowed = (self.roomDataSource.room.isDirect && ChatSettings.shared.roomScreenAllowVoIPForDirectRoom) || (!self.roomDataSource.room.isDirect && ChatSettings.shared.roomScreenAllowVoIPForNonDirectRoom);
...
- (void)setupActions {
    ...
    if (ChatSettings.shared.roomScreenAllowMediaLibraryAction)
    {
    ...
    if (ChatSettings.shared.roomScreenAllowStickerAction)
    {
    ...
    if (ChatSettings.shared.roomScreenAllowFilesAction)
    {
    ...
    if (ChatSettings.shared.roomScreenAllowCameraAction)
    {
...
- (void)sendVideoAsset:(AVAsset *)videoAsset isPhotoLibraryAsset:(BOOL)isPhotoLibraryAsset
{
    ...
    if (ChatSettings.shared.showMediaCompressionPrompt)
    {
...
- (void)dataSource:(MXKDataSource *)dataSource didRecognizeAction:(NSString *)actionIdentifier inCell:(id<MXKCellRendering>)cell userInfo:(NSDictionary *)userInfo
{
    ...
    if (customizedRoomDataSource)
    {
        ...
        else if ([actionIdentifier isEqualToString:kMXKRoomBubbleCellChatEditButtonPressed])
        {
        ...
        else if ([actionIdentifier isEqualToString:kMXKRoomBubbleCellTapOnAttachmentView])
        {
            if (((MXKRoomBubbleTableViewCell*)cell).bubbleData.attachment.eventSentState == MXEventSentStateFailed)
            {
                ...
                if (selectedEvent)
                {
                    [self dataSource:dataSource didRecognizeAction:kMXKRoomBubbleCellChatEditButtonPressed inCell:cell userInfo:@{kMXKRoomBubbleCellEventKey:selectedEvent}];
                }
                else
                {
...
- (void)showAdditionalActionsMenuForEvent:(MXEvent*)selectedEvent inCell:(id<MXKCellRendering>)cell animated:(BOOL)animated
{
    ...
    if (selectedEvent.sentState == MXEventSentStateSent)
    {
        ...
        if (![selectedEvent.sender isEqualToString:self.mainSession.myUser.userId] && ChatSettings.shared.roomContextualMenuShowReportContentOption)
        {
...
- (void)refreshMissedDiscussionsCount:(BOOL)force
{
    ...
    // Compute the missed notifications count of the current room by considering its notification mode in Chat.
...
- (NSArray<RoomContextualMenuItem*>*)contextualMenuItemsForEvent:(MXEvent*)event andCell:(id<MXKCellRendering>)cell
{
    ...
    BOOL showMoreOption = (event.isState && ChatSettings.shared.roomContextualMenuShowMoreOptionForStates) || (!event.isState && ChatSettings.shared.roomContextualMenuShowMoreOptionForMessages);
