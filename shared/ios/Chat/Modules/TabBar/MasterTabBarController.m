...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    if (!authIsShown)
    {
        ...
        if (!ChatSettings.shared.isEnableCrashReportHasBeenSetOnce)
        {
...
- (void)refreshTabBarBadges
{
    ...
    if (ChatSettings.shared.homeScreenShowFavouritesTab)
    {
    ...
    if (ChatSettings.shared.homeScreenShowPeopleTab)
    {
    ...
    if (ChatSettings.shared.homeScreenShowRoomsTab)
    {
...
{
    if (ChatSettings.shared.hideVerifyThisSessionAlert
...
- (void)presentVerifyCurrentSessionAlertWithSession:(MXSession*)session
{
    ...
    [alert addAction:[UIAlertAction actionWithTitle:[VectorL10n doNotAskAgain]
                                            ...
                                            handler:^(UIAlertAction * action) {
                                                ChatSettings.shared.hideVerifyThisSessionAlert = YES;
                                            }]];
...
{
    if (ChatSettings.shared.hideReviewSessionsAlert || self.reviewSessionAlertHasBeenDisplayed)
    {
        return;
    }
...
- (void)presentReviewUnverifiedSessionsAlertWithSession:(MXSession*)session
{
    ...
    [alert addAction:[UIAlertAction actionWithTitle:[VectorL10n doNotAskAgain]
                                              style:UIAlertActionStyleDestructive
                                            handler:^(UIAlertAction * action) {
                                                ChatSettings.shared.hideReviewSessionsAlert = YES;
                                            }]];
...
- (void)resetReviewSessionsFlags
{
    ...
    ChatSettings.shared.hideVerifyThisSessionAlert = NO;
    ChatSettings.shared.hideReviewSessionsAlert = NO;
}
