...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    // Set up theme
    ThemeService.shared.themeId = ChatSettings.shared.userInterfaceTheme;
    ...
    if (!language)
    {
        // Check whether a langage was only defined at the Chat application level.
...
- (void)afterAppUnlockedByPin:(UIApplication *)application
{
    ...
    if (ChatSettings.shared.enableCrashReport)
    {
    ...
    // Chat has its own dark theme. Prevent iOS from applying its one
...
- (void)promptForStunServerFallback
{
    ...
    [_errorNotification addAction:[UIAlertAction actionWithTitle:[VectorL10n callNoStunServerErrorUseFallbackButton:stunFallbackHost]
        ...
        ChatSettings.shared.allowStunServerFallback = YES;
    ...
    [_errorNotification addAction:[UIAlertAction actionWithTitle:[MatrixKitL10n cancel]
                                                         ...
                                                         handler:^(UIAlertAction * action) {
                                                             ...
                                                             ChatSettings.shared.allowStunServerFallback = NO;
...
- (void)checkPermissionForNativeWidget:(Widget*)widget fromUrl:(NSURL*)url completion:(void (^)(BOOL granted))completion
{
    ...
    // Check permission in user Chat settings
...
- (void)presentGDPRConsentFromViewController:(UIViewController*)viewController consentURI:(NSString*)consentURI
{
    ...
    UINavigationController *navigationController = [[ChatNavigationController alloc] initWithRootViewController:gdprConsentViewController];
...
- (void)gdprConsentViewControllerDidConsentToGDPRWithSuccess:(GDPRConsentViewController *)gdprConsentViewController
{
    ...
    if (botCreationEnabled)
    {
        ...
        MXWeakify(self);
        void (^createChatBotDMcompletion)(void) = ^() {
            MXStrongifyAndReturnIfNil(self);

            self.onBoardingManager = nil;
        };
        
        [self.onBoardingManager createChatBotDirectMessageIfNeededWithSuccess:^{
            createChatBotDMcompletion();
        } failure:^(NSError * _Nonnull error) {
            createChatBotDMcompletion();
        }];
    }
...
{
    // Register "Chat-Defaults.plist" default values
    ...
    if (!ChatSettings.shared.isUserDefaultsMigrated)
    {
        [ChatSettings.shared migrate];
    }
    
    // Show encrypted message notification content by default.
    if (!ChatSettings.shared.isShowDecryptedContentInNotificationsHasBeenSetOnce)
    {
        ChatSettings.shared.showDecryptedContentInNotifications = BuildSettings.decryptNotificationsByDefault;
    }
}
