...
final class NewStatusViewController: UIViewController {
    private let viewModel: ViewModels.NewStatusViewModel
    private let rootViewModel: ViewModels.RootViewModel?
...
private extension NewStatusViewController {
    ...
    func presentEmojiPicker(tag: Int) {
        ...
        if fromView.inputView == nil {
            let emojiPickerViewModel = ViewModels.EmojiPickerViewModel(identityContext: viewModel.identityContext)

            emojiPickerViewModel.$alertItem.assign(to: \.alertItem, on: viewModel).store(in: &cancellables)

            let emojiPickerController =
                FlowEmojiPickerViewController(viewModel: emojiPickerViewModel) { [weak self] picker, emoji in
