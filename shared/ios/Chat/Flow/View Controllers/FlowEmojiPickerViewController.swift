...
final class FlowEmojiPickerViewController: UICollectionViewController {
    /// MARK - imzqqq, NOTE: weird bug, we cannot use the prams name `searchBar`
    let mSearchBar = UIKit.UISearchBar()
    ///

    private let viewModel: ViewModels.EmojiPickerViewModel
    private let selectionAction: (FlowEmojiPickerViewController, PickerEmoji) -> Void
    private let deletionAction: ((FlowEmojiPickerViewController) -> Void)?
    private let searchPresentationAction: ((FlowEmojiPickerViewController, UINavigationController) -> Void)?
    ...
    init(viewModel: ViewModels.EmojiPickerViewModel,
         selectionAction: @escaping (FlowEmojiPickerViewController, PickerEmoji) -> Void,
         deletionAction: ((FlowEmojiPickerViewController) -> Void)?,
         searchPresentationAction: ((FlowEmojiPickerViewController, UINavigationController) -> Void)?) {
    ...
    override func viewDidLoad() {
        ...
        mSearchBar.placeholder = NSLocalizedString("emoji.search", comment: "")
        mSearchBar.searchTextField.addAction(
            UIAction { [weak self] _ in
                self?.viewModel.query = self?.mSearchBar.text ?? ""
                self?.collectionView.setContentOffset(.zero, animated: false)
            },
            for: .editingChanged)
        navigationItem.titleView = mSearchBar

        mSearchBar.addSubview(presentSearchButton)
        ...
        NSLayoutConstraint.activate([
            presentSearchButton.leadingAnchor.constraint(equalTo: mSearchBar.leadingAnchor),
            presentSearchButton.topAnchor.constraint(equalTo: mSearchBar.topAnchor),
            presentSearchButton.trailingAnchor.constraint(equalTo: mSearchBar.trailingAnchor),
            presentSearchButton.bottomAnchor.constraint(equalTo: mSearchBar.bottomAnchor)
        ])
        ...
        if let currentKeyboardLanguageIdentifier = mSearchBar.textInputMode?.primaryLanguage {
        ...
        NotificationCenter.default.publisher(for: UITextInputMode.currentInputModeDidChangeNotification)
            .compactMap { [weak self] _ in self?.mSearchBar.textInputMode?.primaryLanguage }
...
private extension FlowEmojiPickerViewController {
