...
final class AutocompleteDataSource: UICollectionViewDiffableDataSource<AutocompleteSection, AutocompleteItem> {
    @Published private var searchViewModel: ViewModels.SearchViewModel
    @Published private var emojiPickerViewModel: ViewModels.EmojiPickerViewModel
...
parentViewModel: ViewModels.NewStatusViewModel) {
             searchViewModel = ViewModels.SearchViewModel(identityContext: parentViewModel.identityContext)
             emojiPickerViewModel = ViewModels.EmojiPickerViewModel(identityContext: parentViewModel.identityContext, queryOnly: true)
        ...
        parentViewModel.$identityContext
            ...
            .sink { [weak self] in
                ...
                self.searchViewModel = ViewModels.SearchViewModel(identityContext: $0)
                self.emojiPickerViewModel = ViewModels.EmojiPickerViewModel(identityContext: $0, queryOnly: true)
            }
            .store(in: &cancellables)
    }
...
private extension AutocompleteDataSource {
    static func combine(query: String, searchViewModel: ViewModels.SearchViewModel, emojiPickerViewModel: ViewModels.EmojiPickerViewModel) {
