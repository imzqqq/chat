...
- (void)setRoomTag:(NSString*)tag completion:(void (^)(void))completion
{
    ...
    MXLogDebug(@"[MXRoom+Chat] Update the room %@ tag from %@ to %@ with tag order %@", self.roomId, oldTag, tag, tagOrder);
    ...
    [self replaceTag:oldTag
           ...
           withOrder:tagOrder
             success: ^{
                 ...
                 MXLogDebug(@"[MXRoom+Chat] Failed to update the tag %@ of room (%@)", tag, self.roomId);
...
- (void)mute:(void (^)(void))completion
{
    ...
    else
    {
        ...
        {
            MXLogDebug(@"[MXRoom+Chat] Request in progress: ignore push rule update");
            if (completion)
            {
                completion();
            }
