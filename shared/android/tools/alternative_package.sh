...
if grep -q "com.imzqqq.app.$package_add" "$build_gradle"; then
...
sed -i "s|\"Chat|\"$name_replace|g" "$build_gradle"
sed -i "s|com.imzqqq.app|com.imzqqq.app.$package_add|g" "$build_gradle" `find "$src_dir" -name google-services.json`
sed -i "s|Chat|$name_replace|g" `find "$fastlane_dir/metadata/android" -name "title.txt"`
