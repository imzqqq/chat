...
if [ "$release_type" = "test" ]; then
    ...
    if [ -f "$HOME/fdroid/sm/data/metadata/com.imzqqq.app.a.yml" ]; then
        lastTestVersionCode="$(cat "$HOME/fdroid/sm/data/metadata/com.imzqqq.app.a.yml"|grep versionCode|tail -n 1|sed 's|.*: ||')"
    else
...
git_changelog() {
    ...
    git log $git_args --pretty=format:"- %s" "$last_tag".. --committer="$(git config user.name)" \
        ...
        | grep -v "Automatic Chat string correction" \
...
if [ "$release_type" = "test" ]; then
    ...
    if [ "$(wc -m "$changelog_file"|sed 's| .*||')" -gt "$max_changelog_len" ]; then
        ...
        changelog_add="$(echo -e "- ...\n\nAll changes: https://github.com/Chat/Chat-android/commits/$current_commit")"
