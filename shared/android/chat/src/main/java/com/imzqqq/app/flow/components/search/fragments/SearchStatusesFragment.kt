...
import com.imzqqq.app.flow.components.search.adapter.SearchStatusesAdapter
...
import com.imzqqq.app.flow.entity.Status.Mention
        ...
        searchAdapter.item(position)?.first?.actionableStatus?.let { actionable ->
            when (actionable.attachments[attachmentIndex].type) {
                ...
                Attachment.Type.UNKNOWN                                                                   -> {
                    com.imzqqq.app.flow.util.LinkHelper.openLink(actionable.attachments[attachmentIndex].url, context)
                }
            }
        }
    }
                ...
                .setPositiveButton(android.R.string.ok) { _, _ ->
                    viewModel.deleteStatus(id)
                        ...
                        .subscribe(
                            ...
                            { error ->
                                Timber.tag("SearchStatusesFragment").w(error, "error deleting status")
