...
import com.imzqqq.app.flow.appstore.Event
...
import com.imzqqq.app.flow.appstore.StatusComposedEvent
...
import com.imzqqq.app.flow.entity.Filter
import com.imzqqq.app.flow.entity.Poll
import com.imzqqq.app.flow.entity.Status
import com.imzqqq.app.flow.network.FilterModel
...
import com.imzqqq.app.flow.util.dec
import com.imzqqq.app.flow.util.firstIsInstanceOrNull
import com.imzqqq.app.flow.util.inc
import com.imzqqq.app.flow.util.isLessThan
import com.imzqqq.app.flow.util.toViewData
...
class TimelineViewModel @Inject constructor(
    ...
    private suspend fun updateCurrent() {
        ...
        val statuses = try {
            ...
            if (isExpectedRequestException(t)) {
                Timber.d(t, "Failed updating timeline")
                triggerViewUpdate()
                return
            } else {
                throw t
    ...
    fun loadGap(position: Int): Job {
        return viewModelScope.launch {
            ...
            if (statuses.size < position || position <= 0) {
                Timber.e("Wrong gap position: $position")
                return@launch
            }
            ...
            val placeholder = statuses[position].asPlaceholderOrNull() ?: run {
                Timber.e("Not a placeholder at $position")
                return@launch
            }
    ...
    fun reblog(reblog: Boolean, position: Int): Job = viewModelScope.launch {
        ...
        try {
            ...
            ifExpected(t) {
                Timber.d(t, "Failed to reblog status " + status.actionableId)
            }
        }
    }
    ...
    fun favorite(favorite: Boolean, position: Int): Job = viewModelScope.launch {
        ...
        try {
            ...
            ifExpected(t) {
                Timber.d(t, "Failed to favourite status " + status.actionableId)
            }
        }
    }
    ...
    fun voteInPoll(position: Int, choices: List<Int>): Job = viewModelScope.launch {
        ...
        val poll = status.status.poll ?: run {
            Timber.tag(TAG).w("No poll on status " + status.id)
            return@launch
        }
        ...
        try {
            ...
            ifExpected(t) {
                Timber.d(t, "Failed to vote in poll: " + status.actionableId)
            }
        }
    }
    ...
    private fun removeAllByInstance(instance: String) {
        statuses.removeAll { vd ->
            ...
            com.imzqqq.app.flow.util.LinkHelper.getDomain(status.account.url) == instance
        }
    }
    ...
    private fun extractNextId(response: Response<*>): String? {
        ...
        val nextHeader = com.imzqqq.app.flow.util.HttpHeaderLink.findByRelationType(links, "next") ?: return null
        ...
        return viewModelScope.launch {
            ...
            if (kind == Kind.HOME) {
                ...
                try {
                    ...
                    Timber.e(e, "Loading above failed")
                    if (!isExpectedRequestException(e)) {
                        throw e
    ...
    private fun reloadFilters() {
        viewModelScope.launch {
            val filters = try {
                ...
                Timber.e(t, "Failed to fetch filters")
                return@launch
            }
