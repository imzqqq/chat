...
import com.imzqqq.app.flow.util.VersionUtils
import com.imzqqq.app.flow.util.combineLiveData
import com.imzqqq.app.flow.util.filter
import com.imzqqq.app.flow.util.map
import com.imzqqq.app.flow.util.randomAlphanumericString
import com.imzqqq.app.flow.util.toLiveData
        ...
        Single.zip(
            ...
            .subscribe(
                ...
                { throwable ->
                    ...
                    Timber.tag(TAG).w(throwable, "error loading instance data")
                }
            )
            .autoDispose()
    }
        ...
        when (token[0]) {
            '@' -> {
                return try {
                    ...
                    Timber.e(e, String.format("Autocomplete search for %s failed.", token))
                    emptyList()
                }
            }
            '#' -> {
