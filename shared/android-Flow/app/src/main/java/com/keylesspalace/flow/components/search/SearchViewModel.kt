...
package com.keylesspalace.flow.components.search
...
import com.keylesspalace.flow.components.search.adapter.SearchPagingSourceFactory
...
class SearchViewModel @Inject constructor(
    ...
    private val statusesPagingSourceFactory = SearchPagingSourceFactory(flowApi, SearchType.Status, loadedStatuses) {
        it.statuses.map { status -> Pair(status, status.toViewData(alwaysShowSensitiveMedia, alwaysOpenSpoiler)) }
            .apply {
                loadedStatuses.addAll(this)
            }
    }
    private val accountsPagingSourceFactory = SearchPagingSourceFactory(flowApi, SearchType.Account) {
        it.accounts
    }
    private val hashtagsPagingSourceFactory = SearchPagingSourceFactory(flowApi, SearchType.Hashtag) {
        it.hashtags
    }
