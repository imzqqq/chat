...
package com.keylesspalace.flow.components.compose
...
import com.keylesspalace.flow.components.compose.dialog.makeCaptionDialog
import com.keylesspalace.flow.components.compose.dialog.showAddPollDialog
import com.keylesspalace.flow.components.compose.view.ComposeOptionsListener
import com.keylesspalace.flow.components.compose.view.ComposeScheduleView
import com.keylesspalace.flow.databinding.ActivityComposeBinding
...
import com.keylesspalace.flow.util.ComposeTokenizer
import com.keylesspalace.flow.util.PickMediaFiles
import com.keylesspalace.flow.util.ThemeUtils
import com.keylesspalace.flow.util.afterTextChanged
import com.keylesspalace.flow.util.combineLiveData
import com.keylesspalace.flow.util.combineOptionalLiveData
import com.keylesspalace.flow.util.hide
import com.keylesspalace.flow.util.highlightSpans
import com.keylesspalace.flow.util.loadAvatar
import com.keylesspalace.flow.util.onTextChanged
...
import com.keylesspalace.flow.util.withLifecycleContext
...
class ComposeActivity :
    ...
    public override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (theme == "black") {
            setTheme(R.style.FlowDialogActivityBlackTheme)
        }
    ...
    private fun updateSensitiveMediaToggle(markMediaSensitive: Boolean, contentWarningShown: Boolean) {
        if (viewModel.media.value.isNullOrEmpty()) {
            ...
            @ColorInt val color = if (contentWarningShown) {
                ...
                ContextCompat.getColor(this, R.color.transparent_flow_blue)
            } else {
                binding.composeHideMediaButton.isClickable = true
                if (markMediaSensitive) {
                    binding.composeHideMediaButton.setImageResource(R.drawable.ic_hide_media_24dp)
                    ContextCompat.getColor(this, R.color.flow_blue)
                } else {
    ...
    private fun updateVisibleCharactersLeft() {
        ...
        val textColor = if (remainingLength < 0) {
            ContextCompat.getColor(this, R.color.flow_red)
        } else {
            ThemeUtils.getColor(this, android.R.attr.textColorTertiary)
        }
    ...
    companion object {
        ...
        // Flow only counts URLs as this long in terms of status character limits
        @VisibleForTesting
