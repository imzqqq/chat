 ...
 *  links are checked against the api if they are flow links so they can be openend in Flow
...
abstract class BottomSheetActivity : BaseActivity() {
    ...
    open fun viewUrl(url: String, lookupFallbackBehavior: PostLookupFallbackBehavior = PostLookupFallbackBehavior.OPEN_IN_BROWSER) {
        if (!looksLikeFlowUrl(url)) {
            openLink(url)
            return
        }

        flowApi.searchObservable(
            query = url,
            resolve = true
...
fun looksLikeFlowUrl(urlString: String): Boolean {
    val uri: URI
    try {
        uri = URI(urlString)
