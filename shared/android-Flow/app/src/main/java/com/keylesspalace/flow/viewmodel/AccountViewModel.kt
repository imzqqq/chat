...
import com.keylesspalace.flow.appstore.DomainMuteEvent
...
import com.keylesspalace.flow.appstore.UnfollowEvent
import com.keylesspalace.flow.db.AccountManager
import com.keylesspalace.flow.entity.Account
import com.keylesspalace.flow.entity.Field
import com.keylesspalace.flow.entity.IdentityProof
...
import com.keylesspalace.flow.util.combineOptionalLiveData
...
class AccountViewModel @Inject constructor(
    ...
    private fun obtainRelationship(reload: Boolean = false) {
        if (relationshipData.value == null || reload) {
            ...
            flowApi.relationships(listOf(accountId))
                .subscribe(
                    { relationships ->
    ...
    private fun obtainIdentityProof(reload: Boolean = false) {
        if (identityProofData.value == null || reload) {
            ...
            flowApi.identityProofs(accountId)
                .subscribe(
                    { proofs ->
    ...
    fun changeSubscribingState() {
        val relationship = relationshipData.value?.data
        if (relationship?.notifying == true || /* Flow 3.3.0rc1 */
    ...
    fun blockDomain(instance: String) {
        flowApi.blockDomain(instance).enqueue(object : Callback<Any> {
    ...
    fun unblockDomain(instance: String) {
        flowApi.unblockDomain(instance).enqueue(object : Callback<Any> {
            override fun onResponse(call: Call<Any>, response: Response<Any>) {
                if (response.isSuccessful) {
                    val relation = relationshipData.value?.data
                    if (relation != null) {
    ...
    private fun changeRelationship(relationshipAction: RelationShipAction, parameter: Boolean? = null, duration: Int? = null) {
        ...
        val isFlow = relationshipData.value?.data?.notifying != null
        ...
        when (relationshipAction) {
            RelationShipAction.FOLLOW -> flowApi.followAccount(accountId, showReblogs = parameter ?: true)
            RelationShipAction.UNFOLLOW -> flowApi.unfollowAccount(accountId)
            RelationShipAction.BLOCK -> flowApi.blockAccount(accountId)
            RelationShipAction.UNBLOCK -> flowApi.unblockAccount(accountId)
            RelationShipAction.MUTE -> flowApi.muteAccount(accountId, parameter ?: true, duration)
            RelationShipAction.UNMUTE -> flowApi.unmuteAccount(accountId)
            RelationShipAction.SUBSCRIBE -> {
                if (isFlow)
                    flowApi.followAccount(accountId, notify = true)
                else flowApi.subscribeAccount(accountId)
            }
            RelationShipAction.UNSUBSCRIBE -> {
                if (isFlow)
                    flowApi.followAccount(accountId, notify = false)
                else flowApi.unsubscribeAccount(accountId)
            }
        }.subscribe(
            { relationship ->
    ...
    fun noteChanged(newNote: String) {
        ...
        noteDisposable = Single.timer(1500, TimeUnit.MILLISECONDS)
            .flatMap {
                flowApi.updateAccountNote(accountId, newNote)
            }
            .doOnSuccess {
